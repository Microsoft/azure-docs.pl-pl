...
> [!IMPORTANT]
> *  DHGroup2048 i PFS2048 są takie same jak grupa Diffie’ego-Hellmana **14** w funkcji PFS protokołów IKE i IPsec. Pełną listę mapowań można znaleźć w sekcji dotyczącej [grup Diffie’ego-Hellmana](#DH).
> * W przypadku algorytmów GCMAES należy określić ten sam algorytm GCMAES i długość klucza na potrzeby integralności i szyfrowania IPsec.
...
> * Okresy istnienia skojarzeń zabezpieczeń QM to parametry opcjonalne. Jeśli żaden nie został określony, są używane wartości domyślne 27 000 sekund (7,5 godz.) i 102400000 KB (102 GB).
> * UsePolicyBasedTrafficSelector to parametr opcji połączenia. Zobacz następny często zadawane pytania dotyczące "UsePolicyBasedTrafficSelectors".
...
Zobacz [Konfigurowanie zasad protokołu IPSec/IKE dla połączeń S2S lub sieci wirtualnej](../articles/vpn-gateway/vpn-gateway-ipsecikepolicy-rm-powershell.md)między sieciami wirtualnymi.