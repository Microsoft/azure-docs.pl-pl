...
5. W odszyfrowanym pliku Web. config w `connectionStrings` węźle Dodaj parametry połączenia dla wystąpienia programu SQL Server i nazwę bazy danych zawartości. Zobacz poniższy przykład.
...
* Na serwerze frontonu sieci Web w powłoce zarządzania programu SharePoint 2013 Uruchom element utrzymujący w następujący sposób:
  ...
  > [!NOTE]
  > W tej `GarbageCollection` chwili tylko ta operacja jest obsługiwana w przypadku StorSimple. Należy również zauważyć, że parametry wystawione dla elementu Microsoft. Data. SqlRemoteBlobs.. exe uwzględniają wielkość liter. 
  > 
  > 
