...
<sup>1</sup> wymaga powiązania `direction` "in" w *funkcji Function. JSON* lub `FileAccess.Read` w bibliotece klas języka C#. Można jednak użyć obiektu kontenera, który środowisko uruchomieniowe zapewnia do wykonywania operacji zapisu, takich jak przekazywanie obiektów BLOB do kontenera.

<sup>2</sup> wymaga powiązania `direction` "Inout" w *funkcji Function. JSON* lub `FileAccess.ReadWrite` w bibliotece klas języka C#.
