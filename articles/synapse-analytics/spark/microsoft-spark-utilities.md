...
description: Samouczek dotyczący używania MSSparkutils w notesach usługi Azure Synapse Analytics.
author: ruxu
...
ms.locfileid: "94428682"
---
# <a name="introduction-of-microsoft-spark-utilities"></a>Wprowadzenie do narzędzi Microsoft Spark Utilities
Microsoft Spark Utilities (MSSparkUtils) to pakiet wbudowane, który ułatwia łatwiejsze wykonywanie typowych zadań. Za pomocą MSSparkUtils można wydajnie współpracować z systemem plików, uzyskiwać zmienne środowiskowe i korzystać z wpisów tajnych. MSSparkUtils są dostępne w `PySpark (Python)` , `Scala` , i w `.NET Spark (C#)` potokach Synapse.
...
Potoki Synapse używają tożsamości obszaru roboczego (MSI) do uzyskiwania dostępu do kont magazynu. Aby można było używać MSSparkUtils w działaniach potoku, tożsamość obszaru roboczego musi być **Współautorem BLOB Storage** , aby uzyskać dostęp do konta ADLS Gen2 (lub folderu).
...
3. W przypadku roli **współautor danych obiektów blob magazynu** na koncie magazynu Przypisz **swoje konto usługi Azure AD** i **tożsamość obszaru roboczego** (taką samą jak nazwa obszaru roboczego) lub upewnij się, że jest już przypisana. 
4. Kliknij pozycję **Zapisz**.

Możesz uzyskać dostęp do danych na ADLS Gen2 za pomocą Synapse Spark za pomocą następującego adresu URL:
...
6. Kliknij przycisk **Test connection** , aby sprawdzić, czy ustawienia są poprawne.
7. Kliknij przycisk **Utwórz** najpierw, a następnie kliknij pozycję **Opublikuj wszystko** , aby zapisać zmiany. 
...
5. Kliknij przycisk **Test connection** , aby sprawdzić, czy ustawienia są poprawne.
6. Kliknij przycisk **Utwórz** najpierw, a następnie kliknij pozycję **Opublikuj wszystko** , aby zapisać zmiany. 
...
3. Kliknij pozycję **Dodaj zasady dostępu** : 
...
4. Kliknij przycisk **Wybierz** i **Dodaj**.
...
Zwraca właściwości pliku, łącznie z nazwą pliku, ścieżką pliku, rozmiarem pliku, bez względu na to, czy jest to katalog, i czy jest plikiem.
...
Tworzy określony katalog, jeśli nie istnieje, również tworząc wszystkie niezbędne katalogi nadrzędne.
...
Kopiuje plik lub katalog, obsługują kopie w systemach plików.
...
Przenosi plik lub katalog, obsługują przechodzenie między systemami plików.
...
Zwraca Azure Key Vault wpis tajny dla danej nazwy Azure Key Vault, nazwy wpisu tajnego i nazwy połączonej usługi przy użyciu tożsamości obszaru roboczego. Upewnij się, że dostęp do [Azure Key Vault](#configure-access-to-azure-key-vault) jest odpowiednio skonfigurowany.
