...
Odp.: usługa Azure Synapse to Zintegrowana platforma danych dla analizy biznesowej, AI i ciągłego. Łączy on różne środowiska uruchomieniowe analizy, takie jak SQL i Apache Spark za pomocą jednej platformy, która zapewnia jednolity sposób:
...
- Zarządzaj metadanymi w różnych aparatach. Utwórz bezserwerową tabelę puli Apache Spark i zostanie ona automatycznie udostępniona w bazach danych usługi Azure Synapse.
...
- Możliwości analizy są oferowane za poorednictwem dedykowanej puli SQL lub bezserwerowej puli SQL (wersja zapoznawcza).
- Bezserwerowa Pula Apache Spark (wersja zapoznawcza) z pełną obsługą Scala, Python, Spark, SQL i C #
...
Odp.: usługa Azure Synapse Analytics to ewolucja Azure SQL Data Warehouse na platformie analitycznej, która obejmuje dedykowaną pulę SQL jako rozwiązanie magazynu danych. Ta platforma łączy eksplorację, pozyskiwanie, przekształcenie, przygotowanie i obsługę warstwy analitycznej.
...
### <a name="q-what-is-a-good-use-case-for-dedicated-sql-pool"></a>P: co to jest dobry przypadek użycia dla dedykowanej puli SQL

Odp.: dedykowana Pula SQL jest sercem potrzeb hurtowni danych. Jest to wiodące rozwiązanie magazynu danych w [cenie/wydajności](https://azure.microsoft.com/services/sql-data-warehouse/compare/). Dedykowana Pula SQL to wiodące w branży rozwiązanie magazynu danych w chmurze, ponieważ można:
...
### <a name="q-what-is-a-good-use-case-for-serverless-apache-spark-pool-in-azure-synapse"></a>P: co to jest dobry przypadek użycia dla puli Apache Spark bezserwerowej na platformie Azure Synapse
...
Innym przypadkiem użycia dla Apache Spark jest analityk danych:

- Wyodrębnij funkcję
- Eksplorowanie danych
- uczenie modelu

### <a name="q-what-is-a-good-use-case-for-serverless-sql-pool-in-azure-synapse"></a>P: co to jest dobry przypadek użycia dla puli SQL bezserwerowej na platformie Azure Synapse

Odp.: bezserwerowa Pula SQL to usługa zapytania na dane w usłudze Data Lake. Dzięki temu można zdemokratyzuj proces dostęp do wszystkich danych, dostarczając znaną składnię T-SQL służącą do wykonywania zapytań dotyczących danych, bez konieczności kopiowania lub ładowania danych do wyspecjalizowanego magazynu.
...
### <a name="q-what-is-a-good-use-case-for-data-flow-in-azure-synapse"></a>P: co to jest dobry przypadek użycia dla przepływu danych w usłudze Azure Synapse
...
Odp.: kompleksowe środowisko logowania jednokrotnego to ważny proces uwierzytelniania w Synapse Analytics. Zarządzanie tożsamościami i przekazywanie ich za pośrednictwem pełnej integracji usługi Azure AD jest konieczne.
...
Odp.: tak, możesz użyć własnych aplikacji innych firm, takich jak Tableau i Power BI, aby nawiązać połączenie z dedykowaną pulą SQL i bezserwerową pulą SQL. Platforma Spark obsługuje IntelliJ.
