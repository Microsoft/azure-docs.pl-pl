...
DevTest Labs obsługuje użycie [zagnieżdżonych szablonów Menedżer zasobów](../azure-resource-manager/templates/linked-templates.md). W artykule [[Deploying nested Azure Resource Manager templates for test](deploy-nested-template-environments.md) Environments `_artifactsLocation` wyjaśniono, jak używać tokenów i `_artifactsLocationSasToken` do tworzenia identyfikatora URI dla szablonu Menedżer zasobów w tym samym folderze co lub w zagnieżdżonym folderze szablonu głównego. Aby uzyskać więcej informacji na temat tych dwóch tokenów, zobacz sekcję **artefakty wdrożenia** w [podręczniku Azure Resource Manager – Best Practices Guide](https://github.com/Azure/azure-quickstart-templates/blob/master/1-CONTRIBUTION-GUIDE/best-practices.md).
