...
1. Na komputerze lokalnym Użyj skryptu z [utrwalania poświadczeń udziału plików platformy Azure w systemie Windows w](../storage/files/storage-how-to-use-files-windows.md#persisting-azure-file-share-credentials-in-windows) sekcji [Korzystanie z udziału plików platformy Azure z systemem Windows](../storage/files/storage-how-to-use-files-windows.md) . 
...
3. [Zainstaluj Azure PowerShell](/powershell/azure/install-az-ps).
...
4. W ramach maszyny wirtualnej Uruchom **Eksploratora plików**, a następnie wybierz **ten komputer** , aby znaleźć udział plików.
    ...
    > [!NOTE]
    > Z jakiegoś powodu, jeśli nie możesz znaleźć udziału plików na maszynie wirtualnej lub na komputerze lokalnym, możesz go zainstalować ponownie, uruchamiając `net use` polecenie. `net use` Polecenie można znaleźć w kreatorze **połączenia** **udziału plików** w Azure Portal.
