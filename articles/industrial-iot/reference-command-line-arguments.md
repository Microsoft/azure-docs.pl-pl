---
title: Argumenty wiersza polecenia programu Microsoft OPC Publisher
description: Ten artykuł zawiera omówienie argumentów wiersza polecenia wydawcy OPC
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787596"
---
# <a name="command-line-arguments"></a>Argumenty wiersza polecenia

W poniższej tabeli opisano kilka argumentów wiersza polecenia, które mogą służyć do ustawiania ustawień globalnych dla wydawcy OPC.

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>Argumenty wiersza polecenia wydawcy OPC dla wersji 2,5 i poniżej

* Sposób użycia: opcpublisher.exe \<applicationname> [ \<iothubconnectionstring> ] [ \<options> ]
    
* ApplicationName: Nazwa aplikacji OPC UA do użycia, wymagana nazwa aplikacji jest również używana do rejestrowania wydawcy w ramach tej nazwy w rejestrze urządzeń IoT Hub.
    
* iothubconnectionstring: element ConnectionString IoT Hub właściciela, opcjonalny. Zazwyczaj parametr ConnectionString właściciela IoTHub jest określany tylko podczas pierwszego uruchomienia aplikacji. Parametry połączenia są szyfrowane i przechowywane w magazynie certyfikatów platform.
W przypadku kolejnych wywołań zostanie ono odczytane i ponownie użyte. Jeśli określisz parametry ConnectionString na każdym uruchomieniu, urządzenie, które jest tworzone dla aplikacji w rejestrze urządzenia IoT Hub, zostanie usunięte i ponownie utworzone.
    
Istnieje kilka zmiennych środowiskowych, których można użyć do sterowania aplikacją:
```
...
> [!NOTE] 
> Argumenty wiersza polecenia mają nadmiarowe ustawienia zmiennych środowiskowych.
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>Argumenty wiersza polecenia wydawcy OPC dla wersji 2,6 i nowszych
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
Więcej zasobów można znaleźć w repozytoriach usługi GitHub:
