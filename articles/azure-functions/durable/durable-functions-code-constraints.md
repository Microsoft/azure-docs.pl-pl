...
| Nieskończone pętle | Unikaj nieskończonych pętli w funkcjach programu Orchestrator. Ponieważ trwała struktura zadań zapisuje historię wykonywania w miarę postępów funkcji aranżacji, pętla nieskończona może spowodować, że w wystąpieniu programu Orchestrator brakuje pamięci. | W przypadku pętli nieskończonych należy użyć interfejsów API, takich jak `ContinueAsNew` w programie .NET lub `continueAsNew` w języku JavaScript, aby ponownie uruchomić wykonywanie funkcji i odrzucić poprzednią historię wykonywania. |
