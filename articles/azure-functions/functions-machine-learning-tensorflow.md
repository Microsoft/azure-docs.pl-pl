...
1. Zastąp całą zawartość `main` funkcji następującym kodem:
    ...
    Ta funkcja odbiera adres URL obrazu w parametrze ciągu zapytania o `img`nazwie. Następnie wywołuje `predict_image_from_url` z biblioteki pomocnika, aby pobrać i sklasyfikować obraz przy użyciu modelu TensorFlow. Funkcja zwraca odpowiedź HTTP z wynikami. 
