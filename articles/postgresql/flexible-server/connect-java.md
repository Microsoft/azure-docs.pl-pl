---
title: 'Szybki Start: używanie języka Java i JDBC z serwerem elastycznym Azure Database for PostgreSQL'
description: W tym przewodniku szybki start dowiesz się, jak używać języka Java i JDBC z serwerem elastycznym Azure Database for PostgreSQL.
author: mksuni
ms.author: sumuth
...
ms.locfileid: "98606127"
---
# <a name="quickstart-use-java-and-jdbc-with-azure-database-for-postgresql-flexible-server"></a>Szybki Start: używanie języka Java i JDBC z serwerem elastycznym Azure Database for PostgreSQL

W tym temacie przedstawiono Tworzenie przykładowej aplikacji, która używa języka Java i [JDBC](https://en.wikipedia.org/wiki/Java_Database_Connectivity) do przechowywania i pobierania informacji w [Azure Database for PostgreSQL elastycznym serwerze](./index.yml).
...
> [!NOTE]
> Więcej szczegółowych informacji na temat tworzenia serwerów PostgreSQL można znaleźć w temacie [Tworzenie serwera Azure Database for PostgreSQL przy użyciu Azure Portal](./quickstart-create-server-portal.md).
...
Utworzony wcześniej serwer PostgreSQL ma pustą bazę danych **Postgres** . Na potrzeby tego przewodnika Szybki Start utworzymy nową bazę danych o nazwie, `demo` wykonując następujące czynności:

1. Zainstaluj lokalne wystąpienie [PSQL](https://www.postgresql.org/docs/current/static/app-psql.html) , aby nawiązać połączenie z serwerem usługi Azure PostgreSQL. 
2. Uruchom następujące polecenie PSQL, aby nawiązać połączenie z bazą danych **Postgres** na serwerze PostgreSQL
...
3. Utwórz nową bazę danych o nazwie **Demonstracja** w wierszu polecenia, wpisując następujące polecenie:
