---
title: Punkt odniesienia zabezpieczeń platformy Azure dla Azure Firewall Manager
description: Punkt Azure Firewall Manager zabezpieczeń zawiera wskazówki proceduralne i zasoby dotyczące wdrażania zaleceń dotyczących zabezpieczeń określonych w te testach porównawczych zabezpieczeń platformy Azure.
author: msmbaldwin
...
ms.locfileid: "107864419"
---
# <a name="azure-security-baseline-for-azure-firewall-manager"></a>Punkt odniesienia zabezpieczeń platformy Azure dla Azure Firewall Manager

Ten punkt odniesienia zabezpieczeń stosuje wskazówki z testu porównawczego zabezpieczeń platformy [Azure w wersji 2.0](../security/benchmarks/overview.md) do Azure Firewall Manager. Test porównawczy zabezpieczeń platformy Azure zawiera zalecenia dotyczące sposobu zabezpieczania rozwiązań w chmurze na platformie Azure. Zawartość jest pogrupowana według **kontrolek zabezpieczeń zdefiniowanych** przez test porównawczy zabezpieczeń platformy Azure i powiązanych wskazówek dotyczących Azure Firewall Manager. **Kontrolki** nie dotyczy Azure Firewall Manager zostały wykluczone.

Aby zobaczyć, Azure Firewall Manager mapowanie na test porównawczy zabezpieczeń platformy Azure, zobacz pełny Azure Firewall Manager [mapowania punktów odniesienia zabezpieczeń](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Wskazówki:** Azure Firewall Manager usługi Azure Active Directory (Azure AD) jako domyślnej usługi zarządzania tożsamościami i dostępem. Należy ustandaryzować usługę Azure AD, aby zarządzać tożsamościami i dostępem w organizacji w ramach:

- Zasoby firmy Microsoft w chmurze, takie jak aplikacje Azure Portal, Azure Storage, Azure Virtual Machine (Linux i Windows), Azure Key Vault, PaaS i SaaS.
...
Usługa Azure AD obsługuje tożsamość zewnętrzną, która umożliwia użytkownikom bez konto Microsoft logowanie się do aplikacji i zasobów przy użyciu tożsamości zewnętrznej.
...
**Wskazówki:** Azure Firewall Manager używa Azure Active Directory, która obsługuje silne mechanizmy kontroli uwierzytelniania za pośrednictwem uwierzytelniania wieloskładnikowego (MFA) i silne metody bez hasła.
...
W przypadku administratorów i użytkowników uprzywilejowanych upewnij się, że jest używany najwyższy poziom silnej metody uwierzytelniania, a następnie wdowaj odpowiednie silne zasady uwierzytelniania dla innych użytkowników.
...
**Wskazówki:** Azure Firewall Manager jest zintegrowany z Azure Active Directory, w którym są następujące źródła danych:
...
Te źródła danych można zintegrować z Azure Monitor, Azure Sentinel lub systemami SIEM innych firm.

Obecnie akcje otaczające grupy kolekcji reguł zasad zapory nie są obsługiwane przez dziennik aktywności. Jest to znany problem i jest on obsługiwany w przyszłych aktualizacjach.
...
- [Azure Firewall Manager znane problemy](overview.md#known-issues)
...
**Wskazówki:** usługa Azure Firewall Manager obsługuje dostęp warunkowy usługi Azure Active Directory (Azure AD) na potrzeby bardziej szczegółowej kontroli dostępu na podstawie warunków zdefiniowanych przez użytkownika, takich jak dane logowania użytkowników z określonych zakresów adresów IP, które wymagają użycia usługi MFA do logowania. Szczegółowe zasady zarządzania sesjami uwierzytelniania mogą być również używane dla różnych przypadków użycia.
...
**Wskazówki:** Azure Firewall Manager tożsamości i dostępu Azure Active Directory (Azure AD). Najbardziej krytyczne role wbudowane to usługa Azure AD: administrator globalny i administrator ról uprzywilejowanych, ponieważ użytkownicy przypisani do tych dwóch ról mogą delegować role administratora:
- Administrator globalny: użytkownicy z tą rolą mają dostęp do wszystkich funkcji administracyjnych w usłudze Azure AD, a także usług, które korzystają z tożsamości usługi Azure AD.
- Administrator ról uprzywilejowanych: użytkownicy z tą rolą mogą zarządzać przypisaniami ról w usłudze Azure AD, a także w usłudze Azure AD Privileged Identity Management (PIM). Ponadto ta rola umożliwia zarządzanie wszystkimi aspektami pim i jednostkami administracyjnymi.

Jeśli używasz ról niestandardowych z przypisanymi pewnymi uprawnieniami uprzywilejowanymi, możesz mieć inne krytyczne role, które muszą być zarządzane. Możesz również zastosować podobne mechanizmy kontroli do konta administratora krytycznych zasobów biznesowych.
...
**Wskazówki:** Użyj niestandardowych ról RBAC platformy Azure, aby odizolować dostęp do grup kolekcji reguł Azure Firewall zasad. Użyj niestandardowej definicji roli platformy Azure, aby zapobiec przypadkowemu usunięciu zasad podstawowych i zapewnić selektywny dostęp do grup kolekcji reguł w ramach subskrypcji lub grupy zasobów.

Należy również ograniczyć dostęp do systemów zarządzania, tożsamości i zabezpieczeń, które mają dostęp administracyjny do systemów o krytycznym znaczeniu dla firmy. Osoby atakujące, które naruszyć te systemy zarządzania i zabezpieczeń może natychmiast irytować ich do naruszenia krytyczne dla działania firmy zasobów.
...
- [Używanie Azure Firewall zasad do definiowania hierarchii reguły](rule-hierarchy.md)
...
**Wskazówki:** Azure Firewall Manager zarządzać zasobami przy Azure Active Directory kont usługi Azure AD. Regularnie sprawdzaj konta użytkowników i przypisywanie dostępu, aby upewnić się, że konta i ich dostęp są prawidłowe. Przeglądów dostępu usługi Azure AD można użyć do przeglądania członkostwa w grupach, dostępu do aplikacji dla przedsiębiorstw i przypisań ról. Raporty usługi Azure AD mogą dostarczać dzienniki, aby ułatwić odnajdywanie nieaktualnych kont. Możesz również użyć Azure AD Privileged Identity Management, aby utworzyć przepływ pracy raportu przeglądu dostępu w celu ułatwienia procesu przeglądu.

Ponadto usługę Azure Privileged Identity Management można również skonfigurować do alertów w przypadku utworzenia nadmiernej liczby kont administratorów oraz identyfikowania kont administratorów, które są nieaktualne lub nieprawidłowo skonfigurowane.

Niektóre usługi platformy Azure obsługują użytkowników i role lokalne, które nie są zarządzane za pośrednictwem usługi Azure AD. Musisz zarządzać tymi użytkownikami oddzielnie.

- [Tworzenie przeglądu dostępu ról zasobów platformy Azure w usłudze Privileged Identity Management(PIM)](../active-directory/privileged-identity-management/pim-resource-roles-start-access-review.md) 
...
**Wskazówki:** Azure Firewall Manager są zintegrowane z Azure Active Directory do zarządzania jej zasobami. Funkcje zarządzania uprawnieniami usługi Azure AD pozwalają zautomatyzować przepływy pracy żądań dostępu, w tym przypisania dostępu, przeglądy i wygaśnięcie. Obsługiwane jest również zatwierdzanie dwuetapowe lub wieloetapowe.

- [Co to są przeglądy dostępu w usłudze Azure AD](../active-directory/governance/access-reviews-overview.md)

- [Co to jest zarządzanie uprawnieniami w usłudze Azure AD](../active-directory/governance/entitlement-management-overview.md)
...
**Wskazówka**: Zabezpieczone, izolowane stacje robocze mają kluczowe znaczenie dla bezpieczeństwa poufnych ról, takich jak administratorzy, deweloperzy i operatorzy usług o kluczowym znaczeniu. Stacje robocze użytkowników o wysokim stopniu bezpieczeństwa mogą wykonywać zadania administracyjne związane z zarządzaniem Azure Firewall Manager w środowiskach produkcyjnych. Użyj usługi Azure Active Directory, zaawansowanej ochrony przed zagrożeniami w usłudze Microsoft Defender i/lub usługi Microsoft Intune do wdrożenia bezpiecznej i zarządzanej stacji roboczej użytkownika na potrzeby zadań administracyjnych. Zabezpieczone stacje robocze mogą być centralnie zarządzane w celu wymuszania zabezpieczonej konfiguracji, w tym silnego uwierzytelniania, punktów odniesienia oprogramowania i sprzętu oraz ograniczonego dostępu logicznego i sieciowego.

- [Opis stacji roboczych z dostępem uprzywilejowanym](https://4sysops.com/archives/understand-the-microsoft-privileged-access-workstation-paw-security-model/)
...
**Wskazówki:** Ochrona poufnych danych, takich jak dane konfiguracji usługi Azure Firewall Policy, przez ograniczenie dostępu przy użyciu usługi Azure Role Based Access Control (Azure RBAC), kontroli dostępu opartej na sieci i określonych kontroli w usługach platformy Azure.
...
**Wskazówki:** Azure Firewall do zasobów usługi Policy mogą uzyskiwać dostęp tylko uwierzytelnieni użytkownicy. Klienci muszą upewnić się, że tylko autoryzowani użytkownicy mają dostęp do danych.
...
**Wskazówki:** Aby uzupełnić kontrolę dostępu, dane podczas przesyłania powinny być chronione przed atakami "poza pasmem" (na przykład przechwytywanie ruchu) przy użyciu szyfrowania, aby zapewnić, że osoby atakujące nie będą w stanie łatwo odczytać ani zmodyfikować danych.

Azure Firewall Manager obsługuje szyfrowanie danych podczas przesyłania z TLS w wersji 1.2 lub większej.

Chociaż jest to opcjonalne w przypadku ruchu w sieciach prywatnych, ma to kluczowe znaczenie dla ruchu w sieciach zewnętrznych i publicznych. W przypadku ruchu HTTP upewnij się, że każdy klient łączący się z zasobami platformy Azure może negocjować protokół TLS w wersji 1.2 lub większej. W przypadku zarządzania zdalnego użyj protokołu SSH (dla systemu Linux) lub protokołu RDP/TLS (dla systemu Windows) zamiast niezaszyfrowanego protokołu. Przestarzałe wersje i protokoły SSL, TLS i SSH oraz słabe szyfry powinny być wyłączone.

Domyślnie platforma Azure zapewnia szyfrowanie danych podczas przesyłania między centrami danych platformy Azure.
...
- [Podwójne szyfrowanie danych platformy Azure podczas przesyłania](../security/fundamentals/double-encryption.md#data-in-transit)
...
Aby uzyskać wgląd w obciążenia i usługi, mogą być wymagane dodatkowe uprawnienia. 
...
**Wskazówki:** Upewnij się, że zespoły ds. zabezpieczeń mają dostęp do stale aktualizowanego spisu zasobów Azure Firewall Manager na platformie Azure. Mogą oni używać Azure Resource Graph do wykonywania zapytań i odnajdywania wszystkich zasobów Azure Firewall w subskrypcjach, w tym usług, aplikacji i zasobów sieciowych platformy Azure.

Zastosuj tagi do zasobów, grup zasobów i subskrypcji platformy Azure, aby logicznie zorganizować je w taksonomię. Każdy tag składa się z nazwy i pary wartości. Na przykład można zastosować nazwę „Środowisko” i wartość „Produkcyjne” do wszystkich zasobów w środowisku produkcyjnym.

- [Jak tworzyć zapytania za pomocą eksploratora usługi Azure Resource Graph](../governance/resource-graph/first-query-portal.md) 

- [Azure Security Center zarządzania spisem zasobów](../security-center/asset-inventory.md) 

- [Aby uzyskać więcej informacji na temat tagowania zasobów, zobacz przewodnik po decyzjach dotyczących nazewnictwa i tagowania zasobów](/azure/cloud-adoption-framework/decision-guides/resource-tagging/?toc=%2fazure%2fazure-resource-manager%2fmanagement%2ftoc.json)
...
**Wskazówki:** użyj Azure Policy do inspekcji i ograniczenia usług, które użytkownicy mogą aprowizować w Twoim środowisku. Obejmuje to możliwość zezwalania na wdrażanie lub blokowania Azure Firewall zasobów. Usługa Azure Resource Graph umożliwia wykonywanie zapytań dotyczących zasobów i odnajdywanie ich w ramach subskrypcji. Za pomocą usługi Azure Monitor można tworzyć reguły wyzwalające alerty w przypadku wykrycia niezatwierdzonej usługi.
...
**Wskazówki:** Usuń Azure Firewall Manager, gdy nie są już potrzebne do zminimalizowania możliwości ataku. Użytkownicy mogą zarządzać swoimi zasobami Azure Firewall Manager za pośrednictwem interfejsu Azure Portal, interfejsu wiersza polecenia lub interfejsów API REST.

- [Azure Firewall wiersza polecenia zasad interfejsu wiersza polecenia](/cli/azure/network/firewall/policy)
...
### <a name="am-5-limit-users-ability-to-interact-with-azure-resource-manager"></a>AM-5: Ogranicz możliwość interakcji użytkowników z Azure Resource Manager

**Wskazówki:** Azure Firewall Manager zintegrowane z usługą Azure Active Directory (Azure AD) na temat tożsamości i uwierzytelniania. Za pomocą dostępu warunkowego platformy Azure można ograniczyć użytkownikom możliwość interakcji z usługą Azure Resources Manager, konfigurując opcję "Blokuj dostęp" dla aplikacji "Microsoft Azure Management".
...
**Wskazówki:** Przekazywanie dzienników aktywności wytwarzanych przez zasady zapory lub związanych z nimi do rozwiązania SIEM, które mogą służyć do skonfigurowania niestandardowych wykrywania zagrożeń. Upewnij się, że monitorujesz różne typy zasobów platformy Azure pod adresem potencjalnych zagrożeń i anomalii. Skoncentruj się na uzyskiwaniu alertów wysokiej jakości w celu zmniejszenia liczby wyników fałszywie dodatnich, które będą sortować analitycy. Alerty mogą pochodzić z danych dziennika, agentów lub innych danych.

- [Tworzenie niestandardowych reguł analizy w celu wykrywania zagrożeń](../sentinel/tutorial-detect-threats-custom.md) 

- [Cyberata threat intelligence with Azure Sentinel (Sztuczna inteligencja przed cyberzagrożeniami przy użyciu Azure Sentinel](/azure/architecture/example-scenario/data/sentinel-threat-intelligence)
...
**Wskazówki:** dzienniki aktywności, które są automatycznie dostępne, zawierają wszystkie operacje zapisu (PUT, POST, DELETE) dla zasobów zasad zapory z wyjątkiem operacji odczytu (GET). Dzienniki aktywności mogą służyć do znalezienia błędu podczas rozwiązywania problemów lub monitorowania sposobu, w jaki użytkownik w organizacji zmodyfikował zasób.
...
**Wskazówki:** Scentralizowanie magazynu rejestrowania i analizy w celu włączenia korelacji. Dla każdego źródła dziennika upewnij się, że przypisano właściciela danych, wskazówki dotyczące dostępu, lokalizację magazynu, narzędzia używane do przetwarzania danych i uzyskiwania do nich dostępu oraz wymagania dotyczące przechowywania danych.

Upewnij się, że integrujesz dzienniki aktywności platformy Azure z centralnym rejestrowaniem. Pozyskaj dzienniki za pośrednictwem Azure Monitor, aby agregować dane zabezpieczeń generowane przez urządzenia punktu końcowego, zasoby sieciowe i inne systemy zabezpieczeń. W Azure Monitor obszarów roboczych usługi Log Analytics można wykonywać zapytania i przeprowadzać analizy, a także używać kont usługi Azure Storage do przechowywania długoterminowego i archiwalnej.

Ponadto włącz i dołączyć dane dziennika do Azure Sentinel rozwiązania SIEM lub rozwiązania SIEM innej firmy.

Wiele organizacji decyduje się na używanie Azure Sentinel danych "gorących", które są często używane, i usługi Azure Storage dla "zimnych" danych, które są rzadziej używane.
...
**Wskazówki:** Upewnij się, że wszystkie konta magazynu lub obszary robocze usługi Log Analytics używane do przechowywania dzienników zasad zapory mają ustawiony okres przechowywania dzienników zgodnie z przepisami dotyczącymi zgodności w organizacji.

W Azure Monitor można ustawić okres przechowywania obszaru roboczego usługi Log Analytics zgodnie z przepisami dotyczącymi zgodności w organizacji. Używaj kont obszarów roboczych usług Azure Storage, Data Lake lub Log Analytics do przechowywania długoterminowego i archiwalnej.
...
Usługa Azure Security Center przypisuje poziom ważności do każdego alertu, aby pomóc w ustaleniu, które alerty powinny być zbadane w pierwszej kolejności. Ważność jest oparta na tym, jak pewna Security Center znajduje się w znajdowaniu lub analizie użytej do wystawienia alertu, a także na poziomie pewności, że za działaniem, które doprowadziło do tego alertu, znajduje się złośliwy zamiar.
...
**Wskazówki:** Automatyzowanie i zabezpieczanie wdrażania i konfigurowania zasobów usługi Azure Firewall Manager w środowiskach przy użyciu takich mechanizmów jak: szablony Azure Resource Manager, kontrolki RBAC platformy Azure i Azure Policy. Zdefiniuj bezpieczne konfiguracje dla zasobów Azure Firewall Manager podczas wdrażania, przejmij inspekcję i wymusz te konfiguracje, definiując niestandardowe definicje Azure Policy przy użyciu aliasów w przestrzeni nazw "Azure.Network".

- [Azure Firewall szablonów zasad](/azure/templates/microsoft.network/firewallpolicies)

- [interfejs wiersza polecenia Azure Firewall Policy](/cli/azure/network/firewall/policy)

- [Ilustracja implementacji zabezpieczeń w strefie docelowej o skali przedsiębiorstwa](/azure/cloud-adoption-framework/ready/enterprise-scale/architecture#landing-zone-expanded-definition)
...
**Wskazówki:** Azure Firewall Manager obsługuje Azure Resource Manager szablonów i wymuszanie ustawień konfiguracji za pośrednictwem Azure Policy. Definiowanie niestandardowych Azure Policy w celu inspekcji i wymuszania Azure Firewall Manager konfiguracji zasobów przy użyciu aliasów w przestrzeni nazw "Azure.Network".
...
### <a name="pv-3-establish-secure-configurations-for-compute-resources"></a>PV-3: Ustanów bezpieczne konfiguracje dla zasobów obliczeniowych

**Wskazówki:** Nie dotyczy; Azure Firewall Manager jest usługą zarządzania płaszczyzną sterowania zaporą i nie uwidacznia infrastruktury obliczeniowej usługi bazowej do skonfigurowania przez klientów.
...
### <a name="br-1-ensure-regular-automated-backups"></a>BR-1: Zapewnianie regularnych automatycznych kopii zapasowych

**Wskazówki:** Azure Firewall Manager nie ma koncepcji kopii zapasowych systemu skierowanych do klientów, podstawowa infrastruktura jest obsługiwany przez firmę Microsoft.

W przypadku kopii zapasowych konfiguracji zasobów użyj programu Azure Resource Manager, aby wyeksportować zasady zapory i powiązane zasoby w szablonie JavaScript Object Notation (JSON), który może służyć jako kopia zapasowa konfiguracji. Konfiguracje zasad zapory można również eksportować przy użyciu funkcji eksportowania szablonu Azure Firewall z Azure Portal. Użyj Azure Automation, aby uruchamiać niestandardowe skrypty kopii zapasowej, aby automatycznie przechwytywać konfiguracje zasobów Azure Firewall Manager zasobów.

- [Wdrażanie bezpiecznego koncentratora wirtualnego przy użyciu szablonu](quick-secure-virtual-hub.md)

- [Informacje o szablonie zasad zapory firmy Microsoft](/azure/templates/microsoft.network/firewallpolicies)
...
**Wskazówki:** Azure Firewall Manager nie ma koncepcji tworzenia kopii zapasowych systemu skierowanych do klientów. W przypadku wszystkich wyeksportowanych Azure Firewall Manager zasobów okresowo przeprowadzaj przywracanie przy użyciu tych Azure Resource Manager szablonów.

- [Wdrażanie bezpiecznego koncentratora wirtualnego przy użyciu Azure Resource Manager szablonów](quick-secure-virtual-hub.md)
