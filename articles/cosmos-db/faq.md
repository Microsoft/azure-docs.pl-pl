...
Tak, Azure Cosmos DB jest zgodny z HIPAA. Ustawa HIPAA określa wymogi dotyczące wykorzystywania, ujawniania i ochrony informacji o zdrowiu umożliwiających indywidualną identyfikację osoby. Aby uzyskać więcej informacji, zobacz [Centrum zaufania Microsoft](/compliance/regulatory/offering-hipaa-hitech).
...
Interfejs API SQL obsługuje agregację o małym opóźnieniu w dowolnej skali za pośrednictwem funkcji agregujących `COUNT` , `MIN` ,, `MAX` `AVG` i `SUM` za pomocą gramatyki SQL. Aby uzyskać więcej informacji, zobacz [funkcje agregujące](sql-query-aggregate-functions.md).
...
Aby użyć optymistycznej współbieżności w programie .NET, należy użyć klasy [AccessCondition](/dotnet/api/microsoft.azure.documents.client.accesscondition) . Aby zapoznać się z przykładem platformy .NET, zobacz temat [program. cs](https://github.com/Azure/azure-cosmos-dotnet-v2/blob/master/samples/code-samples/DocumentManagement/Program.cs) w przykładowej DocumentManagement w witrynie GitHub.
