...
Przedział czasu dostępnego dla przywracania (nazywany także okresem przechowywania) jest niższą wartością następujących dwóch: "30 dni wstecz od teraz" lub "do czasu utworzenia zasobu". Punkt w czasie dla przywracania może być dowolną sygnaturą czasową w okresie przechowywania.
...
## <a name="what-is-restored"></a>Co zostało przywrócone?
...
Można wybrać opcję przywrócenia dowolnej kombinacji zainicjowanych kontenerów przepływności, udostępnionej bazy danych przepływności lub całego konta. Akcja Przywróć przywraca wszystkie dane i właściwości indeksu do nowego konta. Proces przywracania gwarantuje, że wszystkie dane przywrócone w ramach konta, bazy danych lub kontenera mają być spójne z określonym czasem przywracania. Czas trwania przywracania będzie zależeć od ilości danych, które muszą zostać przywrócone.
...
Poniżej przedstawiono niektóre z kluczowych scenariuszy, które są rozwiązywane przez funkcję Przywracanie do określonego momentu. Scenariusze [a] do [c] pokazują, jak wyzwolić przywracanie, jeśli sygnatura czasowa przywracania jest znana wcześniej. Mogą jednak wystąpić sytuacje, w których nie wiadomo, czy nie jest dokładny czas przypadkowego usunięcia lub uszkodzenia. Scenariusze [d] i [e] przedstawiają sposób _odnajdywania_ sygnatury czasowej przywracania przy użyciu nowych interfejsów API strumieniowego źródła zdarzeń w bazie danych lub kontenerach dostępnych.
...
Konta Azure Cosmos DB, dla których włączono ciągłą kopię zapasową, będą naliczane dodatkowe miesięczne opłaty za "przechowywanie kopii zapasowych" oraz "Przywracanie danych". Koszt przywrócenia jest dodawany przy każdym zainicjowaniu operacji przywracania. Jeśli skonfigurujesz konto z ciągłą kopią zapasową, ale nie przywracasz danych, na rachunku zostanie uwzględniony tylko koszt magazynu kopii zapasowej.
...
* W przypadku ciągłej kopii zapasowej są obsługiwane tylko Azure Cosmos DB interfejsów API dla SQL i MongoDB. Interfejsy API Cassandra, Table i Gremlin nie są jeszcze obsługiwane.
...
* Przywrócone konto zostanie utworzone w tym samym regionie, w którym istnieje konto źródłowe. Nie można przywrócić konta do regionu, w którym konto źródłowe nie istnieje.

* Okno przywracania ma tylko 30 dni i nie można go zmienić.

* Kopie zapasowe nie są automatycznie odporne na awarie geograficzne. Musisz jawnie dodać inny region w celu uzyskania odporności dla konta i kopii zapasowej.
...
* Azure Cosmos DB API dla kont SQL lub MongoDB, które tworzą unikatowy indeks po utworzeniu kontenera, nie są obsługiwane w przypadku ciągłej kopii zapasowej. Obsługiwane są tylko kontenery, które tworzą unikatowy indeks jako część początkowego tworzenia kontenera. W przypadku kont MongoDB tworzysz unikatowy indeks przy użyciu [poleceń rozszerzenia](mongodb-custom-commands.md).

* Funkcja przywracania do punktu w czasie zawsze przywraca nowe konto usługi Azure Cosmos. Przywracanie do istniejącego konta nie jest obecnie obsługiwane. Jeśli interesuje Cię przesyłanie opinii na temat przywracania w miejscu, skontaktuj się z zespołem Azure Cosmos DB za pośrednictwem przedstawiciela konta lub usługi [UserVoice](https://feedback.azure.com/forums/263030-azure-cosmos-db).
...
* Proces przywracania przywraca wszystkie właściwości kontenera, w tym jego konfigurację TTL. W związku z tym możliwe jest, że przywrócone dane zostaną usunięte natychmiast po skonfigurowaniu tej metody. Aby zapobiec takiej sytuacji, sygnatura czasowa przywracania musi być przed dodaniem właściwości czasu wygaśnięcia do kontenera.
