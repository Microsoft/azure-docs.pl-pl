...
W tym przypadku `state`właściwości, `country`, i `city` są zagnieżdżone we `address` właściwości.
...
W poprzednim przykładzie `SELECT` klauzula musi utworzyć obiekt JSON, a ponieważ przykład nie zawiera klucza, klauzula używa nazwy `$1`zmiennej argumentu niejawnego. Następujące zapytanie zwraca dwie niejawne zmienne argumentów `$1` : `$2`i.
