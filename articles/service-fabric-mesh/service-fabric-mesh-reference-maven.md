...
- Generuje `secret` YAML o podanej nazwie klucza tajnego `appresources` w folderze 

### `azure-sfmesh:addsecretvalue`
- Generuje `secretvalue` YAML o podanej nazwie tajnej i wartości tajnej `appresources` w folderze 
...
- Tworzy YAML `servicefabric->helloworldservice` usługi w folderze o nazwie `service_helloworldservice` odwołującej `helloworldservicenetwork` się `helloworldserver` do aplikacji &
...
- Odwołania `helloworldservicelistener` jako odbiornik usługi, który nasłuchuje wywołań z tej bramy. Odwołuje się `helloworldservice` również do usługi `helloworldservicenetwork` jako sieci i `helloworldserver` aplikacji. 
...
- Tworzy wpis tajny YAML w `servicefabric->appresources` folderze o nazwie`secret_secret1`
...
Aby uzyskać pomoc dotyczącą `azure-sfmesh:deploytocluster`celu, możesz uruchomić aplikację lokalnie przy użyciu poniższego polecenia:
...
Aby uzyskać pomoc dotyczącą `azure-sfmesh:deploy`celu, możesz wdrożyć program do Service Fabric środowiska siatki, uruchamiając poniższe polecenie:
