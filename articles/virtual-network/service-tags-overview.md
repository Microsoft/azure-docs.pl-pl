...
| Tag | Przeznaczenie | Czy można używać ruchu przychodzącego lub wychodzącego? | Czy może to być region? | Czy można używać z Azure Firewall? |
| --- | -------- |:---:|:---:|:---:|
| **ActionGroup** | Grupa akcji. | Przychodzący | Nie | Nie |
| **ApiManagement** | Ruch zarządzania w przypadku wdrożeń API Management platformy Azure dedykowanych. <br/><br/>*Uwaga:* Ten tag reprezentuje punkt końcowy usługi Azure API Management dla płaszczyzny sterowania na region. Dzięki temu klienci mogą wykonywać operacje zarządzania na interfejsach API, operacjach, zasadach i nazwanych wartościach skonfigurowanych w API Management usługi.  | Przychodzący | Tak | Tak |
...
| **AzureArcInfrastructure** | Azure Arc włączone serwery, Azure Arc ruchu kubernetes i konfiguracji gościa.<br/><br/>*Uwaga:* Ten tag jest zależny od tagów **AzureActiveDirectory,****AzureTrafficManager** i **AzureResourceManager.** *Tego tagu nie można obecnie konfigurować za pośrednictwem witryny Azure Portal.*| Wychodzący | Nie | Tak |
...
| **AzureCognitiveSearch** | Azure Cognitive Search. <br/><br/>Ten tag lub adresy IP objęte tym tagiem mogą służyć do udzielania indeksatorom bezpiecznego dostępu do źródeł danych. Aby uzyskać więcej informacji, [zapoznaj](../search/search-indexer-troubleshooting.md#connection-errors) się z dokumentacją połączenia indeksatora. <br/><br/> *Uwaga:* adres IP usługi wyszukiwania nie znajduje się na liście zakresów  adresów IP dla tego tagu usługi i musi zostać dodany do zapory ADRESÓW IP źródeł danych. | Przychodzący | Nie | Nie |
| **AzureConnectors (Połączenia platformy Azure)** | Ten tag reprezentuje adresy IP używane przez zarządzane łączniki, które wywołują wywołania zwrotne przychodzącego modułu webhook do usługi Azure Logic Apps i wywołania wychodzące do odpowiednich usług, na przykład Azure Storage lub Azure Event Hubs. | Ruch przychodzący/wychodzący | Tak | Tak |
...
| **AzureDevOps** | Azure Dev Ops.<br/><br/>*Uwaga: obecnie nie można konfigurować tego tagu za pośrednictwem witryny Azure Portal*| Przychodzący | Nie | Tak |
...
| **AzurePlatformDNS** | Podstawowa infrastruktura (domyślna) usługa DNS.<br/><br>Ten tag umożliwia wyłączenie domyślnego systemu DNS. Należy zachować ostrożność podczas korzystania z tego tagu. Zalecamy zapoznanie się z [tematem Zagadnienia dotyczące platformy Azure.](./network-security-groups-overview.md#azure-platform-considerations) Zalecamy również przeprowadzenie testowania przed użyciem tego tagu. | Wychodzący | Nie | Nie |
| **AzurePlatformIMDS** | Azure Instance Metadata Service (IMDS), czyli podstawowa usługa infrastruktury.<br/><br/>Możesz użyć tego tagu, aby wyłączyć domyślne ustawienie IMDS. Należy zachować ostrożność podczas korzystania z tego tagu. Zalecamy zapoznanie się z [tematem Zagadnienia dotyczące platformy Azure.](./network-security-groups-overview.md#azure-platform-considerations) Zalecamy również przeprowadzenie testowania przed użyciem tego tagu. | Wychodzący | Nie | Nie |
...
| **SqlManagement** | Ruch zarządzania w przypadku wdrożeń dedykowanych sql. | Oba | Nie | Tak |
...
- [Azure ( Chiny)](https://www.microsoft.com/download/details.aspx?id=57062) 
