---
title: Typy Service Fabric zespole schematu XML modelu usługi Azure Service Fabric Service
description: Opisuje typy złożone w schemacie XML modelu Service Fabric usługi.
ms.topic: reference
ms.date: 12/10/2018
...
|Atrybut|Wartość|
|---|---|
|zawartość|1 elementy, 1 atrybuty|
...
Lista parametrów aplikacji zgodnie z definicją w manifeście aplikacji i ich odpowiednimi wartościami.
...
Opisuje zasady oceny zdarzeń kondycji zgłaszanych dla różnych jednostek powiązanych z aplikacją. Jeśli nie określono żadnych zasad, zakłada się, że jednostka jest w złej kondycji, jeśli raport kondycji jest ostrzeżeniem lub błędem.

|Atrybut|Wartość|
|---|---|
|zawartość|2 elementy, 2 atrybuty|
|zdefiniowane|Globalnie|
...
Określa, czy raporty o kondycji ostrzeżeń mają być traktować jako błędy podczas oceny kondycji. Wartość domyślna: false.
...
Określa maksymalną tolerowaną wartość procentową wdrożonych aplikacji, które mogą być w złej kondycji, zanim aplikacja zostanie uznana za błędną. Jest to obliczane przez podzielenie liczby wdrożonych aplikacji w złej kondycji przez liczbę węzłów, w których aplikacje są obecnie wdrażane w klastrze. Obliczenia są zaokrąglane w górę, aby tolerować jeden błąd na małej liczbie węzłów. Domyślna wartość procentowa: 0.
...
Opisuje zasady oceny zdarzeń kondycji zgłaszanych w usługach, partycjach i replikach określonego typu usługi.
...
Opisuje wystąpienie aplikacji Microsoft Azure Service Fabric aplikacji.

|Atrybut|Wartość|
|---|---|
|zawartość|4 elementy, 1 atrybuty|
|zdefiniowane|Globalnie|
...
Deklaratywnie opisuje typ i wersję aplikacji. Co najmniej jeden manifest usługi składników usług jest przywołyny w celu redagowania typu aplikacji. Ustawienia konfiguracji usług składowych można przesłonić przy użyciu sparametryzowanego ustawień aplikacji. Usługi domyślne, szablony usług, podmioty zabezpieczeń, zasady, konfiguracja diagnostyki i certyfikaty mogą być również deklarowane na poziomie aplikacji.

|Atrybut|Wartość|
|---|---|
|zawartość|9 elementów, 0 atrybutów|
|zdefiniowane|Globalnie|
...
#### <a name="parameters"></a>Parametry
Deklaruje parametry, które są używane w tym manifeście aplikacji. Wartość tych parametrów może być dostarczana podczas wystąpienia aplikacji i może służyć do zastępowania ustawień konfiguracji aplikacji lub usługi.
...
#### <a name="servicetemplates"></a>ServiceTemplates
Deklaruje zestaw dozwolonych typów usług, które mogą być tworzone dynamicznie wewnątrz wystąpienia aplikacji. Domyślne wartości konfiguracji, takie jak współczynnik replikacji, są określane i używane jako szablon do tworzenia wystąpień usługi.
...
|---|---|
|name|Diagnostyka|
|typ|[Typ diagnostyki](#diagnosticstype-complextype)|
|Minoccurs|0|
...
Pakiet ApplicationPackage reprezentuje informacje o wersji aplikacji wymagane przez węzeł.
...
|typ|[Typ środowiska](#environmenttype-complextype)|
...
Opisuje zasady (zbieranie dzienników, domyślne ustawienie Uruchom jako, kondycję i dostęp zabezpieczeń) do zastosowania na poziomie aplikacji.

|Atrybut|Wartość|
|---|---|
|zawartość|4 elementy, 0 atrybutów|
|zdefiniowane|Globalnie|
...
Określ domyślne konto użytkownika dla wszystkich pakietów kodu usługi, które nie mają określonego ustawienia RunAsPolicy zdefiniowanego w sekcji ServiceManifestImport.
...
|Atrybut|Wartość|
|---|---|
|zawartość|0 elementów, 0 atrybutów|
|zdefiniowane|Globalnie|
|name|AzureBlobType|
...
#### <a name="clustercertificate"></a>ClusterCertificate (Certyfikat klastra)
Certyfikat używany do zabezpieczania komunikacji wewnątrz klastra.

|Atrybut|Wartość|
|---|---|
|name|ClusterCertificate (Certyfikat klastra)|
...
Opisuje klaster Microsoft Azure Service Fabric klastra.

|Atrybut|Wartość|
|---|---|
|zawartość|4 elementy, 3 atrybuty|
|zdefiniowane|Globalnie|
...
#### <a name="fabricsettings"></a>Sieć szkieletowaUstawienia
...
Opisuje pakiet kodu, który obsługuje zdefiniowany typ usługi. Gdy usługa jest aktywna względem jednego z tych typów usług, wszystkie pakiety kodu zadeklarowane w tym manifeście są aktywowane przez uruchomienie ich punktów wejścia. Wynikowe procesy powinny rejestrować obsługiwane typy usług w czasie rzeczywistym. Jeśli istnieje wiele pakietów kodu, wszystkie są aktywowane za każdym razem, gdy system szuka jednego z zadeklarowanych typów usług.

|Atrybut|Wartość|
|---|---|
|zawartość|3 elementy, 2 atrybuty|
|zdefiniowane|Globalnie|
...
Wskazuje, czy zawartość tego pakietu kodu jest współdzielona przez inne pakiety kodu. W przypadku wartości true podczas uaktualniania tego pakietu kodu wszystkie pakiety kodu zostaną uruchomione ponownie. Ten atrybut nie jest obecnie obsługiwany i jego wartość zostanie zignorowana.
...
Uprzywilejowany punkt wejścia, który domyślnie jest uruchamiany przy użyciu tych samych poświadczeń co Service Fabric (zazwyczaj konto NETWORKSERVICE) przed każdym innym punktem wejścia. Plik wykonywalny określony przez entrypoint jest zwykle długotrwałym hostem usługi. Obecność oddzielnego punktu wejścia instalacji pozwala uniknąć konieczności uruchamiania hosta usługi z wysokimi uprawnieniami przez dłuższy czas.
...
Opisuje przesłonięcia konfiguracji dla określonego pakietu konfiguracji w manifeście zaimportowanej usługi.
...
#### <a name="name"></a>Nazwa
Nazwa pakietu konfiguracji w manifeście usługi, który zawiera ustawienia, które mają zostać zastąpione.
...
Deklaruje folder o nazwie według atrybutu Name, który zawiera Settings.xml pliku. Ten plik zawiera sekcje ustawień pary klucz-wartość zdefiniowanych przez użytkownika, które proces może odczytać w czasie uruchomienia. Jeśli podczas uaktualniania zmieniła się tylko wersja pakietu ConfigPackage, uruchomiony proces nie zostanie uruchomiony ponownie. Zamiast tego wywołanie zwrotne powiadamia proces, że ustawienia konfiguracji zostały zmienione, aby można je było ładować ponownie dynamicznie.
...
|Atrybut|Wartość|
|---|---|
|zawartość|1 elementy, 1 atrybut|
|zdefiniowane|Globalnie|
...
#### <a name="configpackage"></a>Pakiet ConfigPackage

|Atrybut|Wartość|
|---|---|
|name|Pakiet ConfigPackage|
...
Deklaruje folder o nazwie według atrybutu Name w obszarze PackageRoot, który zawiera Settings.xml plik. Ten plik zawiera sekcje ustawień pary klucz-wartość zdefiniowanych przez użytkownika, które proces może odczytać w czasie uruchomienia. Jeśli podczas uaktualniania zmieniła się tylko wersja pakietu ConfigPackage, uruchomiony proces nie zostanie uruchomiony ponownie. Zamiast tego wywołanie zwrotne powiadamia proces, że ustawienia konfiguracji zostały zmienione, aby można je było ładować ponownie dynamicznie.
...
Określa informacje o certyfikacie X509, który ma zostać ujawniony w środowisku kontenera. Certyfikat musi być zainstalowany w magazynie LocalMachine wszystkich węzłów klastra.
...
Plik PFX i hasło są dostępne w kontenerze przy użyciu Certificates_ServicePackageName_CodePackageName_CertName_PFX i Certificates_ServicePackageName_CodePackageName_CertName_Password zmiennych środowiskowych. Plik PEM jest dostępny w kontenerze przy użyciu Certificates_ServicePackageName_CodePackageName_CertName_PEM i Certificates_ServicePackageName_CodePackageName_CertName_PrivateKey zmienne środowiskowe.

|Atrybut|Wartość|
|---|---|
|zawartość|0, atrybuty 8|
|zdefiniowane|Globalnie|
...
Nazwa magazynu dla certyfikatu X509.
...
Nazwa pakietu danych, który zawiera pliki certyfikatów.
...
Wersja pakietu danych, który zawiera pliki certyfikatów.
...
#### <a name="name"></a>Nazwa
Identyfikator informacji o określonym certyfikacie. Ta nazwa służy do ustawienia zmiennej środowiskowej w kontenerze.
...
|Atrybut|Wartość|
|---|---|
|zawartość|0 elementów, 2 atrybuty|
|zdefiniowane|Globalnie|
...
Jeśli kontener ma włączoną funkcję HEALTHCHECK i zgłasza health_status docker dla tego kontenera, Service Fabric będzie to częścią kondycji zgłaszanej przez system. Gdy health_status jest w złej kondycji, Service Fabric będzie zgłaszać ostrzeżenie o kondycji. Domyślnie jest ustawiona wartość true.
...
Jeśli w kontenerze włączono funkcję HEALTHCHECK, health_status docker zgłasza zdarzenie dla tego kontenera, program Service Fabric ponownie uruchomi kontener, gdy health_status zgłaszana przez platformę Docker jest w złej kondycji.
...
Repo and image on or Azure Container Registry (Repo and image on https://hub.docker.com lub Azure Container Registry).
...
|Atrybut|Wartość|
|---|---|
|zawartość|10 elementów, 8 atrybutów|
|zdefiniowane|Globalnie|
...
Tryb izolacji dla kontenera. Prawidłowe wartości to domyślne, procesowe lub funkcji Hyper-V (obsługiwane tylko w przypadku kontenerów systemu Windows).
...
Liczba kontenerów, które mają być pozostawione, gdy kontener wielokrotnie zginie. Domyślnie ta wartość jest ustawiona na 0, więc żadne kontenery nie zostaną pozostawione.
...
#### <a name="autoremove"></a>AutoUsuń
Flaga wskazująca, czy usuwanie kontenerów zostanie automatycznie (true), czy nie (false), jeśli kontener zostanie zasłany.  W przypadku fałszu sf ręcznie usuń kontenery.
...
Uruchamianie kontenera z włączoną flagą interaktywną. Prawidłowe wartości to true/false. wartość false jest domyślnie wartością false.
...
Poświadczenia repozytorium obrazów kontenerów do ściągania obrazów.
...
Określa zasób punktu końcowego do powiązania z ujawnionym portem kontenera.
...
Określa informacje dotyczące certyfikatu, który zostanie ujawniony w kontenerze.
...
#### <a name="networkconfig"></a>Konfiguracja sieci
...
#### <a name="securityoption"></a>SecurityOption (SecurityOption)
Określa securityoptions dla kontenera.

|Atrybut|Wartość|
|---|---|
|name|SecurityOption (SecurityOption)|
|typ|[SecurityOptionsType](#securityoptionstype-complextype)|
|Minoccurs|0|
|Maxoccurs|Nieograniczony|

#### <a name="imageoverrides"></a>ImageOverrides
Nazwy obrazów odpowiadające numerowi kompilacji systemu operacyjnego do uruchomień.
...
#### <a name="driveroption"></a>DriverOption (DriverOption)
Opcje sterownika, które mają być przekazane do sterownika.

|Atrybut|Wartość|
|---|---|
|name|DriverOption (DriverOption)|
...
|Atrybut|Wartość|
|---|---|
|zawartość|0 elementów, 1 atrybuty|
|zdefiniowane|Globalnie|
...
#### <a name="networktype"></a>Typ sieci
Typ sieci. Obecnie obsługiwane typy to "Otwórz" i "Izolowane".
...
Opisuje skojarzenie między pakietem usługi i siecią kontenerów oraz opcjonalnie skojarzenie między punktami końcowymi pakietu usługi i siecią kontenerów.
...
Nazwa sieci kontenerów (bez uwzględniania liter). Musi to być nazwa już utworzonej sieci kontenerów lub nazwy zarezerwowane, w tym "Otwórz" lub "NAT".
...
|Atrybut|Wartość|
|---|---|
|zawartość|1 element, 4 atrybuty|
|zdefiniowane|Globalnie|
...
Folder źródłowy, który może być folderem na maszynie wirtualnej hostem kontenerów lub trwałym magazynem zdalnym.
...
Lokalizacja, na która źródło jest mapowane w uruchomionym kontenerze. W związku z tym miejsce docelowe nie może być lokalizacją, która już istnieje w kontenerze.
...
#### <a name="driver"></a>Sterownik
Nazwa sterownika wtyczki woluminu Azure Files to "sfazurefile".
...
Opcje sterowników, które mają zostać przekazane do sterownika.
...
Deklaruje folder o nazwie według atrybutu Name w obszarze PackageRoot, który zawiera pliki danych statycznych, które mają być używane przez proces w czasie wykonywania. Service Fabric będą odtwarzać wszystkie pliki EXE i DLLHOSTs określone w hoście i obsługują pakiety po uaktualnieniu dowolnego pakietu danych wymienionego w manifeście usługi.
...
Określa informacje w debugerze, które mają być dołączane podczas aktywowania pakietu kodu.

|Atrybut|Wartość|
|---|---|
|zawartość|4 elementy, 10 atrybutów|
|zdefiniowane|Globalnie|
...
Przesłoniętą pozycję punktu wejścia dla kontenerów, aby można było uruchomić debuger.
...
#### <a name="containerlabel"></a>ContainerLabel (Etykieta kontenera)
Etykiety kontenerów w postaci klucz=wartość.

|Atrybut|Wartość|
|---|---|
|name|ContainerLabel (Etykieta kontenera)|
|typ|xs:string|
|Minoccurs|0|
|Maxoccurs|Nieograniczony|

## <a name="defaultservicestype-complextype"></a>DefaultServicesType complexType
Deklaruje wystąpienia usług tworzone automatycznie za każdym razem, gdy wystąpienie aplikacji jest tworzone dla tego typu aplikacji.

|Atrybut|Wartość|
|---|---|
|zawartość|2 elementy, 0 atrybutów|
|zdefiniowane|Globalnie|
...
Kolekcja usług, które są automatycznie zlokalizowane razem, więc są również przenoszone razem podczas pracy awaryjnej lub zarządzania zasobami.
...
Określa zbieranie zrzutów awaryjnego. Zrzuty awaryjne są zbierane dla plików wykonywalnych, które hostują pakiety kodu wszystkich usług należących do aplikacji.
...
Określa zbieranie śledzenia ETW. Ślady ETW są zbierane dla dostawców, którzy są zarejestrowani przez wszystkie usługi należące do aplikacji.
...
Nieobsługiwane, nie używaj. Obsługa hostingu bibliotek DLL (punkt wejścia zestawu) jest zapewniana przez FWP.exe proces. Service Fabric proces roboczy sieci szkieletowej (FWP.exe) i ładuje zestaw w ramach procesu aktywacji.

|Atrybut|Wartość|
|---|---|
|zawartość|2 elementy, 1 atrybuty|
|zdefiniowane|Globalnie|
...
Nieobsługiwane, nie używaj. Definiuje zasady izolacji dla niezaładowane bibliotek DLL i zarządzanych zestawów załadowanych do hosta DllHost. 
...
Opcje sterowników, które mają zostać przekazane do sterownika. Wtyczka woluminu Azure Files obsługuje następujące opcje sterowników: shareName (udział plików Azure Files, który dostarcza wolumin dla kontenera), storageAccountName (konto magazynu platformy Azure zawierające udział plików Azure Files), storageAccountKey (klucz dostępu dla konta usługi Azure Storage zawierającego udział plików Azure Files).
...
W przypadku ustawienia wartości SecretsStoreRef wartość odwołuje się do klucza tajnego przechowywanego w zaszyfrowanym w klastrze SecureStore, który jest odszyfrowywać i odszyfrowywać po aktywacji.
W przypadku ustawienia na wartość Zaszyfrowane deweloper aplikacji jest odpowiedzialny za utworzenie certyfikatu i użycie polecenia cmdlet Invoke-ServiceFabricEncryptSecret do szyfrowania poufnych informacji.
...
## <a name="endpointbindingpolicytype-complextype"></a>Typ complexType EndpointBindingPolicyType
...
#### <a name="certificateref"></a>CertificateRef
Nazwa certyfikatu punktu końcowego zadeklarowana w sekcji Certyfikaty, która ma być zwracana do klienta. 
...
|Atrybut|Wartość|
|---|---|
|zawartość|0, elementy, 3 atrybuty|
|zdefiniowane|Globalnie|
...
#### <a name="name"></a>Nazwa
Nazwa punktu końcowego do przesłonięcia. Ciąg.
...
Protokół punktu końcowego. Zastępuje wartość w manifeście usługi. Punkty końcowe HTTPS muszą również mieć certyfikaty EndpointCertificate i EndpointBindingPolicy zadeklarowane w manifeście aplikacji. Protokołu nie można zmienić później podczas uaktualniania aplikacji. 
...
#### <a name="type"></a>Typ
Typ punktu końcowego. Zastępuje wartość w manifeście usługi. Wejściowe punkty końcowe są używane do uwidoczniania portu na zewnątrz, a wewnętrzne punkty końcowe są używane do komunikacji wewnątrz aplikacji.
...
## <a name="endpointtype-complextype"></a>Typ punktu końcowego complexType
Definiuje punkt końcowy dla usługi. Można zażądać określonych portów.  Jeśli port nie zostanie jawnie określony, zostanie przypisany z zarezerwowanego zakresu portów aplikacji. Replikom usług uruchomionym w różnych węzłach klastra można przypisać różne numery portów, podczas gdy replikom tej samej usługi uruchomionej w tym samym węźle jest przypisany ten sam port. Takie porty mogą być używane przez repliki usług do różnych celów, takich jak replikacja lub nasłuchiwanie żądań klientów.
...
#### <a name="protocol"></a>Protokół
Protokół punktu końcowego: http, https, tcp lub udp. Punkty końcowe HTTPS muszą również mieć certyfikaty EndpointCertificate i EndpointBindingPolicy zadeklarowane w manifeście aplikacji. Protokołu nie można zmienić później podczas uaktualniania aplikacji. 
...
#### <a name="type"></a>Typ
Typ punktu końcowego.  Wejściowe punkty końcowe są używane do uwidoczniania portu na zewnątrz, a wewnętrzne punkty końcowe są używane do komunikacji wewnątrz aplikacji.
...
#### <a name="certificateref"></a>CertificateRef
Nie używaj tego atrybutu, który nie jest obsługiwany.
...
#### <a name="port"></a>Port
Port zostanie zastąpiony portem określanym przez Microsoft Azure Service Fabric po zarejestrowaniu w Http.sys lub BFE.
...
Plik wykonywalny określony przez entrypoint jest zwykle długotrwałym hostem usługi. Obecność oddzielnego punktu wejścia instalacji pozwala uniknąć konieczności uruchamiania hosta usługi z wysokimi uprawnieniami przez dłuższy czas. Plik wykonywalny określony przez entrypoint jest uruchamiany po pomyślnym zakończeniu pracy instalatoraEntryPoint. Wynikowy proces jest monitorowany i uruchamiany ponownie (począwszy od SetupEntryPoint), jeśli kiedykolwiek zakończy się lub ulega awarii.
...
## <a name="environmenttype-complextype"></a>Typ środowiska complexType

|Atrybut|Wartość|
|---|---|
|zawartość|3 elementy, 0 atrybutów|
|zdefiniowane|Globalnie|
|name|Typ środowiska|
...
Typ zmiennej środowiskowej. Może to być PlainText/Encrypted/SecretsStoreRef.
Jeśli zostanie ustawiona wartość SecretsStoreRef, pobierzemy wartość odwołania z secretStore.
W przypadku ustawienia na wartość Zaszyfrowane deweloper aplikacji jest odpowiedzialny za utworzenie certyfikatu i użycie polecenia cmdlet Invoke-ServiceFabricEncryptSecret do szyfrowania poufnych informacji.
...
|Atrybut|Wartość|
|---|---|
|zawartość|1 elementy, 0 atrybutów|
|zdefiniowane|Globalnie|
...
Wartość true, jeśli plik wykonywalny znajduje się poza pakietem aplikacji. Sprawdzanie obecności programu w pakiecie aplikacji jest pomijane.
...
Ostrzeżenie! Nie używaj przekierowywania konsoli w aplikacji produkcyjnej, a tylko do lokalnego tworzenia i debugowania. Przekierowuje dane wyjściowe konsoli ze skryptu uruchamiania do pliku wyjściowego w folderze aplikacji o nazwie "log" w węźle klastra, w którym aplikacja jest wdrażana i uruchamiana.
...
Jest on używany tylko wtedy, gdy poświadczenie jest X509. Określa sposób znalezienia certyfikatu według nazwy lub odcisku palca 
...
Jest to używane tylko wtedy, gdy poświadczenie jest X509. Jest to rzeczywista nazwa lub odcisk palca certyfikatu.
...
|Atrybut|Wartość|
|---|---|
|zawartość|21 elementów, 0 atrybutów|
|zdefiniowane|Globalnie|
...
#### <a name="upgradeorchestrationservicereplicatorendpoint"></a>UpgradeO nietrwaleusługaReplicatorEndpoint

|Atrybut|Wartość|
|---|---|
|name|UpgradeO nietrwaleusługaReplicatorEndpoint|
|typ|[InternalEndpointType](#internalendpointtype-complextype)|
|Minoccurs|0|
...
Definiuje ścieżkę do udostępnionego dziennika.
...
Określony identyfikator GUID, który ma być identyfikatorem dziennika udostępnionego.
...
Opisuje Microsoft Azure Service Fabric Node.

|Atrybut|Wartość|
|---|---|
|zawartość|0, atrybuty 6|
|zdefiniowane|Globalnie|
|name|FabricNodeType|
...
#### <a name="nodename"></a>NodeName (Nazwa węzła)
Nazwa wystąpienia węzła.

|Atrybut|Wartość|
|---|---|
|name|NodeName (Nazwa węzła)|
|typ|xs:string|
|używanie|wymagane|

#### <a name="ipaddressorfqdn"></a>IPAddressOrFQDN
Adres IP lub fQDN maszyny, na której ma być umieszczany ten węzeł.
...
#### <a name="isseednode"></a>IsSeedNode
Flaga wskazująca, czy ten węzeł jest węzłem iniekcjatora.
...
|Atrybut|Wartość|
|---|---|
|zawartość|1 elementy, 2 atrybuty|
|zdefiniowane|Globalnie|
|name|FileStoreType|
...
#### <a name="passwordencrypted"></a>HasłoSzyfrowane
...
Obraz kontenera odpowiadający numerowi wersji kompilacji systemu operacyjnego do uruchomień. Jeśli atrybut systemu operacyjnego nie zostanie określony, zakłada się, że obraz kontenera działa we wszystkich wersjach systemu operacyjnego i zastępuje obraz określony w manifeście usługi.
...
## <a name="imagetype-complextype"></a>Typ obrazu complexType
...
#### <a name="os"></a>Os
Kompilacja wersji systemu operacyjnego. Na przykład wersja kompilacji systemu Windows Server 1709 to 16299.
...
Zawiera informacje o infrastrukturze dla tego Microsoft Azure Service Fabric klastra.
...
|Atrybut|Wartość|
|---|---|
|zawartość|2 elementy, 7 atrybutów|
|zdefiniowane|Globalnie|
...
#### <a name="ipaddressorfqdn"></a>IPAddressOrFQDN
Adres IP lub WQDN maszyny, na której ma być umieszczany ten węzeł.
...
Nazwa roli, która łączy się z ref typem węzła, który jest zdefiniowany w sekcji NodeTypes.
...
#### <a name="isseednode"></a>IsSeedNode
Wskazuje, czy węzeł jest węzłem iniekcją.
...
Zasób, na który ta usługa powinna być zrównoważona, taki jak użycie pamięci lub procesora CPU.  Zawiera informacje o tym, ile tego zasobu zużywa domyślnie każda replika lub wystąpienie tej usługi.

|Atrybut|Wartość|
|---|---|
|zawartość|0, atrybuty 5|
|zdefiniowane|Globalnie|
|name|LoadMetricType|
...
#### <a name="name"></a>Nazwa
Unikatowy identyfikator metryki w klastrze z Menedżer zasobów klastra perspektywy klienta.
...
Domyślna ilość obciążenia, które ta usługa bez stanowa tworzy dla tej metryki.
...
Domyślna ilość obciążenia, które ta usługa będzie obciążać dla tej metryki, gdy jest to replika podstawowa.
...
Domyślna wielkość obciążenia, które ta usługa będzie obciążać dla tej metryki, gdy jest to replika pomocnicza.
...
Określa wagę metryki względem innych metryk skonfigurowanych dla tej usługi. Jeśli w czasie wykonywania dwie metryki zakończą się konfliktem, Menedżer zasobów klastra preferuje metrykę o większej wag. Zero powoduje wyłączenie równoważenia obciążenia dla tej metryki.
...
```
## <a name="localstoretype-complextype"></a>Typ_złożonego typu LocalStoreType
...
Opisuje typ LogicalDirectoryType.
...
#### <a name="logicaldirectoryname"></a>Nazwa_logiczna
...
Nieobsługiwane, nie używaj. Nazwa zarządzanego zestawu (na przykład Queue.dll), który ma być hostem.
...
Opisuje zasady sieciowe, w tym zasady sieci kontenera dla pakietu usług.
...
Opisuje zasady sieci kontenerów dla pakietu usług.
...
## <a name="paasroletype-complextype"></a>Typ complexType PaaSRoleType
...
## <a name="paasvotetype-complextype"></a>Typ complexType PaaSVoteType

|Atrybut|Wartość|
|---|---|
|zawartość|0, elementy, 3 atrybuty|
|zdefiniowane|Globalnie|
|name|Typ PaaSVoteType|
...
Wskazuje, czy kod, konfiguracja lub pakiet danych powinien być współużytkowane przez wystąpienia usługi tego samego typu.
...
Nazwa kodu, konfiguracji lub pakietu danych, który ma zostać udostępniony. Musi odpowiadać nazwie pakietu zdefiniowanego w manifeście usługi.
...
## <a name="parametertype-complextype"></a>Typ parametru complexType
...
#### <a name="isencrypted"></a>Isencrypted
W przypadku wartości true wartość tego parametru jest szyfrowana
...
|zawartość|1 elementy, 0 atrybutów|
|zdefiniowane|Globalnie|
|name|ParametersType (Typ parametrów)|
...
#### <a name="passwordencrypted"></a>HasłoSzyfrowane
Określa, czy hasło jest zaszyfrowane lub w postaci zwykłego tekstu. Ten atrybut jest przestarzały. Użyj atrybutu "Type", aby wskazać zaszyfrowane hasło.
...
Ta wartość definiuje typ hasła określonego w atrybutu "Hasło". Może to być SecretsStoreRef/Encrypted/PlainText.
Jeśli zostanie ustawiona wartość SecretsStoreRef, pobierzemy wartość odwołania z secretStore.
W przypadku ustawienia na wartość Zaszyfrowane deweloper aplikacji jest odpowiedzialny za utworzenie certyfikatu i użycie polecenia cmdlet Invoke-ServiceFabricEncryptSecret do szyfrowania poufnych informacji.
...
Limit miękki (w MB) dla nadzoru pamięci, który jest wymuszany tylko wtedy, gdy w węźle zostanie wykryta sedno pamięci. Musi być dodatnią liczbą całkowitą.
...
#### <a name="cpushares"></a>Udziały cpu
...
#### <a name="cpupercent"></a>CpuPercent (Wydajność procesora CPU)
Procent dostępnych procesorów CPU dostępnych do wykorzystania (tylko system Windows). Musi być dodatnią liczbą całkowitą. Jeśli dla pakietu usług określono limity procesora CPU, ten parametr jest skutecznie ignorowany.

|Atrybut|Wartość|
|---|---|
|name|CpuPercent (Wydajność procesora CPU)|
|typ|xs:string|
|używanie|optional|
|default|0|

#### <a name="maximumiops"></a>MaximumIOps
Maksymalna szybkość we/wy (odczyt i zapis) w zakresie liczby IOPS, których można użyć. Musi być dodatnią liczbą całkowitą.
...
Maksymalna liczba we/wy (bajtów na sekundę), które mogą być używane (odczyt i zapis). Musi być dodatnią liczbą całkowitą.
...
Blokuj wagę we/wy względem innych pakietów kodu. Musi być dodatnią liczbą całkowitą z wartości od 10 do 1000.
...
Przydział dysku dla kontenerów.  Musi być dodatnią liczbą całkowitą.
...
Limity pamięci jądra w bajtach.  Musi być dodatnią liczbą całkowitą.  Należy pamiętać, że jest to funkcja specyficzna dla systemu Linux, a w przypadku ustawienia platformy Docker w systemie Windows wystąpi błąd.
...
#### <a name="endpoints"></a>Punkty końcowe
Punkty końcowe usługi do przesłonięcia.
...
Opisuje zasoby używane przez tę usługę, które można zadeklarować bez modyfikowania skompilowanego kodu i zmienić podczas wdrażania usługi. Dostęp do tych zasobów jest kontrolowany za pośrednictwem sekcji Podmioty zabezpieczeń i Zasady manifestu aplikacji.
...
Określa użytkownika lokalnego lub lokalnego konta systemowego, który pakiet kodu usługi zostanie uruchomiony jako. Konta domeny są obsługiwane we wdrożeniach systemu Windows Server, Azure Active Directory są dostępne. Domyślnie aplikacje są uruchamiane na koncie, na Fabric.exe jest uruchamiany proces. Aplikacje mogą być również uruchamiane jako inne konta, które muszą być zadeklarowane w sekcji Podmioty zabezpieczeń. Jeśli zasady Uruchom jako są stosowane do usługi, a manifest usługi deklaruje zasoby punktu końcowego za pomocą protokołu HTTP, należy również określić zasady SecurityAccessPolicy, aby upewnić się, że porty przydzielone do tych punktów końcowych są prawidłowo kontrolą dostępu dla konta użytkownika Uruchom jako, w ramach których działa usługa. W przypadku punktu końcowego HTTPS należy również zdefiniować zasady EndpointBindingPolicy, aby wskazać nazwę certyfikatu, który ma być zwracany do klienta.
...
#### <a name="codepackageref"></a>CodePackageRef
Nazwa pakietu kodu. Musi odpowiadać nazwie codepackage określonej w manifeście usługi.
...
Konto użytkownika, jako które będzie działać pakiet kodu usługi.  Konto użytkownika musi być zadeklarowane w sekcji Podmioty zabezpieczeń. Często preferowane jest uruchomienie punktu wejścia instalacji przy użyciu lokalnego konta systemowego, a nie konta administratorów.
...
#### <a name="entrypointtype"></a>EntryPointType
Instalator to setupEntryPoint zadeklarowany w manifeście usługi, uprzywilejowanym punkcie wejścia, który jest uruchamiany przed dowolnym innym punktem wejścia.  Main to punkt EntryPoint zadeklarowany w manifeście usługi, zazwyczaj długotrwałym hostem usługi. Wszystko to wszystkie punkty wejścia.
...
## <a name="scalingpolicytype-complextype"></a>Typ complexType dla typu ScalingPolicyType
...
```
## <a name="securityaccesspolicytype-complextype"></a>Typ complexType securityAccessPolicyType
Przyznaje uprawnienia dostępu do podmiotu zabezpieczeń w zasobie (takim jak punkt końcowy) zdefiniowanym w manifeście usługi. Zazwyczaj bardzo przydatne jest kontrolowanie i ograniczanie dostępu usług do różnych zasobów w celu zminimalizowania ryzyka związanego z bezpieczeństwem. Jest to szczególnie ważne, gdy aplikacja jest budowaną na podstawie kolekcji usług z platformy handlowej, które są opracowywane przez różnych deweloperów.
...
Zasób, do których jest udzielany dostęp, zadeklarowany i skonfigurowany w manifeście usługi.
...
Użytkownik lub grupa, do których są przypisywane prawa dostępu do zasobu, musi być zadeklarowana w sekcji Podmioty zabezpieczeń.
...
Prawa do udzielenia: Odczyt, Zmiana lub Pełne. Wartość domyślna to Odczyt.
...
Opis podmiotów zabezpieczeń (użytkowników, grup) wymaganych przez tę aplikację do uruchamiania usług i zabezpieczania zasobów. Podmioty zabezpieczeń są przywołyowane w sekcjach zasad.
...
Wskazuje, czy informacje specyficzne dla środowiska uruchomieniowego sieci szkieletowej powinny być widoczne dla pakietów kodu w pakiecie usługi.
...
Wskazuje, Service Fabric magazyn blokowy powinien być używany do zapewnienia woluminu dla kontenerów w pakiecie usługi.
...
Typ podstawowy opisujący stanowy lub bez stanowy typ ServiceGroupType.

|Atrybut|Wartość|
|---|---|
|zawartość|4 elementy, 2 atrybuty|
|zdefiniowane|Globalnie|
...
Zdefiniowany przez użytkownika identyfikator typu dla grupy usług, na przykład "ActorQueueSGType". Ta wartość jest używana w pliku ApplicationManifest.xml do identyfikowania grupy usług.
...
Określa, czy wystąpienie grupy usług jest tworzone przez niejawną fabrykę. W przypadku wartości false (wartość domyślna) jeden z pakietów kodu musi zarejestrować fabrykę grupy usług
...
Opisuje zasady (powiązanie punktu końcowego, udostępnianie pakietów, uruchamianie jako i dostęp zabezpieczeń) do zastosowania w manifeście zaimportowanej usługi.

|Atrybut|Wartość|
|---|---|
|zawartość|11 elementów, 0 atrybutów|
|zdefiniowane|Globalnie|
...
#### <a name="securityaccesspolicy"></a>Zasady SecurityAccessPolicy

|Atrybut|Wartość|
|---|---|
|name|Zasady SecurityAccessPolicy|
...
Określa certyfikat, który ma być zwracany do klienta dla punktu końcowego HTTPS.
...
Określa zasady, które określają sposób, w jaki środowisko uruchomieniowe usługi Service Fabric jest udostępniane replikom.
...
Definiuje zasady nadzoru nad zasobami, które są stosowane na poziomie całego pakietu usług.
...
Config Packages to be mounted inside the container .Config Packages to be mounted inside the container .Config Packages to be mounted inside the container .Config
...
Deklaratywnie opisuje typ i wersję usługi. Zawiera on listę niezależnie uaktualnialnych pakietów kodu, konfiguracji i danych, które razem tworzą pakiet usługi w celu obsługi co najmniej jednego typu usługi. Określane są również zasoby, ustawienia diagnostyczne i metadane usługi, takie jak typ usługi, właściwości kondycji i metryki równoważenia obciążenia.

|Atrybut|Wartość|
|---|---|
|zawartość|7 elementów, 1 atrybuty|
|zdefiniowane|Globalnie|
...
Identyfikator manifestu usługi, ciąg bez struktury.
...
Definiuje typy usług obsługiwane przez pakiet CodePackage w tym manifeście. Gdy usługa jest aktywna względem jednego z tych typów usług, wszystkie pakiety kodu zadeklarowane w tym manifeście są aktywowane przez uruchomienie ich punktów wejścia. Typy usług są deklarowane na poziomie manifestu, a nie na poziomie pakietu kodu.
...
|---|---|
|name|Zasoby|
|typ|[Typ zasobów](#resourcestype-complextype)|
|Minoccurs|0|
...
#### <a name="hostname"></a>Hostname (Nazwa hosta)
Określ wartość Nazwa hosta dla kontenera.
...
#### <a name="portbinding"></a>PortBinding
Określa zasób punktu końcowego, który ma być powiązyny z portem ujawnionym kontenera.
...
Pakiet ServicePackage reprezentuje wersjonacyjną jednostkę wdrażania i aktywacji. Wersja pakietu ServicePackage jest określana na podstawie wersji manifestu i wersji zastąpień.

|Atrybut|Wartość|
|---|---|
|zawartość|11 elementów, 4 atrybuty|
|zdefiniowane|Globalnie|
...
#### <a name="statelessservice"></a>Usługa bez stanowa

|Atrybut|Wartość|
|---|---|
|name|Usługa bez stanowa|
...
## <a name="servicetype-complextype"></a>Typ usługi complexType
Typ podstawowy, który definiuje Microsoft Azure Service Fabric usługi.

|Atrybut|Wartość|
|---|---|
|zawartość|5 elementów, 2 atrybuty|
|zdefiniowane|Globalnie|
|name|ServiceType|
...
#### <a name="servicetypename"></a>ServiceTypeName (Nazwa typu usługi)
Nazwa typu usługi zadeklarowanego w manifeście usługi, który zostanie wystąpienia.

|Atrybut|Wartość|
|---|---|
|name|ServiceTypeName (Nazwa typu usługi)|
|używanie|wymagane|
...
#### <a name="loadmetrics"></a>LoadMetrics
Metryki obciążenia zgłoszone przez tę usługę używane do równoważenia zasobów.
...
#### <a name="placementconstraints"></a>PlacementConstraints
Służy do kontrolowania węzłów w klastrze, w których usługa może działać. Para klucz/wartość, która opisuje nazwę właściwości węzła i wymagania usługi dotyczące wartości. Poszczególne instrukcje można grupować razem z prostą logiką logiczną w celu utworzenia niezbędnego ograniczenia. Na przykład "(FirmwareVersion>12  && InDMZ == True)".
...
Deklaruje tagi wymagane do umieszczenia usługi. Przydatne w przypadku dynamicznego wywierania wpływu na umieszczanie usług. 
...
Deklaruje tagi wymagane do umieszczania i uruchamiania usługi. Przydatne do dynamicznego wpływania na umieszczanie i uruchamianie usługi. 
...
## <a name="servicetypeextensionpolicypropertiestype-complextype"></a>Typ_usługiExtensionPolicyPropertiesType complexType
...
## <a name="servicetypehealthpolicytype-complextype"></a>Typ usługi ServiceTypeHealthPolicyType complexType

|Atrybut|Wartość|
|---|---|
|zawartość|0 elementów, 3 atrybuty|
|zdefiniowane|Globalnie|
|name|Typ usługi ServiceTypeHealthPolicyType|
...
Określa maksymalną tolerowaną wartość procentową usług w złej kondycji, zanim aplikacja zostanie uznana za zła. Domyślna wartość procentowa: 0.
...
Określa maksymalną tolerowaną wartość procentową partycji w złej kondycji, zanim usługa zostanie uznana za zła. Domyślna wartość procentowa: 0.
...
Określa maksymalną tolerowaną wartość procentową replik w złej kondycji, zanim partycja zostanie uznana za zła. Domyślna wartość procentowa: 0.
...
## <a name="servicetypetype-complextype"></a>Typ usługi complexTypeType
Typ podstawowy opisujący stanowy lub bez stanowy typ usługi.
...
#### <a name="servicetypename"></a>ServiceTypeName (Nazwa typu usługi)
Zdefiniowany przez użytkownika identyfikator typu dla usługi. Na przykład "QueueType" lub "CalculatorType". Ta wartość jest używana w pliku ApplicationManifest.xml do identyfikowania usługi.
...
#### <a name="statelessservicetype"></a>StatelessServiceType
Opisuje bez stanową typ usługi.
...
Deklaruje ustawienia konfiguracji w manifeście usługi do przesłaniania. Składa się z co najmniej jednej sekcji par klucz-wartość. Wartości parametrów można zaszyfrować przy użyciu Invoke-ServiceFabricEncryptSecret cmdlet.
...
Sekcja w pliku Settings.xml do przesłonięcia.
...
Opisuje ustawienia zdefiniowane przez użytkownika dla servicecomponent lub aplikacji. Składa się z co najmniej jednej sekcji par klucz-wartość.
...
#### <a name="section"></a>Sekcja
Zdefiniowana przez użytkownika nazwana sekcja.
...
Definiuje bez stanową grupę usług.
...
Opisuje typ bez stanowej grupy usług.
...
Opisuje typ usługi bez stanowej.
...
Nieobsługiwane, nie używaj. Nazwa zestawu nieza zdalnego (na przykład Queue.dll) hosta.
...
|Atrybut|Wartość|
|---|---|
|zawartość|0 elementów, 11 atrybutów|
|zdefiniowane|Globalnie|
...
Parametry pliku exe punktu wejścia instalacji
