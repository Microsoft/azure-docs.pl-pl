...
Kontrola dostępu oparta na rolach (RBAC) korzysta z ról i zasad aprowizacji, aby szacować, testować i wymuszać procesy biznesowe oraz zasady udzielania dostępu użytkownikom. Administratorzy kluczy mogą tworzyć zasady aprowizacji i przypisywać użytkowników do ról oraz definiować zestawy uprawnień do zasobów dla tych ról. RBAC rozszerza rozwiązanie do zarządzania tożsamościami w celu używania procesów opartych na oprogramowaniu i zmniejsza ręczną interakcję użytkownika w procesie aprowizacji.
Kontrola RBAC usługi Azure AD pozwala firmie ograniczyć liczbę operacji, które użytkownik może wykonać po uzyskaniu dostępu do Azure Portal. Za pomocą funkcji RBAC do kontrolowania dostępu do portalu Administratorzy IT są delegowane dostępu przy użyciu następujących podejścia do zarządzania dostępem:
