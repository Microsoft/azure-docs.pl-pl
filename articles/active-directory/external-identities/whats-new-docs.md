---
title: Co nowego w Azure Active Directory tożsamości zewnętrznych
description: Nowa i zaktualizowana dokumentacja dla Azure Active Directory tożsamości zewnętrznych.
ms.date: 03/08/2021
ms.service: active-directory
ms.subservice: B2B
ms.topic: reference
ms.workload: identity
ms.author: mimart
author: msmimart
manager: CelesteDG
ms.openlocfilehash: dda3f7c8f4a9a21e16ebab7d243a004c01a40eed
ms.sourcegitcommit: e6de1702d3958a3bea275645eb46e4f2e0f011af
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 03/20/2021
ms.locfileid: "102453670"
---
# <a name="azure-active-directory-external-identities-whats-new"></a>Azure Active Directory tożsamości zewnętrznych: co nowego

Witamy w dokumentacji dotyczącej Azure Active Directory zewnętrznych tożsamości. W tym artykule wymieniono nowe dokumenty, które zostały dodane oraz te, które mają znaczące aktualizacje w ciągu ostatnich trzech miesięcy. Aby dowiedzieć się, co nowego w usłudze tożsamości zewnętrznych, zobacz [co nowego w programie Azure Active Directory](../fundamentals/whats-new.md).

## <a name="february-2021"></a>Luty 2021 r.

### <a name="new-articles"></a>Nowe artykuły

- [Zresetuj stan wykupu dla użytkownika-gościa](reset-redemption-status.md)

### <a name="updated-articles"></a>Zaktualizowane artykuły

- [Azure Active Directory najlepszych rozwiązań B2B](b2b-fundamentals.md)
- [Włączanie zewnętrznej współpracy B2B i zarządzanie osobami, które mogą zapraszać gości](delegate-invitations.md)
- [Azure Active Directory często zadawane pytania dotyczące współpracy B2B](faq.md)
- [Wyślij wiadomość e-mail do jednorazowego uwierzytelniania kodu dostępu](one-time-passcode.md)
- [Realizacja zaproszenia do współpracy B2B w usłudze Azure Active Directory](redemption-experience.md)
- [Rozwiązywanie problemów Azure Active Directory współpracy B2B](troubleshoot.md)
- [Właściwości Azure Active Directory użytkownika współpracy B2B](user-properties.md)
- [Co to jest dostęp użytkowników-gości w usłudze Azure Active Directory B2B?](what-is-b2b.md)
- [Azure Active Directory tożsamości zewnętrznych: co nowego](whats-new-docs.md)
- [Zezwalanie na zaproszenia dla użytkowników B2B z określonych organizacji i blokowanie ich](allow-deny-list.md)
- [Azure Active Directory interfejs API współpracy B2B i dostosowywanie](customize-invitation-api.md)
- [Zapraszanie użytkowników wewnętrznych do współpracy B2B](invite-internal-users.md)
- [Microsoft 365 udostępnianie zewnętrzne i Azure Active Directory (Azure AD) Współpraca B2B](o365-external-user.md)
- [Bezpośrednia Federacja z dostawcami AD FS i innych firm dla użytkowników-Gości (wersja zapoznawcza)](direct-federation.md)

## <a name="january-2021"></a>Styczeń 2021 r.

### <a name="updated-articles"></a>Zaktualizowane artykuły
- [Zezwalanie na zaproszenia dla użytkowników B2B z określonych organizacji i blokowanie ich](allow-deny-list.md)
- [Jak użytkownicy w organizacji mogą zapraszać użytkowników-Gości do aplikacji](add-users-information-worker.md)


## <a name="december-2020"></a>Grudzień 2020 r.

### <a name="updated-articles"></a>Zaktualizowane artykuły
- [Azure Active Directory często zadawane pytania dotyczące współpracy B2B](faq.md)
- [Dodaj firmę Google jako dostawcę tożsamości dla użytkowników gościa B2B](google-federation.md)
- [Dostawcy tożsamości dla tożsamości zewnętrznych](identity-providers.md)
- [Realizacja zaproszenia do współpracy B2B w usłudze Azure Active Directory](redemption-experience.md)
- [Dodawanie łącznika interfejsu API do przepływu użytkownika](self-service-sign-up-add-api-connector.md)
- [Dodawanie niestandardowego przepływu pracy zatwierdzenia do rejestracji samoobsługowej](self-service-sign-up-add-approvals.md)
- [Rozwiązywanie problemów Azure Active Directory współpracy B2B](troubleshoot.md)
- [Co to jest dostęp użytkowników-gości w usłudze Azure Active Directory B2B?](what-is-b2b.md)
- [Azure Active Directory najlepszych rozwiązań B2B](b2b-fundamentals.md)
- [Włączanie zewnętrznej współpracy B2B i zarządzanie osobami, które mogą zapraszać gości](delegate-invitations.md)
- [Wyślij wiadomość e-mail do jednorazowego uwierzytelniania kodu dostępu](one-time-passcode.md)

## <a name="november-2020"></a>Listopad 2020 r.

### <a name="updated-articles"></a>Zaktualizowane artykuły
- [Microsoft 365 udostępnianie zewnętrzne i Azure Active Directory (Azure AD) Współpraca B2B](o365-external-user.md)
- [Przyznawanie lokalnie zarządzanym kontom partnerskim dostępu do zasobów w chmurze przy użyciu funkcji współpracy B2B w usłudze Azure AD](hybrid-on-premises-to-cloud.md)
- [Właściwości Azure Active Directory użytkownika współpracy B2B](user-properties.md)


## <a name="october-2020"></a>Październik 2020 r.

### <a name="updated-articles"></a>Zaktualizowane artykuły
- [Dodaj firmę Google jako dostawcę tożsamości dla użytkowników gościa B2B](google-federation.md)
- [Jak użytkownicy w organizacji mogą zapraszać użytkowników-Gości do aplikacji](add-users-information-worker.md)
- [Zezwalanie na zaproszenia dla użytkowników B2B z określonych organizacji i blokowanie ich](allow-deny-list.md)
- [Azure Active Directory często zadawane pytania dotyczące współpracy B2B](faq.md)
- [Dokumentacja tożsamości zewnętrznych](index.yml)
- [Realizacja zaproszenia do współpracy B2B w usłudze Azure Active Directory](redemption-experience.md)
- [Dodawanie niestandardowego przepływu pracy zatwierdzenia do rejestracji samoobsługowej](self-service-sign-up-add-approvals.md)
- [Co to są tożsamości zewnętrzne w Azure Active Directory?](compare-with-b2c.md)
- [Dodawanie łącznika interfejsu API do przepływu użytkownika](self-service-sign-up-add-api-connector.md)


## <a name="september-2020"></a>Wrzesień 2020

### <a name="updated-articles"></a>Zaktualizowane artykuły
- [Elementy wiadomości e-mail z zaproszeniem do współpracy B2B — Azure Active Directory](invitation-email-elements.md)
- [Rozwiązywanie problemów Azure Active Directory współpracy B2B](troubleshoot.md)
- [Model rozliczeń dla tożsamości zewnętrznych usługi Azure AD](external-identities-pricing.md)
- [Dodaj firmę Google jako dostawcę tożsamości dla użytkowników gościa B2B](google-federation.md)


## <a name="august-2020"></a>Sierpień 2020 r.

### <a name="new-articles"></a>Nowe artykuły
- [Model rozliczeń dla tożsamości zewnętrznych usługi Azure AD](external-identities-pricing.md)


### <a name="updated-articles"></a>Zaktualizowane artykuły
- [Włączanie zewnętrznej współpracy B2B i zarządzanie osobami, które mogą zapraszać gości](delegate-invitations.md)
- [Dodawanie łącznika interfejsu API do przepływu użytkownika](self-service-sign-up-add-api-connector.md)
- [Dodawanie niestandardowego przepływu pracy zatwierdzenia do rejestracji samoobsługowej](self-service-sign-up-add-approvals.md)


