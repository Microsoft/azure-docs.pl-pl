...
1. Kliknij pozycję **Ustaw dodatkowe adresy URL** i wykonaj następujące kroki, jeśli chcesz skonfigurować aplikację w trybie inicjowania programu **SP** :
    ...
    > [!NOTE]
    > Te wartości nie są prawdziwe. Należy je zastąpić rzeczywistymi wartościami identyfikatora, adresu URL odpowiedzi i adresu URL logowania. Skontaktuj się z [zespołem pomocy technicznej EasySSO for BitBucket](mailto:support@techtime.co.nz) , aby uzyskać te wartości. Przydatne mogą się również okazać wzorce przedstawione w sekcji **Podstawowa konfiguracja protokołu SAML** w witrynie Azure Portal.
...
1. Zaloguj się do swojego wystąpienia usługi EasySSO for BitBucket z uprawnieniami administratora i przejdź do sekcji **Zarządzanie aplikacjami** .

    ![Zarządzanie aplikacjami](./media/easysso-for-bitbucket-tutorial/jira-admin-1.png)

1. Kliknij pozycję **EasySSO**.

    ![Łatwe logowanie jednokrotne](./media/easysso-for-bitbucket-tutorial/jira-admin-2.png)
...
1. Wybierz **kartę certyfikaty** u góry, a zobaczysz poniższy ekran i Zlokalizuj **plik** **certyfikatu (base64)** , który został zapisany we wcześniejszych krokach konfiguracji **rejestracji jednokrotnej usługi Azure AD** . Dostępne są następujące opcje:
    ...
    ![Adres URL metadanych](./media/easysso-for-bitbucket-tutorial/jira-admin-4.png)
...
1. W celu uzyskania najlepszego środowiska testowania przejdź do karty **& działanie** i zaznacz opcję **przycisk Zaloguj SAML** . Spowoduje to włączenie osobnego przycisku na ekranie logowania JIRA w celu przetestowania kompleksowej integracji protokołu SAML usługi Azure AD. Ten przycisk można opuścić i skonfigurować jego położenie, kolor oraz tłumaczenie dla trybu produkcji.
    ...
    ![Wygląd &](./media/easysso-for-bitbucket-tutorial/jira-admin-5.png)
...
W tej sekcji użytkownik o nazwie B. Simon został utworzony w EasySSO dla BitBucket. EasySSO dla BitBucket obsługuje Inicjowanie obsługi użytkowników just in Time, która jest domyślnie **wyłączona** . Aby włączyć Inicjowanie obsługi użytkowników, należy jawnie zaznaczyć opcję **Utwórz użytkownika przy pomyślnym logowaniu** w sekcji Ogólne konfiguracji wtyczki EasySSO. Jeśli użytkownik nie istnieje jeszcze w usłudze EasySSO dla BitBucket, zostanie utworzony nowy po uwierzytelnieniu.

Jeśli jednak użytkownik nie chce włączać automatycznego inicjowania obsługi użytkowników przy pierwszym logowaniu użytkownika, musi istnieć w katalogach użytkowników zaplecza EasySSO dla wystąpienia BitBucket, które używają, takich jak LDAP lub Atlassian.

![Aprowizowanie użytkowników](./media/easysso-for-bitbucket-tutorial/jira-admin-6.png)

## <a name="test-sso"></a>Testuj Logowanie jednokrotne

W tej sekcji przetestujesz konfigurację logowania jednokrotnego usługi Azure AD przy użyciu panelu dostępu.

Po kliknięciu kafelka EasySSO dla BitBucket w panelu dostępu należy automatycznie zalogować się do EasySSO dla BitBucket, dla którego skonfigurowano Logowanie jednokrotne. Aby uzyskać więcej informacji na temat panelu dostępu, zobacz [wprowadzenie do panelu dostępu](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction).
