...
Oświadczenia są obecne tylko wtedy, gdy istnieje wartość do wypełnienia. Twoja aplikacja nie powinna podejmować zależności od obecnego żądania. Do przykładów należą: `pwd_exp` (nie każdy dzierżawca wymaga hasła) i `family_name` przepływy ([poświadczenia klienta] (v2-OAuth2-Client-creds-Grant-Flow.MD) są w imieniu aplikacji, które nie mają nazw). Oświadczenia używane do sprawdzania poprawności tokenu dostępu będą zawsze obecne.
