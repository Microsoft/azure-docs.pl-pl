...
------------ | --------- | ---------- | ----------
Użytkownicy i kontakty | <ul><li>Wyliczanie listy wszystkich użytkowników i kontaktów<li>Odczytywanie wszystkich publicznych właściwości użytkowników i kontaktów</li><li>Zapraszanie gości<li>Zmiana własnego hasła<li>Zarządzanie własnym numerem telefonu komórkowego<li>Zarządzanie własnym zdjęciem<li>Unieważnianie własnych tokenów odświeżania</li></ul> | <ul><li>Odczytywanie własnych właściwości<li>Odczytywanie właściwości nazwy wyświetlanej, adresu e-mail, nazwy logowania, zdjęcia, głównej nazwy użytkownika i typu użytkownika innych użytkowników i kontaktów<li>Zmiana własnego hasła<li>Wyszukiwanie innego użytkownika według pola ObjectId (jeśli jest dozwolone)<li>Odczytywanie informacji o menedżerze i raportach bezpośrednich innych użytkowników</li></ul> | <ul><li>Odczytywanie własnych właściwości<li>Zmiana własnego hasła</li><li>Zarządzanie własnym numerem telefonu komórkowego</li></ul>
Grupy | <ul><li>Tworzenie grup zabezpieczeń<li>Tworzenie Microsoft 365 grup<li>Wyliczanie listy wszystkich grup<li>Odczytywanie wszystkich właściwości grup<li>Odczytywanie nieukrytych członkostw w grupach<li>Odczytywanie ukrytych Microsoft 365 członkostwa w grupach dla grupy przyłączone<li>Zarządzanie właściwościami, własnością i członkostwem w grupach, których użytkownik jest właścicielem<li>Dodawanie gości do posiadanych grup<li>Zarządzanie dynamicznymi ustawieniami członkostwa<li>Usuwanie posiadanych grup<li>Przywracanie posiadanych Microsoft 365 zasobów</li></ul> | <ul><li>Odczytywanie właściwości nieutanych grup, w tym członkostwa i własności (nawet grup niespołączenia)<li>Odczytywanie ukrytych Microsoft 365 grup dla grup przyłączone<li>Wyszukiwanie grup według nazwy wyświetlanej lub pola ObjectId (jeśli jest dozwolone)</li></ul> | <ul><li>Odczytywanie identyfikatora obiektu dla grup sprzężenia<li>Odczytywanie członkostwa i własności grup przyłączone w niektórych Microsoft 365 aplikacji (jeśli jest to dozwolone)</li></ul>
...
## <a name="restrict-member-users-default-permissions"></a>Ograniczanie domyślnych uprawnień użytkowników członkowskich 

Uprawnienia domyślne dla użytkowników-członków można ograniczyć w następujący sposób:
