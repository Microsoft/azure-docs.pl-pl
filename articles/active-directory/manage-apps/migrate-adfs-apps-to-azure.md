...
Wiele organizacji korzysta z oprogramowania jako usługi (SaaS) lub niestandardowych aplikacji biznesowych, które są federacyjne bezpośrednio do AD FS, wraz z pakietem Office 365 i aplikacjami opartymi na usłudze Azure AD. 
...
Aplikacje LOB, które używają protokołu OAuth 2,0, OpenID Connect Connect lub WS-Federation, można zintegrować z usługą Azure AD jako [rejestracje aplikacji](https://docs.microsoft.com/azure/active-directory/develop/app-registrations-training-guide-for-app-registrations-legacy-users). Integruj aplikacje niestandardowe korzystające z protokołu SAML 2,0 lub WS-Federation jako [aplikacji innych niż Galeria](https://docs.microsoft.com/azure/active-directory/manage-apps/add-non-gallery-app) na stronie aplikacje dla przedsiębiorstw w [Azure Portal](https://portal.azure.com/).
...
Zwróć uwagę, że podpisane żądania są akceptowane, ale podpis nie jest weryfikowany.  
...
| **Identyfikator/"wystawca"**<p>Identyfikator dostawcy tożsamości z perspektywy aplikacji (czasami nazywany "IDENTYFIKATORem wystawcy").<p>W tokenie SAML wartość jest wyświetlana jako element wystawcy.| Identyfikator AD FS jest zazwyczaj identyfikatorem usługi federacyjnej w AD FS Management w obszarze **> Edytowanie właściwości usługa federacyjna**. Na przykład:[http://fs.contoso.com/adfs/services/trust](http://fs.contoso.com/adfs/services/trust)| Zastąp ciąg {dzierżawca} IDENTYFIKATORem dzierżawy.<p>https:\//STS.Windows.NET/{tenant-ID}/ |
| **Metadane federacji dostawcy tożsamości**<p>Lokalizacja publicznie dostępnych metadanych Federacji dostawcy tożsamości. (Niektóre aplikacje używają metadanych federacji jako alternatywy sytuacji, w której administrator indywidualnie konfiguruje adresy URL, identyfikator i certyfikat podpisywania tokenu).| Znajdź adres URL metadanych Federacji AD FS w obszarze Zarządzanie AD FS w obszarze **punkty końcowe > usługi > typ > metadanych: metadane federacji**. Na przykład:[https://fs.contoso.com/FederationMetadata/2007-06/FederationMetadata.xml](https://fs.contoso.com/FederationMetadata/2007-06/FederationMetadata.xml)| Odpowiadająca wartość w usłudze Azure AD jest zgodna [https://login.microsoftonline.com/{TenantDomainName}/FederationMetadata/2007-06/FederationMetadata.xml](https://login.microsoftonline.com/{TenantDomainName}/FederationMetadata/2007-06/FederationMetadata.xml)z wzorcem. Zastąp ciąg {Nazwadomenydzierżawy} nazwą swojej dzierżawy w formacie "contoso.onmicrosoft.com".   <p>Aby uzyskać więcej informacji, zobacz [Metadane federacji](https://docs.microsoft.com/azure/active-directory/azuread-dev/azure-ad-federation-metadata). |
...
Przykład sposobu konfigurowania opcji wykluczania dla zaufanych lokalizacji w witrynie Azure Portal:
...
‎ |
| Logowanie jednokrotne oparte na hasłach| Pobierz i zainstaluj[rozszerzenie](https://docs.microsoft.com/azure/active-directory/user-help/active-directory-saas-access-panel-introduction)" [bezpieczne logowanie](https://docs.microsoft.com/azure/active-directory/user-help/active-directory-saas-access-panel-introduction)[-](https://docs.microsoft.com/azure/active-directory/user-help/active-directory-saas-access-panel-introduction)do aplikacji". To rozszerzenie ułatwia rozpoczęcie pracy z aplikacjami w chmurze w organizacji, które wymagają korzystania z procesu rejestracji jednokrotnej.  
‎ |
