...
|[Właściwości serviceprincipals](https://docs.microsoft.com/graph/api/resources/serviceprincipal?view=graph-rest-1.0)|[Aktualizuj obiekt serviceprincipal](https://docs.microsoft.com/graph/api/serviceprincipal-update?view=graph-rest-1.0&tabs=http) <br> [Utwórz appRoleAssignments](https://docs.microsoft.com/graph/api/serviceprincipal-post-approleassignments?view=graph-rest-1.0&tabs=http) <br> [Przypisz claimsMappingPolicies](https://docs.microsoft.com/graph/api/serviceprincipal-post-claimsmappingpolicies?view=graph-rest-beta&tabs=http)|
|[zastosowania](https://docs.microsoft.com/graph/api/resources/application?view=graph-rest-1.0)|[Aktualizuj aplikację](https://docs.microsoft.com/graph/api/application-update?view=graph-rest-1.0&tabs=http)|
...
> [!NOTE]
> Poczekaj chwilę, aż aplikacja zostanie zainicjowana w dzierżawie usługi Azure AD. Nie jest to błyskawiczne. Jedną z strategii jest wykonanie zapytania GET w obiekcie głównym aplikacji/usługi co 5-10 sekund do momentu pomyślnego wykonania zapytania.
...
W tym przykładzie ustawisz `saml` jako tryb logowania jednokrotnego w [typie zasobu serviceprincipal](https://docs.microsoft.com/graph/api/resources/serviceprincipal?view=graph-rest-1.0). Inne właściwości logowania jednokrotnego SAML, które można skonfigurować, to: `notificationEmailAddresses` , `loginUrl` i`samlSingleSignOnSettings.relayState`

Zanim to zapytanie będzie działało, musisz wyrazić zgodę na karcie **Modyfikowanie uprawnień** w Eksploratorze grafu. Upewnij się również, że używasz identyfikatora **podmiotu zabezpieczeń** uzyskanym wcześniej.
...
Upewnij się, że używasz pozyskanego wcześniej identyfikatora **aplikacji** .
...
Aby uzyskać więcej informacji, zobacz [Dostosowywanie oświadczeń emitowanych w tokenie](https://docs.microsoft.com/azure/active-directory/develop/active-directory-claims-mapping).

> [!NOTE]
> W niektórych kluczach w zasadzie mapowania oświadczeń jest uwzględniana wielkość liter (np. "wersja"). Jeśli zostanie wyświetlony komunikat o błędzie, taki jak "właściwość ma nieprawidłową wartość", może to być problem z rozróżnianiem wielkości liter.
...
Aby przetestować, możesz użyć następującego polecenia programu PowerShell, aby uzyskać certyfikat z podpisem własnym. Następnie konieczne będzie manipulowanie i ściąganie potrzebnych wartości ręcznie przy użyciu innych narzędzi. Aby utworzyć certyfikat podpisywania dla środowiska produkcyjnego, należy użyć najlepszych rozwiązań w zakresie zabezpieczeń firmy.
...
Alternatywnie można użyć następującej aplikacji konsolowej w języku C# jako dowodu koncepcji, aby zrozumieć, jak można uzyskać wymagane wartości. Należy zauważyć, że ten kod jest przeznaczony **wyłącznie do uczenia i referencyjnego** i nie powinien być używany jako — w środowisku produkcyjnym.
...
Upewnij się, że keyId dla poświadczenia dla poświadczeń użyty dla "Sign" pasuje do keyId passwordCredential. Można wygenerować, pobierając `customkeyIdentifier` skrót odcisku palca certyfikatu. Zobacz kod odwołania C# powyżej.

#### <a name="request"></a>Żądanie

> [!NOTE]
> Wartość "klucz" we właściwości poświadczenia klucza jest skracana na potrzeby odczytywania. Wartość jest zakodowana przy użyciu Base 64. Dla klucza prywatnego właściwość ma wartość `usage` "Sign". Dla klucza publicznego Właściwość `usage` "verify".
...
Aby uzyskać więcej informacji, zobacz [appRoleAssignment](https://docs.microsoft.com/graph/api/resources/approleassignment?view=graph-rest-1.0) typu zasobu.
