...
Ta linia bazowa zabezpieczeń ma zastosowanie do wskazówek dotyczących usługi [Azure Security test w wersji 1,0](../security/benchmarks/overview.md) do punktu kontrolnego platformy Azure. Test porównawczy zabezpieczeń platformy Azure zawiera zalecenia dotyczące sposobu zabezpieczania rozwiązań w chmurze na platformie Azure. Zawartość jest pogrupowana według **kontroli zabezpieczeń** zdefiniowanej przez program Azure Security test i powiązane wskazówki dotyczące platformy Azure. Nie zostały wykluczone **kontrolki dotyczące kontroli** wskaźnikowej platformy Azure. Aby dowiedzieć się, jak punkt kontrolny platformy Azure całkowicie jest mapowany na test porównawczy zabezpieczeń platformy Azure, zapoznaj się z [pełnym plikiem mapowania linii bazowej zabezpieczeń Azure](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Wskazówki**: przechowywanie spisu kont użytkowników, które mają dostęp administracyjny do płaszczyzny kontroli (np. Azure Portal) obszaru roboczego wskaźnikowego platformy Azure. 

Za pomocą okienka tożsamość i kontrola dostępu (IAM) w Azure Portal subskrypcji możesz skonfigurować kontrolę dostępu opartą na rolach (Azure RBAC). Role są stosowane do użytkowników, grup, podmiotów usługi i tożsamości zarządzanych w Active Directory.  Za pomocą funkcji kontroli dostępu platformy Azure można także zapewnić wbudowane role administracyjne, takie jak współautor wskaźnikowego platformy Azure, które mogą być przypisane do użytkowników, grup i usług na platformie Azure. 
...
**Wskazówki**: implementowanie osobnych subskrypcji i/lub grup zarządzania na potrzeby tworzenia, testowania i produkcyjnego obszaru roboczego.
...
- [Azure Monitor — Omówienie klucza zarządzanego przez klienta](../azure-monitor/platform/customer-managed-keys.md#customer-managed-key-cmk-overview)
...
**Wskazówki**: Użyj wbudowanych definicji Azure Policy, a także aliasów Azure Policy w przestrzeni nazw "Microsoft. OperationalInsights", aby utworzyć zasady niestandardowe na potrzeby alertów, inspekcji i wymuszania konfiguracji systemu. Użyj zasad platformy Azure [Audit], [Odmów] i [Wdróż, jeśli nie istnieje], aby automatycznie wymuszać konfiguracje dla zasobów platformy Azure.
...
- [Jak przesłać strumieniowo alerty do usługi Azure wskaźnikowego](connect-azure-security-center.md)
