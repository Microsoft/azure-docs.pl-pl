---
title: Private Link — interfejs wiersza polecenia platformy Azure — Azure Database for MariaDB
description: Dowiedz się, jak skonfigurować link prywatny dla usługi Azure Database for MariaDB interfejsie wiersza polecenia platformy Azure
author: mksuni
ms.author: sumuth
ms.service: mariadb
ms.topic: how-to
...
ms.locfileid: "107790551"
---
# <a name="create-and-manage-private-link-for-azure-database-for-mariadb-using-cli"></a>Tworzenie aplikacji i zarządzanie nimi Private Link dla Azure Database for MariaDB przy użyciu interfejsu wiersza polecenia

Prywatny punkt końcowy to podstawowy blok blokowy linku prywatnego na platformie Azure. Dzięki temu zasoby platformy Azure, takie Virtual Machines (maszyny wirtualne), mogą komunikować się prywatnie z zasobami linków prywatnych. W tym artykule dowiesz się, jak za pomocą interfejsu wiersza polecenia platformy Azure utworzyć maszynę wirtualną w usłudze Azure Virtual Network i serwer Azure Database for MariaDB z prywatnym punktem końcowym platformy Azure.

> [!NOTE]
> Funkcja łącza prywatnego jest dostępna tylko dla Azure Database for MariaDB w warstwach cenowych Ogólnego przeznaczenia lub Zoptymalizowane pod kątem pamięci. Upewnij się, że serwer bazy danych znajduje się w jednej z tych warstw cenowych.
...
Utwórz nową Azure Database for MariaDB za pomocą polecenia az mariadb server create. Pamiętaj, że nazwa serwera MariaDB musi być unikatowa na platformie Azure, dlatego zastąp wartość symbolu zastępczego w nawiasach własną unikatową wartością: 
...
> [!NOTE]
> W niektórych przypadkach Azure Database for MariaDB i podsieć sieci wirtualnej znajdują się w różnych subskrypcjach. W takich przypadkach należy zapewnić następujące konfiguracje:
> - Upewnij się, że w obu subskrypcjach zarejestrowano dostawcę zasobów **Microsoft.DBforMariaDB.** Aby uzyskać więcej informacji, [zobacz rejestracja menedżera zasobów][resource-manager-portal]
...
Utwórz strefę Prywatna strefa DNS dla domeny serwera Usługi Active Directory i utwórz link skojarzenia z Virtual Network. 
```azurecli-interactive
...
1. Wybierz pozycję **Pobierz plik RDP**. Platforma Azure utworzy Remote Desktop Protocol *(rdp)* i pobierze go na komputer.
...
## <a name="access-the-mariadb-server-privately-from-the-vm"></a>Prywatny dostęp do serwera MariaDB z maszyny wirtualnej
...
3. Przetestuj połączenie łącza prywatnego dla serwera MariaDB przy użyciu dowolnego dostępnego klienta. W poniższym przykładzie do tej operacji został użyty program [MySQL Workbench.](https://dev.mysql.com/doc/workbench/en/wb-installing-windows.html)
...
4. W **nowe połączenie**, wprowadź lub wybierz te informacje:
    ...
    | Nazwa użytkownika | Wprowadź nazwę użytkownika *username@servername* podaną podczas tworzenia serwera MariaDB. |
    | Hasło | Wprowadź hasło podane podczas tworzenia serwera MariaDB. |
    ||

5. Wybierz **pozycję Testuj połączenie** lub **OK.**

6. (Opcjonalnie) Przeglądanie baz danych z menu po lewej stronie i tworzenie informacji lub wykonywanie zapytań z bazy danych MariaDB

8. Zamknij połączenie pulpitu zdalnego z myVm.

## <a name="clean-up-resources"></a>Czyszczenie zasobów 
Gdy grupa zasobów i wszystkie jej zasoby nie będą już potrzebne, możesz je usunąć za pomocą funkcji az group delete: 
