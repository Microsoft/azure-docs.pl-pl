...
ms.locfileid: "107313644"
---
# <a name="routes-in-azure-static-web-apps-preview"></a>Trasy w usłudze Azure static Web Apps Preview

> [!IMPORTANT]
> Funkcje zdefiniowane w _routes.jsw_ pliku są obecnie przestarzałe i lepiej zaimplementowane w [pliku konfiguracyjnym](./configuration.md#routes)Web Apps statycznym platformy Azure.

Routing w usłudze Azure static Web Apps definiuje reguły routingu zaplecza i zachowanie autoryzacji dla zawartości statycznej i interfejsów API<sup>1</sup>. Reguły są definiowane jako tablica reguł w _staticwebapp.config.js_ pliku.

- _staticwebapp.config.js_ pliku musi znajdować się w folderze głównym folderu artefaktów kompilacji aplikacji.
...
- Role są zdefiniowane w _staticwebapp.config.jsw_ pliku i użytkownicy są skojarzeni z rolami za pośrednictwem [zaproszeń](authentication-authorization.md).
...
_staticwebapp.config.js_ pliku musi znajdować się w folderze głównym folderu artefaktów kompilacji aplikacji. Jeśli aplikacja sieci Web zawiera krok kompilacji, który kopiuje skompilowane pliki z określonego folderu do folderu artefaktów kompilacji, wówczas _staticwebapp.config.jsw_ pliku musi istnieć w tym konkretnym folderze.

W poniższej tabeli wymieniono odpowiednie lokalizacje, w których należy umieścić _staticwebapp.config.jsw_ pliku dla wielu platform i bibliotek frontonu.
...
Trasy są zdefiniowane w _staticwebapp.config.jsw_ pliku jako tablica reguł tras we `routes` właściwości. Każda reguła składa się ze wzorca trasy wraz z co najmniej jedną opcjonalną właściwością reguły. Zobacz [przykładowy plik tras](#example-route-file) dla przykładów użycia.
...
Użytkownicy mogą napotkać różne sytuacje, które mogą spowodować wystąpienie błędu. Korzystając z `platformErrorOverrides` tablicy, można zapewnić niestandardowe środowisko w odpowiedzi na te błędy. Zapoznaj się z [przykładowym plikiem trasy](#example-route-file) do umieszczania tablicy w _staticwebapp.config.js_ pliku.
...
- Nagłówki zdefiniowane w _staticwebapp.config.js_ dotyczą tylko zawartości statycznej. Można dostosować nagłówki odpowiedzi punktu końcowego interfejsu API w kodzie funkcji.
...
Poniższy przykład pokazuje, jak utworzyć reguły tras dla zawartości statycznej i interfejsów API w _staticwebapp.config.js_ pliku. Niektóre trasy używają [folderu systemowego _/.auth_](authentication-authorization.md) , który uzyskuje dostęp do punktów końcowych powiązanych z uwierzytelnianiem.
...
| _/customers/contoso_               | Uwierzytelnieni użytkownicy, którzy należą do ról _administratorzy_ lub _customers_contoso_ , są obsługiwani _/Customers/contoso/index.html_ pliku <sup>2</sup>. Uwierzytelnieni użytkownicy, którzy nie znajdują się w rolach _administratorzy_ lub _customers_contoso_ , mają błąd 401. Nieuwierzytelnieni użytkownicy przekierowani do programu _/login_. |
...
- _staticwebapp.config.jsw_ pliku nie może być większy niż 100 KB
- _staticwebapp.config.jsw_ pliku obsługuje maksymalnie 50 odrębnych ról
