...
Aby włączyć Logowanie użytkowników przy użyciu konta usługi WeChat w Azure Active Directory B2C (Azure AD B2C), musisz utworzyć aplikację w [centrum zarządzania WeChat](https://open.weixin.qq.com/). Jeśli nie masz jeszcze konta WeChat, możesz uzyskać informacje o [https://kf.qq.com](https://kf.qq.com/faq/161220Brem2Q161220uUjERB.html) .
...
7. Wprowadź **nazwę** klucza zasad. Na przykład `WeChatSecret`. Prefiks `B2C_1A_` jest automatycznie dodawany do nazwy klucza.
...
Jeśli chcesz, aby użytkownicy mogli się logować przy użyciu konta usługi WeChat, musisz zdefiniować konto jako dostawcę oświadczeń, z którym Azure AD B2C może komunikować się za pośrednictwem punktu końcowego. Punkt końcowy zawiera zestaw oświadczeń, które są używane przez Azure AD B2C do sprawdzenia, czy określony użytkownik został uwierzytelniony.

Konto WeChat można zdefiniować jako dostawcę oświadczeń przez dodanie go do elementu **ClaimsProviders** w pliku rozszerzenia zasad.
...
Teraz zasady zostały skonfigurowane tak, aby Azure AD B2C wie, jak komunikować się z kontem WeChat. Spróbuj przekazać plik rozszerzenia zasad tylko, aby upewnić się, że nie ma żadnych problemów do tej pory.
...
W tym momencie dostawca tożsamości został skonfigurowany, ale nie jest dostępny na żadnym z ekranów rejestracji/logowania. Aby można było go udostępnić, należy utworzyć duplikat istniejącej podróży użytkownika szablonu, a następnie zmodyfikować go tak, aby miał także dostawcę tożsamości WeChat.
...
5. Zmień nazwę identyfikatora podróży użytkownika. Na przykład `SignUpSignInWeChat`.

### <a name="display-the-button"></a>Wyświetl przycisk

Element **ClaimsProviderSelection** jest analogiczny do przycisku dostawcy tożsamości na ekranie rejestracji/logowania. Jeśli dodasz element **ClaimsProviderSelection** dla konta WeChat, nowy przycisk będzie wyświetlany, gdy użytkownik zostanie wystawiony na stronie.

1. Znajdź element **OrchestrationStep** , który obejmuje `Order="1"` w podróży użytkownika.
2. W obszarze **ClaimsProviderSelects** Dodaj następujący element. Ustaw wartość **TargetClaimsExchangeId** na odpowiednią wartość, na przykład `WeChatExchange` :
...
Teraz, gdy masz już przycisk, musisz połączyć go z akcją. W tym przypadku akcja w tym przypadku Azure AD B2C do komunikowania się z kontem WeChat w celu uzyskania tokenu.
...
2. Dodaj następujący element **ClaimsExchange** , aby upewnić się, że używasz tej samej wartości dla identyfikatora, który został użyty dla **TargetClaimsExchangeId**:
    ...
    Zaktualizuj wartość **TechnicalProfileReferenceId** na identyfikator utworzonego wcześniej profilu technicznego. Na przykład `WeChat-OAuth`.
...
## <a name="add-wechat-identity-provider-to-a-user-flow"></a>Dodawanie dostawcy tożsamości WeChat do przepływu użytkownika 

1. W dzierżawie Azure AD B2C wybierz pozycję **przepływy użytkownika**.
1. Kliknij przepływ użytkownika, do którego chcesz dodać dostawcę tożsamości WeChat.
1. W obszarze **dostawcy tożsamości społecznościowej** wybierz pozycję **WeChat**.
1. Wybierz pozycję **Zapisz**.
...
1. Utwórz kopię *SignUpOrSignIn.xml* w katalogu roboczym i zmień jej nazwę. Na przykład zmień nazwę na *SignUpSignInWeChat.xml*.
1. Otwórz nowy plik i zaktualizuj wartość atrybutu **PolicyId** dla **TrustFrameworkPolicy** przy użyciu unikatowej wartości. Na przykład `SignUpSignInWeChat`.
1. Zaktualizuj wartość **PublicPolicyUri** za pomocą identyfikatora URI dla zasad. Na przykład`http://contoso.com/B2C_1A_signup_signin_WeChat`
1. Zaktualizuj wartość atrybutu **ReferenceId** w **DefaultUserJourney** w taki sposób, aby odpowiadała identyfikatorowi nowej podróży użytkownika, która została utworzona (SignUpSignWeChat).
...
1. Wybierz pozycję **Uruchom teraz** i wybierz pozycję WeChat, aby zalogować się za pomocą WeChat i przetestować zasady niestandardowe.
