---
title: Przegląd zasad niestandardowych Azure Active Directory B2C | Microsoft Docs
...
[Pakiet startowy](tutorial-create-user-flows.md?pivots=b2c-custom-policy#get-the-starter-pack) Azure AD B2C zasad niestandardowych zawiera kilka wstępnie utworzonych zasad pozwalających szybko rozpocząć pracę. Każdy z tych pakietów początkowych zawiera najmniejszą liczbę profilów technicznych i podróży użytkowników, które są konieczne do osiągnięcia opisanych scenariuszy:
...
![Diagram przedstawiający przepływ wykonywania zasad](./media/custom-policy-overview/custom-policy-execution.png)
...
![dostosowana podróż użytkownika](media/custom-policy-overview/user-journey-flow.png)
...
![Azure AD B2C podróży użytkownika](media/custom-policy-overview/user-journey-diagram.png)
...
![Diagram profilu technicznego weryfikacji](media/custom-policy-overview/validation-technical-profile.png)
...
![Diagram przedstawiający model dziedziczenia zasad struktury zaufania](media/custom-policy-overview/policies.png)
...
1. Dodaj niezbędne [klucze zasad](tutorial-create-user-flows.md?pivots=b2c-custom-policy#add-signing-and-encryption-keys) i [zarejestruj aplikacje struktury obsługi tożsamości](tutorial-create-user-flows.md?pivots=b2c-custom-policy#register-identity-experience-framework-applications).
1. [Pobierz pakiet startowy zasad Azure AD B2C](tutorial-create-user-flows.md?pivots=b2c-custom-policy#get-the-starter-pack) i przekaż go do swojej dzierżawy. 
1. Po przekazaniu pakietu początkowego [Sprawdź zasady rejestracji lub logowania](tutorial-create-user-flows.md?pivots=b2c-custom-policy#test-the-custom-policy).
