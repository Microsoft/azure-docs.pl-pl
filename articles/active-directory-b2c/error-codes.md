...
| `AADB2C90067` | Identyfikator URI przekierowania wylogowania{0}"" ma nieprawidłowy format. Określ adres URL oparty na protokole httpshttps://example.com/return, taki jak "", lub dla klientów natywnych Użyj NATYWNEGO identyfikatora URI "urn: IETF klienta" (IETF: 2.0: OOB). |
...
| `AADB2C90075` | W{0}kroku "{1}" określono wymianę oświadczeń "", która zwróciła odpowiedź na błąd http{2}z kodem ""{3}i przyczyną "". |
...
| `AADB2C90080` | Podany przydział wygasł. Uwierzytelnij ponownie i spróbuj ponownie. Bieżący czas: {0}, udziel czasu wystawienia {1}:, przyznaj czas wygaśnięcia okna {2}przewijania:. |
...
| `AADB2C90120` | Maksymalny parametr wieku "{0}" określony w żądaniu jest nieprawidłowy. Maksymalny wiek musi być liczbą całkowitą z przedziału{1}od{2}"" do "" włącznie. |
| `AADB2C90122` | Nie powiodło{0}się sprawdzenie poprawności żądania HTTP dla danych wejściowych "" odebranych w żądaniu. Upewnij się, że dane wejściowe nie zawierają znaków takich jak < lub &. |
...
| `AADB2C90149` | Nie można{0}załadować skryptu "". |
...
| `AADB2C90165` | Nie można odnaleźć komunikatu inicjującego SAML{0}o identyfikatorze "" w stanie. |
...
| `AADB2C90277` | Krok aranżacji "{0}" podróży{1}użytkownika "" zasad "{2}" nie zawiera odwołania do definicji zawartości. |
...
| `AADB2C90288` | UserJourney z identyfikatorem{0}"", do którego{1}odwołuje się element profilu technicznym "" dla{2}dzierżawy tokenów odświeżania ""{3}nie istnieje w zasadach "" ani w żadnej z jej zasad podstawowych. |
...
| `AADB2C99006` | Azure AD B2C nie może znaleźć aplikacji rozszerzenia o identyfikatorze aplikacji{0}"". Aby uzyskać https://go.microsoft.com/fwlink/?linkid=851224 więcej informacji, odwiedź stronę. |
...
| `AADB2C99015` | W profilu{0}"" w ramach{1}zasad "" w{2}dzierżawie "" brakuje wszystkich InputClaims wymaganych dla przepływu poświadczeń hasła właściciela zasobu. |
