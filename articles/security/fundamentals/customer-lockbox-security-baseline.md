---
title: Podstawowa usługa Azure Security dla Skrytka klienta dla Microsoft Azure
description: Podstawowa usługa Azure Security dla Skrytka klienta dla Microsoft Azure
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/05/2020
...
Podstawowa usługa Azure Security dla Skrytka klienta dla Microsoft Azure zawiera zalecenia, które pomogą ulepszyć stan bezpieczeństwa wdrożenia.

Punkt odniesienia dla tej usługi jest rysowany w [wersji 1,0 usługi Azure Security test](../benchmarks/overview.md), która zawiera zalecenia dotyczące sposobu zabezpieczania rozwiązań w chmurze na platformie Azure z naszymi najlepszymi wskazówkami.

Aby uzyskać więcej informacji, zobacz [podstawy zabezpieczeń platformy Azure — omówienie](../benchmarks/security-baselines-overview.md).

## <a name="network-security"></a>Bezpieczeństwo sieci

*Aby uzyskać więcej informacji, zobacz [Kontrola zabezpieczeń: zabezpieczenia sieci](../benchmarks/security-control-network-security.md).*
...
**Wskazówki**: nie dotyczy; nie można skojarzyć sieci wirtualnej, podsieci ani sieciowej grupy zabezpieczeń z Skrytka klienta.
...
**Wskazówki**: nie dotyczy; nie można skojarzyć sieci wirtualnej, podsieci ani sieciowej grupy zabezpieczeń z usługą Azure Skrytka klienta.
...
**Wskazówki**: nie dotyczy; nie można skojarzyć sieci wirtualnej, podsieci ani sieciowej grupy zabezpieczeń z Skrytka klienta. Brak konfiguracji sieci do wypróbowania lub monitorowania.
...
**Wskazówki**: nie dotyczy; Firma Microsoft utrzymuje źródło czasu używane do zasobów, takich jak Skrytka klienta.
...
**Wskazówki**: dzienniki inspekcji dla skrytka klienta są automatycznie włączane i utrzymywane w dziennikach aktywności platformy Azure. Te dane można wyświetlić, przesyłając je strumieniowo z dziennika aktywności platformy Azure do obszaru roboczego usługi log Analytics, w którym można następnie wykonywać badania i analizy.

Dołącz dzienniki aktywności wygenerowane przez Skrytka klienta do centrum kontrolnego platformy Azure lub innego SIEM, aby włączyć centralne agregację dzienników i zarządzanie nimi.

* [Dzienniki inspekcji dla Skrytka klienta](./customer-lockbox-overview.md#auditing-logs)
...
**Wskazówki**: w Azure monitor ustawić okres przechowywania dziennika dla log Analytics obszarów roboczych skojarzonych z skrytka klienta zgodnie z regulacjami zgodności w organizacji.
...
**Wskazówki**: dzienniki inspekcji dla skrytka klienta są automatycznie włączane i utrzymywane w dziennikach aktywności platformy Azure. Te dane można wyświetlić, przesyłając je strumieniowo z dziennika aktywności platformy Azure do obszaru roboczego usługi log Analytics, w którym można następnie wykonywać badania i analizy. Analizowanie i monitorowanie dzienników z Skrytka klientach żądań w celu zachowania nietypowego działania. Użyj sekcji "Logs" w obszarze roboczym Azure wskaźnikowym, aby wykonywać zapytania lub tworzyć alerty na podstawie dzienników Skrytka klienta.

* [Inspekcja dzienników w Skrytka klienta](./customer-lockbox-overview.md#auditing-logs)
...
**Wskazówki**: nie dotyczy; Skrytka klienta nie przetwarza ani nie tworzy dzienników związanych z oprogramowaniem chroniącym przed złośliwym kodem.
...
**Wskazówki**: nie dotyczy; Skrytka klienta nie przetwarza ani nie tworzy dzienników związanych z usługą DNS.
...
* [Informacje o uprawnieniach żądania dostępu w Skrytka klienta](./customer-lockbox-overview.md)
...
**Wskazówki**: nie dotyczy; dostęp do Skrytka klienta odbywa się za pomocą Azure Portal i zarezerwowanych dla kont z rolą dzierżawy właściciela. Logowanie jednokrotne nie jest obsługiwane.
...
**Wskazówki**: Aby zalogować się do i skonfigurować żądania skrytka klienta, użyj stacji roboczej z dostępem uprzywilejowanym (dostępem uprzywilejowanym) z włączoną usługą Azure AD Multi-Factor Authentication (MFA).
...
**Wskazówki**: Użyj Azure Active Directory PRIVILEGED Identity Management (PIM) do generowania dzienników i alertów w przypadku wystąpienia podejrzanych lub niebezpiecznych działań w środowisku.

Ponadto do wyświetlania alertów i raportów na temat ryzykownego zachowania użytkownika służy Azure Active Directory wykrywanie ryzyka.
...
**Wskazówki**: w razie potrzeby użyj Azure Active Directory jako centralnego systemu uwierzytelniania i autoryzacji. Azure Active Directory chroni dane przy użyciu silnego szyfrowania danych przechowywanych i podczas przesyłania. Azure Active Directory również Sole, skróty i bezpieczne przechowywanie poświadczeń użytkownika.

* [Jak utworzyć i skonfigurować wystąpienie Azure Active Directory](../../active-directory/fundamentals/active-directory-access-create-new-tenant.md)
...
* [Informacje o raportowaniu Azure Active Directory](../../active-directory/reports-monitoring/index.yml)

* [Jak korzystać z przeglądów dostępu Azure Active Directory](../../active-directory/governance/access-reviews-overview.md)
...
Masz dostęp do Azure Active Directory źródeł działań związanych z logowaniem, inspekcją i ryzykiem, które umożliwiają integrację z platformą Azure, lub SIEM innej firmy.

Proces ten można usprawnić, tworząc ustawienia diagnostyczne dla Azure Active Directory kont użytkowników i wysyłając dzienniki inspekcji i dzienniki logowania do obszaru roboczego Log Analytics. Żądane alerty dzienników można skonfigurować w ramach Log Analytics.
...
**Wskazówki**: w celu odróżnienia zachowania logowania do konta na płaszczyźnie kontroli (np. Azure Portal) użyj funkcji Azure Active Directory ochrony tożsamości i wykrywania ryzyka, aby skonfigurować automatyczne odpowiedzi na wykryte podejrzane działania związane z tożsamościami użytkowników. Możesz również pozyskać dane do usługi Azure wskaźnikowej na potrzeby dalszej analizy.

* [Jak wyświetlić Azure Active Directory ryzykowne logowanie](../../active-directory/identity-protection/overview-identity-protection.md)
...
**Wskazówki**: to zalecenie nie ma zastosowania do skrytka klienta.
...
**Wskazówki**: to zalecenie nie dotyczy; Tagi nie są obsługiwane dla Skrytka klienta.
...
**Wskazówki**: nie dotyczy; Skrytka klienta zostanie zainicjowana w tej samej subskrypcji co zasoby, do których udzielany jest dostęp. Brak publicznego punktu końcowego do ochrony lub izolowania. Skrytka klienta dostęp do żądania jest udzielany użytkownikowi, który posiada rolę właściciela na poziomie dzierżawy.

* [Informacje o przepływie pracy Skrytka klienta](./customer-lockbox-overview.md)
...
**Wskazówki**: Firma Microsoft zarządza podstawową infrastrukturą dla skrytka klienta i ma zaimplementowane ścisłe kontrole, aby zapobiec utracie lub narażeniu danych klientów.

* [Informacje na temat ochrony danych klientów na platformie Azure](./protection-customer-data.md)
...
**Wskazówki**: nie dotyczy; Skrytka klienta nie przechowuje żadnych danych klienta.
...
**Wskazówki**: zatwierdzenie żądania skrytka klienta użytkownikowi, który posiada rolę właściciela na poziomie dzierżawy.
...
**Wskazówki**: nie dotyczy; to zalecenie jest przeznaczone do zasobów obliczeniowych. Firma Microsoft zarządza podstawową infrastrukturą dla Skrytka klienta i ma zaimplementowane ścisłe kontrole, aby zapobiec utracie lub narażeniu danych klientów.
...
**Wskazówki**: nie dotyczy; Skrytka klienta samo nie przechowuje danych klienta.
...
**Wskazówki**: dzienniki inspekcji dla skrytka klienta są automatycznie włączane i utrzymywane w dziennikach aktywności platformy Azure. Użyj dziennika aktywności platformy Azure do monitorowania i wykrywania zmian zasobów usługi Azure Skrytka klienta. Utwórz alerty w Azure Monitor, które będą wyzwalane po wprowadzeniu zmian w zasobach krytycznych.

* [Jak włączyć inspekcję w Skrytka klienta](./customer-lockbox-overview.md)
...
*Aby uzyskać więcej informacji, zobacz [Kontrola zabezpieczeń: Zarządzanie lukami w zabezpieczeniach](../benchmarks/security-control-vulnerability-management.md).*

### <a name="51-run-automated-vulnerability-scanning-tools"></a>5,1: uruchamianie narzędzi do skanowania automatycznych luk w zabezpieczeniach

**Wskazówki**: nie dotyczy; Firma Microsoft przeprowadza zarządzanie lukami w systemach podstawowych, które obsługują Skrytka klienta.
...
Mimo że klasyczne zasoby platformy Azure mogą zostać odnalezione za pośrednictwem usługi Azure Resource Graph, zdecydowanie zaleca się tworzenie i używanie zasobów Azure Resource Manager.

* [Jak tworzyć zapytania przy użyciu grafu zasobów platformy Azure](../../governance/resource-graph/first-query-portal.md)

* [Jak wyświetlić subskrypcje platformy Azure](/powershell/module/az.accounts/get-azsubscription)

* [Omówienie kontroli dostępu opartej na rolach (Azure RBAC)](../../role-based-access-control/overview.md)
...
**Wskazówki**: Tagi nie są obsługiwane dla skrytka klienta.
...
**Wskazówki**: nie dotyczy, skrytka klienta nie ma konfigurowalnych ustawień zabezpieczeń.
...
**Wskazówki**: nie dotyczy; dostęp do żądań Skrytka klienta są ograniczone do właściciela subskrypcji platformy Azure, która przechowuje zasób. Brak haseł, wpisów tajnych lub kluczy wymaganych do uzyskania dostępu Skrytka klienta poza logowaniem jako właściciel dzierżawy.
...
**Wskazówki**: nie dotyczy; Skrytka klienta nie korzysta z zarządzanych tożsamości.
...
**Wskazówki**: nie dotyczy; te wytyczne są przeznaczone dla zasobów obliczeniowych. Oprogramowanie chroniące przed złośliwym oprogramowaniem firmy Microsoft jest włączone na podstawowym hoście obsługującym rozwiązanie Skrytka klienta.
...
**Wskazówki**: oprogramowanie chroniące przed złośliwym oprogramowaniem firmy Microsoft jest włączone na podstawowym hoście obsługującym usługi platformy Azure, takie jak skrytka klienta.
...
*Aby uzyskać więcej informacji, zobacz [Kontrola zabezpieczeń: odzyskiwanie danych](../benchmarks/security-control-data-recovery.md).*
...
**Wskazówki**: nie dotyczy; Skrytka klienta samo nie przechowuje danych klienta, ale w celu uzyskania dostępu nie są również używane klucze ani hasła.
