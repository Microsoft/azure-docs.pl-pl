...
1. W przeglądarcehttp://events.wingtip-dpt.&ltsieci Web otwórz centrum zdarzeń biletów Wingtip (; user&gt;. trafficmanager.NET — Zamień &lt;użytkownika&gt; na wartość użytkownika wdrożenia).
...
1. W _ISE programu PowerShell_Otwórz plik. ..\Learning Modules\UserConfig.PSM1. `<resourcegroup>` Zastąp `<user>` wartości i w wierszach 10 i 11 wartością używaną podczas wdrażania aplikacji.  Zapisz plik!
...
1. Natychmiast po odzyskaniu bazy danych wykazu Odśwież centrum zdarzeń biletów Wingtip w przeglądarce internetowej.
   ...
   * Jeśli otworzysz bezpośrednio stronę zdarzeń dzierżawcy w trybie offline, zostanie wyświetlone powiadomienie "dzierżawa w trybie offline". Jeśli na przykład firma Contoso jest w trybie offline, spróbuj otworzyć http://events.wingtip-dpt.&ltstronę "user&gt;. Trafficmanager.NET/contosoconcerthall ![contoso w trybie offline](media/saas-dbpertenant-dr-geo-replication/dr-in-progress-offline-contosoconcerthall.png) 
...
4. Odśwież stronę ze zdarzeniami korytarzy whttp://events.wingtip-dpt.&ltfirmie Contoso&gt;(; User. &lt;trafficmanager.NET/contosoconcerthall&gt; — podstawianie użytkownika z wartością użytkownika wdrożenia) i Zauważ, że ostatnie zdarzenie zostało usunięte.
