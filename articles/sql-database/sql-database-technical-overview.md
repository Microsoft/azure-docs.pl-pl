...
- Warstwa usługi do [skalowania](sql-database-service-tier-hyperscale.md) dla bardzo dużych baz danych OLTP oraz możliwość płynnego skalowania magazynu i skalowania w poziomie.    
...
Platforma Azure udostępnia wbudowane narzędzia do monitorowania i [generowania alertów](sql-database-insights-alerts-portal.md) [wydajności](sql-database-performance-guidance.md) połączone z ocenami wydajności, które umożliwiają monitorowanie stanu tysięcy baz danych. Za pomocą tych narzędzi możesz szybko ocenić wpływ skalowania w górę lub w dół na podstawie bieżących lub przewidywanych wymagań dotyczących wydajności. Ponadto SQL Database mogą [emitować metryki i dzienniki zasobów](sql-database-metrics-diag-logging.md) w celu łatwiejszego monitorowania. Usługę SQL Database można skonfigurować do przechowywania danych dotyczących użycia zasobów, pracowników i sesji oraz połączeń z jednym z następujących zasobów platformy Azure:
