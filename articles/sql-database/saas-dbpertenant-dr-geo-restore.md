...
1. W przeglądarcehttp://events.wingtip-dpt.&ltsieci Web otwórz centrum zdarzeń biletów Wingtip (; user&gt;. trafficmanager.NET, Zastąp &lt;użytkownika&gt; swoją wartością użytkownika wdrożenia).
...
1. W ISE programu PowerShell Otwórz plik. ..\Learning Modules\UserConfig.psm1. `<resourcegroup>` Zastąp `<user>` wartości i w wierszach 10 i 11 wartością używaną podczas wdrażania aplikacji. Zapisz plik.
...
4. Odśwież stronę zdarzeń korytarzy w firmie Contosohttp://events.wingtip-dpt.&lt(;&gt;User. trafficmanager.NET/contosoconcerthall) i zwróć uwagę na to, że brakuje zdarzenia poważnie Strauss.
