...
Nazwa | Opis
------- | -------
type | Typ zdarzenia dziennika. Obecnie udostępniamy informacje o łączności z usługą Azure Signal Service. Dostępny `ConnectivityLogs` jest tylko typ
...
Różnica między `ConnectionAborted` i `ConnectionEnded` jest `ConnectionEnded` to oczekiwane rozłączenie, które jest wyzwalane przez klienta lub po stronie serwera. Mimo że `ConnectionAborted` jest to zwykle nieoczekiwane zdarzenie porzucania połączenia, a powód przerwania `message`zostanie podany w.
