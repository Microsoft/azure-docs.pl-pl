...
1. W Jupyter Notebook wklej poniższy kod, a następnie naciśnij klawisze **SHIFT + ENTER**. Kod sprawdza obecność tabel.
    ...
    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-show-tables.png" alt-text="Wyświetlanie tabel na platformie Spark" border="true":::
...
2. Wklej następujący kod do pustej komórki, a następnie naciśnij klawisze **SHIFT + ENTER**. Kod sprawdza dane w tabeli.
    ...
    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-select-limit.png" alt-text="Pokazywanie wierszy z tabeli hvac platformy Spark" border="true":::
...
2. Przejdź na kartę **Narzędzia główne** , aby **uzyskać**  >  **więcej danych..**.
    ...
    :::image type="content" source="./media/apache-spark-use-bi-tools/hdinsight-spark-power-bi-desktop-get-data.png " alt-text="Pobierz dane do Power BI Desktop z usługi HDInsight Apache Spark" border="true":::er = "true":::

3. Wprowadź `Spark` w polu wyszukiwania, wybierz pozycję **Azure HDInsight Spark**, a następnie wybierz pozycję **Połącz**.

    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-import-data-power-bi.png " alt-text="Pobierz dane do Power BI z Apache Spark BI" border="true":::er = "true":::
...
7. Wybierz `hvac` tabelę, poczekaj na wyświetlenie podglądu danych, a następnie wybierz pozycję **Załaduj**.
    ...
    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-select-table.png " alt-text="Nazwa użytkownika klastra Spark i hasło" border="true":::d "Border =" true ":::
...
8. Wizualizuj różnicę między temperaturą docelową i temperaturą rzeczywistą każdego budynku:
    ...
    2. Przeciągnij pole **BuildingID** do obszaru **Oś** i przeciągnij pola **ActualTemp** i **TargetTemp** do obszaru **Wartość**.
        ...
        :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-add-value-columns.png " alt-text="Dodaj kolumny wartości" border="true":::t = "Dodaj kolumny wartości" Border = "true":::

        Diagram wygląda następująco:

        :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-area-graph-sum.png " alt-text="Wykres obszaru sum" border="true":::lt-text = "wykres obszaru sum" Border = "true":::
    ...
    3. Wybierz strzałkę w dół obok pozycji **ActualTemp** i **TragetTemp** w okienku wizualizacje, a następnie wybierz pozycję **średnia** , aby uzyskać średnią rzeczywistą i docelową temperaturę dla każdego budynku.
        ...
        :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-average-of-values.png " alt-text="średnia wartości" border="true":::t = "Średnia wartość" Border = "true":::
        ...
        :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-area-graph.png " alt-text="obszar Graf" border="true":::. png "Alt-text =" Area Graph "Border =" true ":::
...
1. Na karcie **Narzędzia główne** wybierz pozycję **Publikuj**.
    ...
    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-publish.png " alt-text="Publikowanie z Power BI Desktop" border="true"::: Pulpit "Border =" true ":::
...
1. Wybierz obszar roboczy, w którym chcesz opublikować zestaw danych i raport, a następnie wybierz pozycję **Wybierz**. Na poniższej ilustracji domyślnie wybrany jest **Mój obszar roboczy**.
    ...
    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-select-workspace.png " alt-text="Wybierz obszar roboczy, aby opublikować zestaw danych i raport do" border="true":::UE ":::

1. Po pomyślnym opublikowaniu wybierz pozycję **Otwórz plik "BuildingTemperature. pbix" w Power BI**.

    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-publish-success.png " alt-text="Publikowanie powiodło się, kliknij, aby wprowadzić poświadczenia" border="true":::er = "true":::

1. W usługa Power BI wybierz pozycję **Wprowadź poświadczenia**.

    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-enter-credentials.png " alt-text="Wprowadź poświadczenia w usługa Power BI" border="true":::"Border =" true ":::

1. Wybierz pozycję **Edytuj poświadczenia**.

    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-edit-credentials.png " alt-text="Edytuj poświadczenia w usługa Power BI" border="true":::e "Border =" true ":::
...
1. Wprowadź informacje o koncie logowania usługi HDInsight, a następnie wybierz pozycję **Zaloguj się**. Domyślna nazwa konta to *admin*.
    ...
    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-sign-in.png " alt-text="Logowanie do klastra Spark" border="true"::: Klaster Spark "Border =" true ":::
...
1. W lewym okienku przejdź do **obszaru obszary robocze**  >  **raporty moje obszary robocze**  >  , a następnie wybierz pozycję **BuildingTemperature**.
    ...
    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-service-left-pane.png " alt-text="Raport wyświetlany w obszarze raporty w lewej kolejności okienka" border="true":::= "true":::
...
1. Umieść kursor na wizualizacji, a następnie wybierz ikonę pinezki w prawym górnym rogu.
    ...
    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-service-report.png " alt-text="Raport w usłudze Usługa Power BI" border="true":::"Border =" true ":::

1. Wybierz pozycję "nowy pulpit nawigacyjny", wprowadź nazwę `Building temperature` , a następnie wybierz pozycję **Przypnij**.

    :::image type="content" source="./media/apache-spark-use-bi-tools/apache-spark-bi-pin-dashboard.png " alt-text="Przypnij do nowego pulpitu nawigacyjnego" border="true"::: do nowego pulpitu nawigacyjnego "Border =" true ":::
