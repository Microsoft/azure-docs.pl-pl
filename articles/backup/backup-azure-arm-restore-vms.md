...
Azure Backup zapewnia kilka sposobów przywracania maszyny wirtualnej.
...
- **Wymiana dysku:** podczas wymiany dysku na istniejącej maszynie wirtualnej program Azure Backup migawkę istniejącej maszyny wirtualnej przed zastąpieniem dysku. Migawka jest również kopiowana do magazynu usługi Recovery Services za pośrednictwem transferu danych w ramach procesu w tle. Jednak po zakończeniu fazy migawki zostanie wyzwolona operacja wymiany dysków. Po operacji wymiany dysku dyski źródłowej maszyny wirtualnej platformy Azure zostaną pozostawione w określonej grupie zasobów dla operacji, a dyski VHD są przechowywane na określonym koncie magazynu. Możesz usunąć lub zachować te wirtualne dyski twarde i dyski.
...
- **Premium Storage:**
  - W przypadku przywracania maszyn wirtualnych spoza wersji Premium konta magazynu w chmurze w chmurze nie są obsługiwane.
...
1. W magazynie skojarzonym z maszyną wirtualną, którą chcesz przywrócić, wybierz pozycję **Elementy kopii**  >  **zapasowej Maszyna wirtualna platformy Azure.**
...
1. W **przywróceniu** maszyny wirtualnej wybierz opcję przywracania:
    ...
    - **Zastąp istniejący:** użyj tej opcji, jeśli chcesz zastąpić dyski na istniejącej maszynie wirtualnej.
...
1. W **nazwa maszyny wirtualnej**, określ maszynę wirtualną, która nie istnieje w subskrypcji.
...
Jedną z opcji [przywracania](#restore-options)jest utworzenie dysku z punktu przywracania. Następnie za pomocą dysku możesz wykonać jedną z następujących czynności:
...
- [Utwórz nową maszynę wirtualną](./backup-azure-vms-automation.md#create-a-vm-from-restored-disks) z przywróconych dysków przy użyciu programu PowerShell.
...
Jeśli maszyna wirtualna używa dysków  zarządzanych i wybierzesz opcję Utwórz maszynę wirtualną, Azure Backup nie użyje określonego konta magazynu. W przypadku **przywracania dysków i** natychmiastowego **przywracania** konto magazynu jest używane tylko do przechowywania szablonu. Dyski zarządzane są tworzone w określonej grupie zasobów.
Gdy maszyna wirtualna korzysta z dysków niezabędnych, są one przywracane jako obiekty blob na koncie magazynu.
...
1. Aby dostosować ustawienie maszyny wirtualnej podane w szablonie, wybierz pozycję **Edytuj szablon.** Jeśli chcesz dodać więcej dostosowań, wybierz pozycję **Edytuj parametry.**
   ...
   ![Ładowanie wdrożenia szablonu](./media/backup-azure-arm-restore-vms/edit-template1.png)
...
Jedną z opcji [przywracania](#restore-options)jest zastąpienie istniejącego dysku maszyny wirtualnej wybranym punktem przywracania. [Przejrzyj](#restore-options) wszystkie opcje przywracania.

1. W **konfiguracji przywracania** wybierz pozycję **Zastąp istniejący**.
1. W **typie przywracania** wybierz **pozycję Zamień dyski/s.** Jest to punkt przywracania, który będzie używany do zastępowania istniejących dysków maszyn wirtualnych.
...
1. W portalu przejdź do pozycji **Magazyn usługi Recovery Services** Elementy kopii  >  **zapasowej.**
...
Środowisko użytkownika przywracania regionu pomocniczego będzie podobne do interfejsu użytkownika przywracania regionu podstawowego. Podczas konfigurowania szczegółów w okienku Konfiguracja przywracania w celu skonfigurowania przywracania zostanie wyświetlony monit o podanie tylko parametrów regionu pomocniczego.

Obecnie cel [punktu punktu dostępu](azure-backup-glossary.md#rpo-recovery-point-objective) w regionie pomocniczym jest do 12 godzin od regionu podstawowego, mimo że replikacja magazynu geograficznie nadmiarowego dostępnego do odczytu [(RA-GRS)](../storage/common/storage-redundancy.md#redundancy-in-a-secondary-region) wynosi 15 minut.
...
![Powiadomienie o trwaniu przywracania wyzwalacza](./media/backup-azure-arm-restore-vms/restorenotifications.png)
...
>- Funkcja przywracania między regionami przywraca maszyny wirtualne platformy Azure z włączoną funkcją CMK (klucze zarządzane przez klienta), których nie ma w magazynie usługi Recovery Services z włączoną obsługą klucza CMK, jako maszyny wirtualne bez obsługi klucza CMK w regionie pomocniczym.
>- Role platformy Azure potrzebne do przywrócenia w regionie pomocniczym są takie same jak role w regionie podstawowym.
>- Podczas przywracania maszyny wirtualnej platformy Azure Azure Backup automatycznie konfiguruje ustawienia sieci wirtualnej w regionie pomocniczym. Jeśli przywracasz [dyski podczas](#restore-disks) wdrażania szablonu, upewnij się, że ustawienia sieci wirtualnej są odpowiednie dla regionu pomocniczego.

[Maszyny wirtualne przypięte do strefy platformy Azure](../virtual-machines/windows/create-portal-availability-zone.md) można przywrócić w dowolnych [strefach dostępności](../availability-zones/az-overview.md) w tym samym regionie.

W procesie przywracania zobaczysz opcję **Strefa dostępności.** Najpierw zobaczysz strefę domyślną. Aby wybrać inną strefę, wybierz liczbę wybranej strefy. Jeśli przypięta strefa jest niedostępna, nie będzie można przywrócić danych do innej strefy, ponieważ dane kopii zapasowej nie są replikowane w sposób strefowy. Przywracanie w strefach dostępności jest możliwe tylko z punktów odzyskiwania w warstwie magazynu.
...
1. Wybierz **pozycję Region pomocniczy,** aby wyświetlić elementy w regionie pomocniczym.
    ...
    ![Przefiltrowane zadania tworzenia kopii zapasowej](./media/backup-azure-arm-restore-vms/secbackupjobs.png)
...
Podczas przywracania jest dostępna opcja [przywracania](../storage/common/storage-disaster-recovery-guidance.md#azure-unmanaged-disks) dysków nieza zarządzanych [jako](../virtual-machines/managed-disks-overview.md) dysków zarządzanych. Domyślnie niezaładowane maszyny wirtualne/dyski są przywracane jako niezaładowane maszyny wirtualne/dyski. Jednak jeśli zdecydujesz się przywrócić jako zarządzane maszyny wirtualne/dyski, teraz jest to możliwe. Te przywracanie nie jest wyzwalane z fazy migawki, ale tylko z fazy magazynu. Ta funkcja nie jest dostępna dla niezaładowanych zaszyfrowanych maszyn wirtualnych.

![Przywracanie jako dysków zarządzanych](./media/backup-azure-arm-restore-vms/restore-as-managed-disks.png)
...
**Przywracanie maszyn wirtualnych podczas awarii centrum danych platformy Azure** | Jeśli magazyn używa magazynu GRS i podstawowego centrum danych dla maszyny wirtualnej nie działa, Azure Backup obsługuje przywracanie kopii zapasowych maszyn wirtualnych do sparowanego centrum danych. Wybierz konto magazynu w sparowanych centrach danych i przywróć je w zwykły sposób. Azure Backup maszyny wirtualnej używa usługi obliczeniowej w sparowanych regionach. [Dowiedz się więcej](/azure/architecture/resiliency/recovery-loss-azure-region) o odporności centrum danych.<br><br> Jeśli magazyn używa magazynu GRS, możesz wybrać nową funkcję Przywracanie [między regionami](#cross-region-restore). Dzięki temu można przywrócić do drugiego regionu w scenariuszach pełnej lub częściowej awarii, a nawet jeśli w ogóle nie ma żadnych awarii.
**Przywracanie od zerowych** | Główna różnica między maszynami wirtualnym platformy Azure i lokalnymi funkcjami hypervisor polega na tym, że na platformie Azure nie ma dostępnej konsoli maszyny wirtualnej. Konsola jest wymagana w niektórych scenariuszach, takich jak odzyskiwanie przy użyciu kopii zapasowej typu odzyskiwanie systemu od zerowego (BMR, bare-metal recovery). Jednak przywracanie maszyny wirtualnej z magazynu jest pełnym zamiennikiem odzyskiwania po chmurze.
**Przywracanie maszyn wirtualnych ze specjalnymi konfiguracjami sieci** | Specjalne konfiguracje sieci obejmują maszyny wirtualne używające wewnętrznego lub zewnętrznego równoważenia obciążenia, używające wielu kart sieciowych lub wielu zastrzeżonych adresów IP. Te maszyny wirtualne są przywracane przy użyciu opcji [przywróć dysk](#restore-disks). Ta opcja powoduje utworzenie kopii wirtualnych dysków twardych na określone konto [](../load-balancer/quickstart-load-balancer-standard-internal-powershell.md) magazynu, [](../load-balancer/quickstart-load-balancer-standard-public-powershell.md) a następnie można utworzyć maszynę wirtualną z wewnętrznym lub zewnętrznym równoważeniem [obciążenia,](../virtual-machines/windows/multiple-nics.md)wieloma kartami sieciowym lub wieloma zastrzeżonych adresów [IP](../virtual-network/virtual-network-multiple-ip-addresses-powershell.md)zgodnie z konfiguracją.
...
1. Aby wyświetlić operacje dla zadania, wybierz hiperlink powiadomienia. Alternatywnie w magazynie wybierz pozycję **Zadania tworzenia kopii zapasowej,** a następnie wybierz odpowiednią maszynę wirtualną.
...
1. Aby monitorować postęp przywracania, wybierz dowolne zadanie przywracania ze **stanem W toku.** Zostanie wyświetlony pasek postępu, na którym są wyświetlane informacje o postępie przywracania:
    ...
    - **Szacowany czas przywracania:** początkowo zapewnia czas trwania operacji przywracania. W trakcie operacji czas, który trwa, zmniejsza się i osiąga zero po zakończeniu operacji przywracania.
    - **Procent przywracania**. Przedstawia procent wykonanej operacji przywracania.
    - **Liczba przeniesionych** bajtów: jeśli przywracasz przez utworzenie nowej maszyny wirtualnej, pokazuje ona bajty, które zostały przeniesione, względem całkowitej liczby bajtów do przetransferowania.

## <a name="post-restore-steps"></a>Kroki po przywróceniu

Istnieje kilka rzeczy, o których należy pamiętać po przywróceniu maszyny wirtualnej:
...
- Jeśli maszyna wirtualna kopii zapasowej ma statyczny adres IP, przywrócona maszyna wirtualna będzie miała dynamiczny adres IP, aby uniknąć konfliktu. Do [przywróconej maszyny wirtualnej](/powershell/module/az.network/set-aznetworkinterfaceipconfig#description)możesz dodać statyczny adres IP.
...
- Jeśli nie możesz uzyskać dostępu do maszyny wirtualnej po jej przywróceniu, ponieważ maszyna wirtualna ma uszkodzoną relację z kontrolerem domeny, wykonaj poniższe kroki, aby wytworzyć maszynę wirtualną:
  ...
  - Włączanie dostępu do konsoli szeregowej na maszynie wirtualnej w celu umożliwienia dostępu wiersza polecenia do maszyny wirtualnej
  ...
  - Po przebudowie maszyny wirtualnej użyj Azure Portal resetowania konta administratora lokalnego i hasła
...
- Po odłączenia i ponownym uruchomieniu maszyny wirtualnej będzie można pomyślnie nawiązać z maszyną wirtualną protokołu RDP przy użyciu poświadczeń administratora lokalnego i ponownie dołączyć maszynę wirtualną z powrotem do domeny.

## <a name="backing-up-restored-vms"></a>Backing up restored VMs

- Jeśli maszyna wirtualna została przywrócona do tej samej grupy zasobów o takiej samej nazwie, jak pierwotnie tworzyć kopię zapasową maszyny wirtualnej, kopia zapasowa będzie kontynuowana na maszynie wirtualnej po przywróceniu.
- Jeśli maszyna wirtualna jest przywracana do innej grupy zasobów lub określono inną nazwę przywróconej maszyny wirtualnej, należy skonfigurować kopię zapasową przywróconej maszyny wirtualnej.
