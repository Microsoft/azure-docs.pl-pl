...
ms.locfileid: "107716765"
---
# <a name="azure-backup-architecture-and-components"></a>Azure Backup architektury i składników

Możesz użyć usługi Azure Backup [do](backup-overview.md) kopii zapasowej danych na platformie Microsoft Azure w chmurze. Ten artykuł zawiera podsumowanie Azure Backup, składników i procesów.
...
- **Kopii zapasowej maszyn lokalnych:**
  - Kopię zapasową lokalnych maszyn z systemem Windows można utworzyć bezpośrednio na platformie Azure przy użyciu agenta Azure Backup Microsoft Azure Recovery Services (MARS). Maszyny z systemem Linux nie są obsługiwane.
...
- **Kopii zapasowej maszyn wirtualnych platformy Azure:**
  - Kopię zapasową maszyn wirtualnych platformy Azure można utworzyć bezpośrednio. Azure Backup instaluje rozszerzenie kopii zapasowej dla agenta maszyny wirtualnej platformy Azure uruchomionego na maszynie wirtualnej. To rozszerzenie kopii zapasowej całej maszyny wirtualnej.
...
Dowiedz się więcej o [tym, czego można tworzyć kopie zapasowe,](backup-overview.md) oraz o [obsługiwanych scenariuszach tworzenia kopii zapasowych.](backup-support-matrix.md)

## <a name="where-is-data-backed-up"></a>Gdzie jest kopii zapasowej danych?

Azure Backup dane kopii zapasowej są przechowywane w magazynach — magazynach usługi Recovery Services i magazynach kopii zapasowych. Magazyn to jednostka magazynu online na platformie Azure, która służy do przechowywania danych, takich jak kopie zapasowe, punkty odzyskiwania i zasady tworzenia kopii zapasowych.

Magazyny mają następujące funkcje:

- Magazyny ułatwiają organizowanie danych kopii zapasowych przy jednoczesnym zminimalizowaniu narzutu związanego z zarządzaniem.
- Możesz monitorować elementy kopii zapasowej w magazynie, w tym maszyny wirtualne platformy Azure i maszyny lokalne.
- Dostępem do magazynu można zarządzać za pomocą kontroli dostępu opartej na rolach [(RBAC)](../role-based-access-control/role-assignments-portal.md)platformy Azure.
...
--- | --- | ---
**Pełne** | Pełna kopia zapasowa zawiera całe źródło danych. Przepustowość sieci jest większa niż w przypadku różnicowych lub przyrostowych kopii zapasowych. | Służy do tworzenia początkowej kopii zapasowej.
**Różnicowe** |  Różnicowa kopia zapasowa przechowuje bloki, które uległy zmianie od czasu tworzenia początkowej pełnej kopii zapasowej. Używa mniejszej ilości sieci i magazynu i nie przechowywać nadmiarowych kopii niezmienionych danych.<br/><br/> Nieefektywne, ponieważ bloki danych, które są niezmienione między późniejszymi kopiami zapasami, są transferowane i przechowywane. | Nie jest używany przez Azure Backup.
**Przyrostowy** | Przyrostowa kopia zapasowa przechowuje tylko te bloki danych, które uległy zmianie od czasu poprzedniej kopii zapasowej. Wysoka wydajność magazynu i sieci. <br/><br/> W przypadku przyrostowych kopii zapasowych nie ma potrzeby uzupełniania pełnymi kopiami zapasowymi. | Używany przez program DPM/usługę MABS do tworzenia kopii zapasowych dysków i używany we wszystkich kopiach zapasowych na platformie Azure. Nie jest używany do tworzenia SQL Server kopii zapasowej.

## <a name="sql-server-backup-types"></a>SQL Server kopii zapasowych

W poniższej tabeli opisano różne typy kopii zapasowych używanych SQL Server baz danych oraz ich często używane:
...
**Funkcja** | **Bezpośrednie tworzenie kopii zapasowych plików i folderów (przy użyciu agenta MARS)** | **Kopia zapasowa maszyny wirtualnej platformy Azure** | **Maszyny lub aplikacje z programem DPM/usługą MABS**
--- | --- | --- | ---
Kopii zapasowej w magazynie | ![Tak][green] | ![Tak][green] | ![Tak][green]
...
- Można utworzyć zasady tworzenia kopii zapasowych dla kopii zapasowych następujących obciążeń: maszyn wirtualnych platformy Azure, bazy danych SQL na platformie Azure, maszyn wirtualnych SAP HANA wirtualnych platformy Azure i udziałów plików platformy Azure. Zasady tworzenia kopii zapasowej plików i folderów przy użyciu agenta MARS są określone w konsoli usługi MARS.
...
- Przechowywanie można zdefiniować dla punktów kopii zapasowych "codziennie", "co tydzień", "co miesiąc", "co rok".
  ...
  - "co miesiąc" oznacza kopię zapasową w określonym dniu miesiąca
...
- Wszelkie zmiany wprowadzone w okresie przechowywania zasad kopii zapasowych zostaną wstecznie zastosowane do wszystkich starszych punktów odzyskiwania poza nowymi.
...
1. Aby skonfigurować scenariusz, należy pobrać i zainstalować agenta MARS na maszynie. Następnie wybierz, co należy utworzyć kopię zapasową, kiedy będą uruchamiane kopie zapasowe i jak długo będą przechowywane na platformie Azure.
...
1. Po zakończeniu tworzenia przyrostowej kopii zapasowej nowy wirtualny dysk twardy jest scalony z dyskiem VHD utworzonym po replikacji początkowej. Ten scalony wirtualny dysk twardy zapewnia najnowszy stan używany do porównywania bieżących kopii zapasowych.
...
1. Agenta ochrony programu DPM lub mabs należy zainstalować na maszynach, które mają być chronioną usługą. Następnie należy dodać maszyny do grupy ochrony programu DPM.
...
1. Kopię zapasową dysków programu DPM/USŁUGI MABS w magazynie jest zapasowych przez agenta MARS uruchomionego na serwerze DPM/MABS.
...
- Istnieją różne warstwy wydajności dla dysków:
  - **HDD w warstwie Standardowa dysku:** Dzięki dyskom HDD i używanym do ekonomicznego magazynowania.
  - **SSD w warstwie Standardowa dysku:** Łączy elementy dysków SSD w warstwie Premium i dysków HDD w warstwie Standardowa. Oferuje bardziej spójną wydajność i niezawodność niż hdd, ale nadal opłacalne.
...
- Dyskami można zarządzać lub nie zarządzać nimi:
  - **Dyski niezamażowane:** Tradycyjny typ dysków używanych przez maszyny wirtualne. W przypadku tych dysków należy utworzyć własne konto magazynu i określić je podczas tworzenia dysku. Następnie należy ustalić, jak zmaksymalizować zasoby magazynu dla maszyn wirtualnych.
...
### <a name="back-up-and-restore-azure-vms-with-premium-storage"></a>Back up and restore Azure VMs with Premium storage (Kopii zapasowej i przywracania maszyn wirtualnych platformy Azure przy użyciu usługi Premium Storage)

Kopię zapasową maszyn wirtualnych platformy Azure można utworzyć przy użyciu magazynu w chmurze w Azure Backup:
...
- Upewnij się, że konto magazynu w chmurze Premium ma wystarczającą ilość wolnego miejsca do przechowywania tymczasowej lokalizacji przejściowej. Aby uzyskać więcej informacji, zobacz [Cele skalowalności dla kont stronicowych obiektów blob w chmurze w chmurze Premium.](../storage/blobs/scalability-targets-premium-page-blobs.md) Nie należy modyfikować lokalizacji przejściowej.
- Po zakończeniu zadania tworzenia kopii zapasowej lokalizacja przemieszczania zostanie usunięta.
- Cena magazynu używanego dla lokalizacji przejściowej jest zgodna z cenami [magazynu w chmurze Premium Storage.](../virtual-machines/disks-types.md#billing)

Podczas przywracania maszyn wirtualnych platformy Azure przy użyciu magazynu w witrynie Premium Storage można przywrócić je do magazynu w chmurze Premium lub Standardowa. Zazwyczaj należy przywrócić je do magazynu w chmurze Premium. Jeśli jednak potrzebujesz tylko podzestawu plików z maszyny wirtualnej, przywrócenie ich do magazynu w chmurze standardowej może okazać się opłacalne.
...
Podczas przywracania maszyn wirtualnych z dyskami zarządzanymi można przywrócić do pełnej maszyny wirtualnej z dyskami zarządzanymi lub do konta magazynu:
...
- Jeśli przywracasz zaszyfrowaną zarządzaną maszynę wirtualną, upewnij się, że klucze i wpisy tajne maszyny wirtualnej istnieją w magazynie kluczy przed rozpoczęciem procesu przywracania.

## <a name="next-steps"></a>Następne kroki

- Zapoznaj się z macierzą [obsługi, aby dowiedzieć się więcej o obsługiwanych funkcjach i ograniczeniach dla scenariuszy tworzenia kopii zapasowych.](backup-support-matrix.md)
