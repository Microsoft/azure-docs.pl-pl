...
description: Rozpoczynanie pracy z Azure App Service przez wdrożenie aplikacji w języku Python w kontenerze systemu Linux w App Service.
ms.topic: quickstart
ms.date: 09/22/2020
...
ms.locfileid: "107379540"
---
# <a name="quickstart-create-a-python-app-in-azure-app-service-on-linux"></a>Szybki start: tworzenie aplikacji w języku Python w Azure App Service dla systemu Linux

W tym przewodniku Szybki start wdrożysz aplikację internetową w języku Python App Service dla systemu Linux [—](overview.md#app-service-on-linux)wysoce skalowalną i samonadzyscyjną usługę hostingu w Internecie na platformie Azure. Lokalny interfejs wiersza polecenia [(CLI)](/cli/azure/install-azure-cli) platformy Azure na komputerze Mac, Linux lub Windows umożliwia wdrożenie przykładu za pomocą platformy Flask lub Django. Skonfigurowana aplikacja internetowa używa bezpłatnej App Service, więc w trakcie pracy z tym artykułem nie są naliczane żadne koszty.

> [!TIP]
> Jeśli wolisz wdrażać aplikacje za pośrednictwem środowiska IDE, zobacz **[Deploy Python apps to App Service from Visual Studio Code](/azure/developer/python/tutorial-deploy-app-service-on-linux-01)**.
...
Sklonuj przykładowe repozytorium przy użyciu następującego polecenia i przejdź do folderu sample. (Zainstaluj[program git,](https://git-scm.com/downloads) jeśli nie masz jeszcze narzędzia git).
...
Przykład zawiera kod specyficzny dla struktury, który Azure App Service rozpoznany podczas uruchamiania aplikacji. Aby uzyskać więcej informacji, zobacz [Proces uruchamiania kontenera](configure-language-python.md#container-startup-process).
...
- Jeśli polecenie nie zostanie rozpoznane, upewnij się, że masz zainstalowany interfejs wiersza polecenia platformy Azure zgodnie z opisem w tece `az` [Konfigurowanie początkowego środowiska](#set-up-your-initial-environment).
- Jeśli polecenie nie zostanie rozpoznane, ponieważ interfejs wiersza polecenia platformy Azure ma `webapp` wersję 2.0.80 lub nowszą. Jeśli nie, [zainstaluj najnowszą wersję programu](/cli/azure/install-azure-cli).
- Zastąp nazwą unikatową na całej `<app_name>` platformie Azure (prawidłowe *znaki to , `a-z` `0-9` i `-`*). Dobrym wzorcem jest użycie kombinacji nazwy firmy i identyfikatora aplikacji.
- Argument `--sku B1` tworzy aplikację internetową w warstwie cenowej Podstawowa, co powoduje naliczanie niewielkiego kosztu godzinowego. Pomiń ten argument, aby użyć szybszej warstwy Premium.
- Opcjonalnie możesz dołączyć argument `--location <location-name>` , gdzie `<location_name>` jest dostępnym regionem świadczenia usługi Azure. Listę regionów, które można zezwolić na korzystanie z konta platformy Azure, możesz pobrać, uruchamiając [`az account list-locations`](/cli/azure/appservice#az-appservice-list-locations) polecenie .
- Jeśli zostanie wyświetlony błąd "Nie można automatycznie wykryć stosu środowiska uruchomieniowego aplikacji", upewnij się, że uruchamiasz polecenie w folderze *python-docs-hello-world* (Flask) lub w folderze *python-docs-hello-django* (Django), który *requirements.txt* plik. (Zobacz [Rozwiązywanie problemów z automatycznym wykrywaniem za pomocą polecenia az webapp up](https://github.com/Azure/app-service-linux-docs/blob/master/AzWebAppUP/runtime_detection.md) (GitHub).)
...
Przejdź do wdrożonej aplikacji w przeglądarce internetowej pod adresem URL `http://<app-name>.azurewebsites.net` . Początkowe uruchomienie aplikacji może potrwać kilka minut.
...
1. Utwórz środowisko wirtualne i zainstaluj zależności:
    ...
    Jeśli napotkasz błąd "[Errno 2] No such file or directory: 'requirements.txt'".", upewnij się, że znajdujesz się w folderze *python-docs-hello-world.*
...
1. Uruchom serwer dewelopera.
    ...
    Domyślnie serwer zakłada, że moduł wprowadzania aplikacji znajduje się w app.py *,* jak w przykładzie. (Jeśli używasz innej nazwy modułu, ustaw zmienną `FLASK_APP` środowiskową na wartość tej nazwy).

1. Otwórz przeglądarkę internetową i przejdź do przykładowej aplikacji na stronie `http://localhost:5000/` . Aplikacja wyświetli komunikat **Hello, World!**.

    ![Lokalne uruchamianie przykładowej aplikacji w języku Python](./media/quickstart-python/run-hello-world-sample-python-app-in-browser-localhost.png)
...
1. W oknie terminalu naciśnij **klawisze Ctrl** + **C,** aby zakończyć działanie serwera projektowego.
::: zone-end
...
W tej sekcji dokonasz niewielkiej zmiany kodu, a następnie ponownie wdepniesz kod na platformie Azure. Zmiana kodu zawiera `print` instrukcje służące do generowania danych wyjściowych rejestrowania, z których będziesz pracować w następnej sekcji.

::: zone pivot="python-framework-flask"
Otwórz *app.py* w edytorze i zaktualizuj funkcję w celu dopasowania `hello` do poniższego kodu. 
...
::: zone-end
::: zone pivot="python-framework-django"
Otwórz *w edytorze element hello/views.py* i zaktualizuj `hello` funkcję tak, aby dopasować go do poniższego kodu.
...
> [!TIP]
> Visual Studio Code udostępnia zaawansowane rozszerzenia dla języka Python i Azure App Service, które upraszczają proces wdrażania aplikacji internetowych w języku Python w App Service. Aby uzyskać więcej informacji, zobacz [Deploy Python apps to App Service from Visual Studio Code](/azure/python/tutorial-deploy-app-service-on-linux-01)(Wdrażanie aplikacji języka Python App Service z Visual Studio Code .

## <a name="stream-logs"></a>Strumieniowe przesyłanie dzienników

Możesz uzyskać dostęp do dzienników konsoli wygenerowanych z aplikacji i kontenera, w którym jest uruchamiana. Dzienniki zawierają wszystkie dane wyjściowe wygenerowane przy `print` użyciu instrukcji .
...
Możesz również dołączyć parametr `--logs` z `az webapp up` poleceniem , aby automatycznie otworzyć strumień dziennika podczas wdrażania.

Odśwież aplikację w przeglądarce, aby wygenerować dzienniki konsoli, które zawierają komunikaty opisujące żądania HTTP do aplikacji. Jeśli dane wyjściowe nie pojawiają się natychmiast, spróbuj ponownie za 30 sekund.
...
Przejdź do witryny <a href="https://portal.azure.com" target="_blank">Azure Portal</a>, aby zarządzać utworzoną aplikacją. Wyszukaj i wybierz **pozycję App Services**.
...
W poprzednich krokach utworzono zasoby platformy Azure w grupie zasobów. Grupa zasobów ma nazwę, na przykład "appsvc_rg_Linux_CentralUS" w zależności od lokalizacji. Jeśli aplikacja internetowa będzie nadal działać, spowoduje to naliczenie pewnych bieżących kosztów (zobacz [cennik App Service ).](https://azure.microsoft.com/pricing/details/app-service/linux/)
