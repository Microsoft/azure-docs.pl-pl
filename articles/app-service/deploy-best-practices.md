...
Możesz również zautomatyzować wdrożenie kontenera [za pomocą akcji usługi GitHub](containers/deploy-container-github-action.md).  Poniższy plik przepływu pracy kompiluje i oznaczy kontener przy użyciu identyfikatora zatwierdzenia, wypchnij go do rejestru kontenerów i zaktualizuje określone miejsce w miejscu przy użyciu nowego tagu obrazu.
