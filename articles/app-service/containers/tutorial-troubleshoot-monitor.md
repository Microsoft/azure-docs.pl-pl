...
Wybierz dwa pierwsze obrazy, a następnie `convert`kliknij przycisk. Konwersja zostanie zakończona pomyślnie.
...
Ta akcja kończy się niepowodzeniem `HTTP 500` i generuje błąd, który nie został przetestowany podczas opracowywania.
...
Po uzyskaniu dostępu do aplikacji wyświetlmy dane skojarzone z żądaniami HTTP, które `AppServiceHTTPLogs`znajdują się w temacie.
...
(2) kliknij dwukrotnie `AppServiceConsoleLogs` tabelę, a następnie kliknij `Run`przycisk. 

Ze względu na to, że konwertowanie pięciu obrazów powoduje błędy serwera, można sprawdzić, czy aplikacja również zapisuje `ResultDescription` błędy przez filtrowanie pod kątem błędów, jak pokazano poniżej:
