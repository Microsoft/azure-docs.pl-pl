...
W celu zapewnienia ciągłości działania i odzyskiwania po awarii (BCDR) można skonfigurować wystąpienie Azure SQL Database/zarządzane z [grupą replikacji geograficznej/trybu failover](../azure-sql/database/auto-failover-group-overview.md), gdzie SSISDB w podstawowym regionie platformy Azure z dostępem do odczytu i zapisu (rola podstawowa) będzie stale replikowana do regionu pomocniczego z dostępem tylko do odczytu (rola pomocnicza). Gdy wystąpi awaria w regionie podstawowym, zostanie wyzwolone przejście w tryb failover, gdzie podstawowa i pomocnicza SSISDBs zastąpią role.
...
1. Możesz [przetestować grupę Azure SQL Database trybu failover](../azure-sql/database/failover-group-add-single-database-tutorial.md?tabs=azure-portal#3---test-failover) i sprawdzić [Azure-SSIS IR stronę monitorowania w portalu ADF](./monitor-integration-runtime.md#monitor-the-azure-ssis-integration-runtime-in-azure-portal) , niezależnie od tego, czy podstawowy i pomocniczy urząd Azure-SSIS ma zamienione role. 
...
1. Za pomocą Azure Portal można [utworzyć grupę trybu failover dla podstawowych i pomocniczych wystąpień usługi Azure SQL](../azure-sql/managed-instance/failover-group-add-instance-tutorial.md?tabs=azure-portal) na stronie **grupy trybu failover** w podstawowym wystąpieniu zarządzanym Azure SQL.
