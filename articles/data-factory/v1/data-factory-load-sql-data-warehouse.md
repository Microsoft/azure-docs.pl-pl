...
[Azure SQL Data Warehouse](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md) to oparta na chmurze baza danych skalowalna w poziomie, która umożliwia przetwarzanie ogromnych ilości danych, zarówno relacyjnych, jak i nierelacyjnych.  Oparta na architekturze wysoce Parallel Processing (MPP), SQL Data Warehouse jest zoptymalizowana pod kątem obciążeń magazynu danych w przedsiębiorstwie.  Oferuje elastyczność chmury i umożliwia niezależne skalowanie magazynu i obliczanie.

Wprowadzenie do Azure SQL Data Warehouse jest teraz łatwiejsze niż kiedykolwiek przy użyciu **Azure Data Factory**.  Azure Data Factory to w pełni zarządzana usługa integracji danych oparta na chmurze, która może służyć do wypełniania SQL Data Warehouse danymi z istniejącego systemu i oszczędności czasu podczas oceniania SQL Data Warehouse i tworzenia rozwiązań analitycznych. Oto najważniejsze zalety ładowania danych do Azure SQL Data Warehouse przy użyciu Azure Data Factory:
...
W tej sekcji przedstawiono sposób konfigurowania lokalizacji docelowej: `lineitem` tabeli w bazie danych Azure SQL Data Warehouse.
...
Aby uzyskać szczegółowe informacje, zobacz [najlepsze rozwiązania dotyczące Azure SQL Data Warehouse](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-best-practices.md) .
