...
ms.locfileid: "90947667"
---
W tym przewodniku szybki start dowiesz się, jak zacząć wywoływanie przy użyciu biblioteki klienta wywołania usługi Azure Communications Services dla systemu iOS.
...
W Xcode Utwórz nowy projekt dla systemu iOS i wybierz szablon **aplikacji pojedynczego widoku** . W tym samouczku jest stosowana [Struktura SwiftUI](https://developer.apple.com/xcode/swiftui/), więc należy ustawić **Język** **SWIFT** i **interfejs użytkownika** na **SwiftUI**. Nie ma potrzeby tworzenia testów jednostkowych ani testów interfejsu użytkownika podczas tego przewodnika Szybki Start. Nie sprawdzaj, czy **są dostępne testy jednostkowe** , i nie sprawdzaj także **testów interfejsu użytkownika**.
...
Możesz utworzyć aplikację Uruchom w symulatorze systemu iOS, wybierając opcję **Product**  >  **Uruchom** produkt lub używając skrótu klawiaturowego (&#8984;-R).
...
Możesz pobrać przykładową aplikację z usługi [GitHub](https://github.com/Azure/Communication/tree/master/samples/AzureCommunicationCalling/iOS/Swift)
