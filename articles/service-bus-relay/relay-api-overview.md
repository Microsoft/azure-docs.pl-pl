...
Biblioteka .NET Framework obsługuje tylko model programowania WCF i opiera się na zastrzeżonym protokole binarnym opartym na transportach `net.tcp` WCF. Ten protokół i Biblioteka są utrzymywane w celu zapewnienia zgodności z poprzednimi wersjami z istniejącymi aplikacjami.
...
`hyco-https` Moduł zmienia i częściowo zastępuje podstawowe moduły `http` Node. js i `https`dostarcza implementację odbiornika https zgodną z wieloma istniejącymi modułami Node. js i aplikacjami zależnymi od tych modułów podstawowych.

Moduły `hyco-ws` i `hyco-websocket` zmieniają popularne `ws` i `websocket` moduły dla środowiska Node. js, zapewniając alternatywne implementacje odbiorników, które umożliwiają modułom i aplikacjom korzystanie z dowolnego modułu, aby działały za połączenia hybrydowe Relay.
