...
| 0x0000FDD0 — 0x0000FDEF (formularze prezentacji arabskich-a) | 32 |
| 0x0000FFF0 — 0x0000FFFF (specjalne) | 16 |
| <ul><li>0x0001FFFE — 0x0001FFFF = 2 (noncharacter)</li><li>0x0002FFFE — 0x0002FFFF = 2 (noncharacter)</li><li>0x0003FFFE — 0x0003FFFF = 2 (noncharacter)</li><li>0x0004FFFE — 0x0004FFFF = 2 (noncharacter)</li><li>0x0005FFFE — 0x0005FFFF = 2 (noncharacter)</li><li>0x0006FFFE — 0x0006FFFF = 2 (noncharacter)</li><li>0x0007FFFE — 0x0007FFFF = 2 (noncharacter)</li><li>0x0008FFFE — 0x0008FFFF = 2 (noncharacter)</li><li>0x0009FFFE — 0x0009FFFF = 2 (noncharacter)</li><li>0x000AFFFE — 0x000AFFFF = 2 (noncharacter)</li><li>0x000BFFFE — 0x000BFFFF = 2 (noncharacter)</li><li>0x000CFFFE — 0x000CFFFF = 2 (noncharacter)</li><li>0x000DFFFE — 0x000DFFFF = 2 (nieznakowo)</li><li>0x000EFFFE — 0x000EFFFF = 2 (niezdefiniowane)</li><li>0x000FFFFE — 0x000FFFFF = 2 (dodatkowy obszar użytku prywatnego)</li></ul> | 30 |
...
Ten błąd występuje, Azure File Sync nie obsługuje przekierowania HTTP (kod stanu 3xx). Aby rozwiązać ten problem, wyłącz przekierowanie HTTP na serwerze proxy lub urządzeniu sieciowym.
...
| 0x80070005 | -2147024891 | Error_access_denied | Warstwa pliku nie powiodła się z powodu błędu odmowy dostępu. Ten błąd może wystąpić, jeśli plik znajduje się w folderze replikacji tylko do odczytu systemu plików DFS-R. | Usługa Azure File Sync nie obsługuje punktów końcowych serwera w folderach replikacji tylko do odczytu usługi DFS-R. Zobacz [przewodnik planowania,](./storage-sync-files-planning.md#distributed-file-system-dfs) aby uzyskać więcej informacji. |
| 0x80072efe | -2147012866 | WININET_E_CONNECTION_ABORTED | Nie można warstwy pliku z powodu problemu z siecią. | Żadna akcja nie jest wymagana. Jeśli błąd będzie się powtarzać, sprawdź łączność sieciową z udziałem plików platformy Azure. |
| 0x80c80261 | -2134375839 | ECS_E_GHOSTING_MIN_FILE_SIZE | Warstwa pliku nie powiodła się, ponieważ rozmiar pliku jest mniejszy niż obsługiwany rozmiar. | Jeśli wersja agenta jest mniejsza niż 9.0, minimalny obsługiwany rozmiar pliku to 64 kb. Jeśli wersja agenta to 9.0 lub nowsza, minimalny obsługiwany rozmiar pliku zależy od rozmiaru klastra systemu plików (podwójny rozmiar klastra systemu plików). Jeśli na przykład rozmiar klastra systemu plików wynosi 4 kb, minimalny rozmiar pliku to 8 kb. |
| 0x80c83007 | -2134364153 | ECS_E_STORAGE_ERROR | Warstwa pliku nie powiodła się z powodu problemu z usługą Azure Storage. | Jeśli błąd będzie się powtarzać, otwórz wniosek o pomoc techniczną. |
| 0x800703e3 | -2147023901 | ERROR_OPERATION_ABORTED | Nie można przesłonić pliku, ponieważ został on odwołany w tym samym czasie. | Żadna akcja nie jest wymagana. Plik będzie warstwowy po zakończeniu odwoływania i plik nie jest już w użyciu. |
| 0x80c80264 | -2134375836 | ECS_E_GHOSTING_FILE_NOT_SYNCED | Nie można podzielić pliku na warstwy, ponieważ nie został on zsynchronizowany z udziałem plików platformy Azure. | Żadna akcja nie jest wymagana. Po zsynchronizowaniu pliku z udziałem plików platformy Azure plik będzie warstwowy. |
...
| 0x80070070 | -2147024784 | ERROR_DISK_FULL | Warstwa pliku nie powiodła się z powodu niewystarczającej ilości miejsca na dysku na woluminie, na którym znajduje się punkt końcowy serwera. | Aby rozwiązać ten problem, wolne co najmniej 100 MB miejsca na dysku na woluminie, na którym znajduje się punkt końcowy serwera. |
| 0x80070490 | -2147023728 | ERROR_NOT_FOUND | Nie można podzielić pliku na warstwy, ponieważ nie został on zsynchronizowany z udziałem plików platformy Azure. | Żadna akcja nie jest wymagana. Plik będzie warstwowy po zsynchronizowaniu z udziałem plików platformy Azure. |
| 0x80c80262 | -2134375838 | ECS_E_GHOSTING_UNSUPPORTED_RP | Nie można przesłonić pliku, ponieważ jest to nieobsługiwany punkt ponownej ponownej dystrybucji. | Jeśli plik jest punktem ponownej deduplikacji danych, wykonaj [](./storage-sync-files-planning.md#data-deduplication) kroki opisane w przewodniku planowania, aby włączyć obsługę deduplikacji danych. Pliki z punktami ponownej dystrybucji innymi niż deduplikacja danych nie są obsługiwane i nie będą warstwowe.  |
| 0x80c83052 | -2134364078 | ECS_E_CREATE_SV_STREAM_ID_MISMATCH | Nie można przewarstwowować pliku, ponieważ został on zmodyfikowany. | Żadna akcja nie jest wymagana. Plik będzie warstwowy po zsynchronizowaniu zmodyfikowanego pliku z udziałem plików platformy Azure. |
| 0x80c80269 | -2134375831 | ECS_E_GHOSTING_REPLICA_NOT_FOUND | Nie można podzielić pliku na warstwy, ponieważ nie został on zsynchronizowany z udziałem plików platformy Azure. | Żadna akcja nie jest wymagana. Po zsynchronizowaniu pliku z udziałem plików platformy Azure plik będzie warstwowy. |
| 0x80072ee2 | -2147012894 | WININET_E_TIMEOUT | Warstwa pliku nie powiodła się z powodu problemu z siecią. | Żadna akcja nie jest wymagana. Jeśli błąd będzie się powtarzać, sprawdź łączność sieciową z udziałem plików platformy Azure. |
| 0x80c80017 | -2134376425 | ECS_E_SYNC_OPLOCK_BROKEN | Nie można przesłonić pliku, ponieważ został zmodyfikowany. | Żadna akcja nie jest wymagana. Plik będzie warstwowy po zsynchronizowaniu zmodyfikowanego pliku z udziałem plików platformy Azure. |
| 0x800705aa | -2147023446 | ERROR_NO_SYSTEM_RESOURCES | Nie można przesłonić pliku z powodu niewystarczających zasobów systemowych. | Jeśli błąd będzie się powtarzać, sprawdź, która aplikacja lub sterownik trybu jądra wyczerpuje zasoby systemowe. |
...
| 0x80070036 | -2147024842 | ERROR_NETWORK_BUSY | Nie można odwołać pliku z powodu problemu z siecią.  | Jeśli błąd będzie się powtarzać, sprawdź łączność sieciową z udziałem plików platformy Azure. |
...
| 0x800705aa | -2147023446 | ERROR_NO_SYSTEM_RESOURCES | Nie można odwołać pliku z powodu niewystarczających zasobów systemowych. | Jeśli błąd będzie się powtarzać, sprawdź, która aplikacja lub sterownik trybu jądra wyczerpuje zasoby systemowe. |
| 0x8007000e | -2147024882 | ERROR_OUTOFMEMORY | Nie można odwołać pliku z powodu niewystarczającej ilości pamięci. | Jeśli błąd będzie się powtarzać, sprawdź, która aplikacja lub sterownik trybu jądra powoduje stan małej ilości pamięci. |
| 0x80070070 | -2147024784 | ERROR_DISK_FULL | Nie można odwołać pliku z powodu niewystarczającej ilości miejsca na dysku. | Aby rozwiązać ten problem, należy usunąć miejsce na woluminie, przenosząc pliki do innego woluminu, zwiększyć rozmiar woluminu lub wymusić warstwy plików przy użyciu Invoke-StorageSyncCloudTiering cmdlet. |
...
- Aby zapobiec oddzielonym plikom warstwowym w przyszłości, wykonaj kroki opisane w tece [Usuwanie](./storage-sync-files-server-endpoint.md#remove-a-server-endpoint) punktu końcowego serwera podczas usuwania punktu końcowego serwera.
...
1. Otwórz okno programu PowerShell z podwyższonym poziomem uprawnień, a następnie uruchom następujące polecenia (po każdym poleceniu naciśnij klawisz Enter):
