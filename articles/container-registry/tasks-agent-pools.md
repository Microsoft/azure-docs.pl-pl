...
Za pomocą polecenia az acr build za pomocą polecenia [az acr build][az-acr-build] zakoduj szybkie zadanie w puli agentów i przekaż `--agent-pool` parametr :
...
Aby uzyskać więcej przykładów kompilacji i konserwacji obrazu kontenera w chmurze, zapoznaj się z [zadania usługi ACR samouczkiem.](container-registry-tutorial-quick-task.md)
