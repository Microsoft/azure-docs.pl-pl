...
- **Skalowanie w razie potrzeby** — zwiększenie liczby wystąpień w puli agentów na potrzeby zadań intensywnie korzystających z obliczeń lub skalowania do zera. Opłaty są naliczane na podstawie przydziału.
...
- Pule agentów zadań są dostępne w wersji zapoznawczej w następujących regionach: zachodnie stany USA 2, Południowo-środkowe stany USA, Wschodnie stany USA 2 i Wschodnie stany USA.
- Dla każdego rejestru domyślny całkowity limit przydziału vCPU (rdzeń) dla wszystkich pul agentów wynosi 16. Otwórz [żądanie pomocy technicznej][open-support-ticket] , aby uzyskać dodatkowe alokacje.
...
|Warstwa  |Procesor CPU  |Pamięć (GB)  |
|---------|---------|---------|
|S1     |  2       |  3       |
|S2     |  4       |   8      |
|S3     |    8     |   16      |
...
| Kierunek | Protocol (Protokół) | Element źródłowy         | Port źródłowy | Element docelowy          | Port docelowy | Służyć    |
|-----------|----------|----------------|-------------|----------------------|-----------|---------|
| Wychodzący  | TCP      | VirtualNetwork | Dowolne         | Magazyn              | 443       | Domyślny |
| Wychodzący  | TCP      | VirtualNetwork | Dowolne         | EventHub             | 443       | Domyślny |
| Wychodzący  | TCP      | VirtualNetwork | Dowolne         | Usługi azureactivedirectory | 443       | Domyślny |
| Wychodzący  | TCP      | VirtualNetwork | Dowolne         | AzureMonitor         | 443       | Domyślny |

> [!NOTE]
> Jeśli zadania wymagają dodatkowych zasobów z publicznego Internetu, Dodaj odpowiednie reguły. Na przykład te reguły są potrzebne w przypadku uruchomienia zadania kompilacji platformy Docker, które wymaga ściągnięcia obrazów podstawowych z usługi Docker Hub lub przywrócenia pakietu NuGet.
...
### <a name="quick-task-run"></a>Szybkie uruchamianie zadań

Przejdź do kolejki szybkiego przebiegu w puli agentów przy użyciu polecenia [AZ ACR Build][az-acr-build] i przekaż `--agent-pool` parametr:
