...
Ta linia bazowa zabezpieczeń ma zastosowanie do zapory aplikacji sieci Web platformy Azure ze wskazówkami dotyczącymi [usługi Azure Security test w wersji 1,0](../security/benchmarks/overview-v1.md) . Test porównawczy zabezpieczeń platformy Azure zawiera zalecenia dotyczące sposobu zabezpieczania rozwiązań w chmurze na platformie Azure. Zawartość jest pogrupowana według **kontroli zabezpieczeń** zdefiniowanych przez usługę Azure Security test i powiązane wskazówki dotyczące zapory aplikacji sieci Web platformy Azure. Wykluczenia **nie mają** zastosowania do zapory aplikacji sieci Web platformy Azure. 

Aby dowiedzieć się, jak Zapora aplikacji sieci Web platformy Azure jest całkowicie mapowana na test porównawczy zabezpieczeń platformy Azure, zobacz [pełny plik mapowania zabezpieczeń zapory aplikacji sieci Web platformy Azure](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
- [Tryby WAF na Application Gateway](ag/ag-overview.md#waf-modes)

- [Tryby WAF na przednich drzwiach](afds/afds-overview.md#waf-modes)
...
- [Zasady i reguły reguły KSR zapory aplikacji sieci Web](ag/application-gateway-crs-rulegroups-rules.md?tabs=owasp31)
...
- [Zasady WAF na Application Gateway](https://docs.microsoft.com/cli/azure/network/application-gateway/waf-policy?view=azure-cli-latest) 

- [Zasady WAFymi na wierzchu drzwi](https://docs.microsoft.com/cli/azure/ext/front-door/network/front-door/waf-policy?view=azure-cli-latest)
...
Wybierz Azure PowerShell lub interfejs wiersza polecenia platformy Azure, aby wyszukać lub wykonać akcje dotyczące zasobów na podstawie ich tagów.
...
- [Łączenie danych z zapory aplikacji sieci Web firmy Microsoft z platformą Azure — wskaźnikiem](/azure/sentinel/connect-microsoft-waf)
...
- [Jak włączyć ustawienia diagnostyczne dla dziennika aktywności platformy Azure](/azure/azure-monitor) 
...
- [Jak tworzyć alerty na platformie Azure](/azure/azure-monitor/learn/tutorial-response)
...
- [Informacje na temat tworzenia użytkowników administracyjnych w Azure Database for PostgreSQL](../postgresql/howto-create-users.md#the-server-admin-account)
...
**Wskazówki**: Użyj stacji roboczej dostępu uprzywilejowanego (dostępem uprzywilejowanym) z usługą Multi-Factor Authentication (MFA) skonfigurowaną w celu logowania się i konfigurowania zapory aplikacji sieci Web platformy Azure (WAF) i powiązanych zasobów. 
...
- [Jaki jest warunek lokalizacji w Azure Active Directory dostęp warunkowy](../active-directory/reports-monitoring/quickstart-configure-named-locations.md)
...
Usprawnij ten proces, tworząc ustawienia diagnostyczne dla kont użytkowników usługi Azure Active Directory (Azure AD) i wysyłając dzienniki inspekcji i dzienniki logowania do Log Analytics obszaru roboczego. Skonfiguruj żądane alerty w obszarze roboczym Log Analytics.
...
**Wskazówki**: Użyj funkcji ryzyka i ochrony tożsamości Azure Active Directory (Azure AD), aby skonfigurować automatyczne odpowiedzi na wykryte podejrzane działania związane z tożsamościami użytkowników. Pozyskiwanie danych na platformie Azure — Wskaźnikowanie do dalszej analizy.
...
- [Jak skonfigurować klucze szyfrowania zarządzane przez klienta](/azure/storage/common/storage-encryption-keys-portal)
...
- [Omówienie integracji między Application Gateway i Azure Security Center](../application-gateway/application-gateway-integration-security-center.md#overview)
...
Użyj Azure Policy, aby wprowadzić ograniczenia dotyczące typu zasobów, które mogą być tworzone w ramach subskrypcji. Użyj grafu zasobów platformy Azure do wykonywania zapytań dotyczących zasobów i odnajdywania ich w ramach subskrypcji. Upewnij się, że wszystkie zasoby platformy Azure obecne w środowisku zostały zatwierdzone.
...
Przyznaj lub Odmów uprawnień do określonych użytkowników, wbudowanych grup zabezpieczeń lub grup zdefiniowanych w Azure Active Directory (Azure AD), jeśli są zintegrowane z usługą Azure DevOps lub Active Directory, jeśli są zintegrowane z usługą Team Foundation Server (TFS).
