...
- Wiersz zaczynający się od `@params` definiuje wszystkie zmienne używane przez kod języka R oraz odpowiednie typy danych SQL.
...
Ponieważ `utils` pakiet jest zainstalowany, ale nie został załadowany domyślnie, należy użyć funkcji, `library()` Aby załadować ją w pierwszej kolejności.
