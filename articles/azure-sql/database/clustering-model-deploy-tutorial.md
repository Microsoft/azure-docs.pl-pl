...
* W trzeciej części tej serii samouczków przyjęto założenie, że wykonano [**część jeden**](clustering-model-prepare-data-tutorial.md) i [**drugą część**](clustering-model-build-tutorial.md).
...
Procedura zapisuje wyniki mapowań klastra klienta w tabeli bazy danych **customer_return_clusters**.
...
Załóżmy, że chcesz wysłać promocyjną wiadomość e-mail do klientów w klastrze 3, grupy, która ma bardziej aktywne zachowanie zwrotne (można zobaczyć, jak cztery klastry zostały opisane w [drugiej części](clustering-model-build-tutorial.md#analyze-the-results)). Poniższy kod wybiera adresy e-mail klientów w klastrze 3.
...
Po zakończeniu pracy z tym samouczkiem możesz usunąć bazę danych tpcxbb_1gb z serwera.
...
* [Samouczek: Przygotowywanie danych do uczenia modelu predykcyjnego w języku R z Azure SQL Database Machine Learning Services (wersja zapoznawcza)](predictive-model-prepare-data-tutorial.md)
...
* [Dodaj pakiet języka R do Azure SQL Database Machine Learning Services (wersja zapoznawcza)](machine-learning-services-add-r-packages.md)
