...
Azure SQL Database obejmuje [klasyfikatory baz danych](database-advisor-implement-performance-recommendations.md) , które zapewniają zalecenia dotyczące dostrajania wydajności dla jednej i puli baz danych. Te zalecenia są dostępne w Azure Portal, a także przy użyciu [programu PowerShell](https://docs.microsoft.com/powershell/module/az.sql/get-azsqldatabaseadvisor). Możesz również włączyć [dostrajanie automatyczne](automatic-tuning-overview.md) , aby Azure SQL Database mogły automatycznie zaimplementować te zalecenia dostrajania.
