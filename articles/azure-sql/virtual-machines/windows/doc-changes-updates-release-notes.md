...
| **Tworzenie usługi AG przy użyciu Azure PowerShell** | Teraz można uprościć tworzenie grupy dostępności przy użyciu [Azure PowerShell](availability-group-az-commandline-configure.md) , a także interfejsu wiersza polecenia platformy Azure. | 
...
| Zmiany | Szczegóły |
| --- | --- |
| **Obsługa Azure Government** | Teraz można rejestrować SQL Server maszyny wirtualne z rozszerzeniem agenta SQL IaaS dla maszyn wirtualnych hostowanych w chmurze [Azure Government](https://azure.microsoft.com/global-infrastructure/government/) . | 
| &nbsp; | &nbsp; |
...
| **Rejestracja zbiorczych rozszerzeń SQL IaaS** | Teraz można [rejestrować zbiorczo](sql-agent-extension-manually-register-vms-bulk.md) SQL Server maszyn wirtualnych przy użyciu [rozszerzenia programu SQL IaaS Agent](sql-server-iaas-agent-extension-automate-management.md). | 
...
| **Zmiany w rozszerzeniu programu SQL IaaS Agent** | [Maszynę wirtualną SQL Server można zarejestrować za pomocą rozszerzenia programu SQL IaaS Agent](sql-agent-extension-manually-register-single-vm.md) przy użyciu nowych trybów IaaS języka SQL. Ta możliwość obejmuje obrazy [systemu Windows Server 2008](sql-server-iaas-agent-extension-automate-management.md#management-modes) .|
