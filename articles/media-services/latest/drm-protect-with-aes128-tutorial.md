...
Każdy element zawartości można szyfrować przy użyciu wielu typów szyfrowania (AES-128, PlayReady, Widevine, FairPlay). Zobacz [Streaming protocols and encryption types (Protokoły i typy szyfrowania przesyłania strumieniowego)](drm-content-protection-concept.md#streaming-protocols-and-encryption-types), aby sprawdzić, które rozwiązania warto łączyć. Sprawdź również, [jak chronić za pomocą technologii DRM](drm-protect-with-drm-tutorial.md).
...
> [Ochrona przy użyciu technologii DRM](drm-protect-with-drm-tutorial.md)
