...
Usługa Azure Media Services umożliwia dostarczanie strumieni zaszyfrowanych za pomocą licencji Microsoft PlayReady, Google Widevine i Apple FairPlay. Szczegółowe wyjaśnienie zawiera temat [Ochrona zawartości przy użyciu szyfrowania dynamicznego](drm-content-protection-concept.md).
...
![Wskaźnik AMS z chronionym przez DRM wideo w Azure Media Player](./media/drm-protect-with-drm-tutorial/ams_player.png)
...
* Zapoznanie się z artykułem [Content protection overview (Omówienie ochrony zawartości)](drm-content-protection-concept.md).
...
Jak wspomniano powyżej, obiekt **Transform** jest przepisem, a obiekt [Job](transform-jobs-concept.md) to rzeczywiste żądanie skierowane do usługi Media Services i mające na celu zastosowanie obiektu **Transform** do określonej wejściowej zawartości wideo lub dźwiękowej. **Zadanie** określa informacje na przykład o lokalizacji wejściowego wideo i lokalizacji danych wyjściowych.
...
* Konfigurowanie
    ...
    Licencje [PlayReady](drm-playready-license-template-concept.md) i [Widevine](drm-widevine-license-template-concept.md) są skonfigurowane tak, aby można było dostarczać je przy użyciu usługi dostarczania licencji w usłudze Media Services. Mimo że ta Przykładowa aplikacja nie konfiguruje licencji [FairPlay](drm-fairplay-license-overview.md) , zawiera metodę, której można użyć do skonfigurowania FairPlay. Konfigurację FairPlay można dodać jako inną opcję.
...
1. Utwórz [lokalizator przesyłania strumieniowego](stream-streaming-locators-concept.md).
...
![Ochrona przy użyciu technologii DRM](./media/drm-protect-with-drm-tutorial/playready_encrypted_url.png)
...
> [Ochrona za pomocą algorytmu AES-128](drm-playready-license-template-concept.md)
