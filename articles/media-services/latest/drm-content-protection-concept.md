...
1. Utwórz i skonfiguruj [zasady klucza zawartości](drm-content-key-policy-concept.md).
   ...
   * Skonfiguruj licencje [PlayReady](drm-playready-license-template-concept.md), [Widevine](drm-widevine-license-template-concept.md)i/lub [FairPlay](drm-fairplay-license-overview.md) . Szablony umożliwiają skonfigurowanie praw i uprawnień dla każdego z protokołów DRM.
...
2. Utwórz [lokalizator przesyłania strumieniowego](stream-streaming-locators-concept.md) skonfigurowany do przesyłania strumieniowego zaszyfrowanego zasobu.
  
   Lokalizator przesyłania strumieniowego musi być skojarzony z [zasadami przesyłania strumieniowego](stream-streaming-policy-concept.md). W tym przykładzie ustawimy `StreamingLocator.StreamingPolicyName` zasady "Predefined_MultiDrmCencStreaming".
...
> [!NOTE]
> Obsługa usługi PIFF 1,1 jest zapewniana jako zgodne rozwiązanie z poprzednimi wersjami dla telewizji inteligentnej (Samsung, LG), które implementuje wczesną wersję "Silverlight" Common Encryption. Zalecane jest używanie tylko formatu PIFF, gdy jest to konieczne do obsługi legacey Samsung lub LG inteligentnych telewizorów dostarczonych między 2009-2015, które obsługują wersję PIFF 1,1 szyfrowania PlayReady. 
...
Aby zapoznać się z przykładami użycia niestandardowych licencji/adresów URL pozyskiwania kluczy, zobacz [zasady przesyłania strumieniowego — tworzenie](/rest/api/media/streamingpolicies/create).
