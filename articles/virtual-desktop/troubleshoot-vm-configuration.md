...
Sprawdź wpisy rejestru wymienione poniżej i upewnij się, że ich wartości pasują do siebie. Jeśli brakuje kluczy rejestru lub wartości są niezgodne, upewnij się, że korzystasz [z obsługiwanego systemu operacyjnego](troubleshoot-agent.md#error-operating-a-pro-vm-or-other-unsupported-os). Jeśli jesteś, postępuj zgodnie z instrukcjami w temacie [Tworzenie puli hostów przy użyciu programu PowerShell](create-host-pools-powershell.md) w przypadku ponownego instalowania stosu równoległego.
