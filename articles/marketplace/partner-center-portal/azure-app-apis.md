...
Aby uzyskać token dostępu, postępuj zgodnie z instrukcjami w temacie [Service to Service Calls using przy użyciu poświadczeń klienta](https://azure.microsoft.com/documentation/articles/active-directory-protocols-oauth-service-to-service/) , aby wysłać `HTTP POST` do `https://login.microsoftonline.com/<tenant_id>/oauth2/token` punktu końcowego. Oto przykładowe żądanie:
