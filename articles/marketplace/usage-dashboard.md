...
| Serviceplanname | Przestarzałe | Przestarzałe (Definicja jednostki SKU) |
...
| Rozmiar podstawowy | Rozmiar podstawowy | Liczba rdzeni skojarzonych z ofertą opartą na maszynie wirtualnej. |
| Typ użycia | Typ użycia | Oznacza, czy zdarzenie użycia skojarzone z ofertą jest jednym z następujących:<ul><li>Znormalizowane użycie</li><li>Niesformatowane użycie</li><li>Użycie zmierzone</li></ul> |
...
| Waluta klienta (DW) | Waluta klienta | Waluta używana przez klienta do komercyjnej transakcji portalu Marketplace. |
| Cena (DW) | Cena | Cena jednostkowa jednostki SKU pokazana w walucie klienta. |
| Waluta wypłaty (komputer) | Waluta wypłaty | Wydawca jest płatny za zdarzenia użycia skojarzone z zasobem w walucie skonfigurowanej przez wydawcę. |
| Szacowana cena (komputer) | Szacowana cena | Cena jednostkowa jednostki SKU w walucie skonfigurowanej przez wydawcę. |
| Informacje o użyciu | Informacje o użyciu | Połączony identyfikator GUID, który służy do łączenia raportu użycia (w komercyjnej analizie Marketplace) z raportem transakcji wypłaty. Odwołanie do użycia jest połączone z identyfikatorami IDZamówienia i LineItemId w raporcie transakcji wypłaty. |
