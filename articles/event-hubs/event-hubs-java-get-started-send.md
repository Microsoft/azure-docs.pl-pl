...
> [!IMPORTANT]
> Zaktualizuj `<Event Hubs namespace connection string>` Parametry połączenia do przestrzeni nazw Event Hubs. Zaktualizuj `<Event hub name>` nazwę swojego centrum zdarzeń w przestrzeni nazw. 
...
```
### <a name="add-code-to-publish-events-to-the-event-hub"></a>Dodawanie kodu do publikowania zdarzeń w centrum zdarzeń
Dodaj metodę o nazwie `publishEvents` do `Sender` klasy, jak pokazano poniżej. 
...
2. Utwórz klasę o nazwie `Receiver` i Dodaj następujące zmienne ciągu do klasy. Zastąp symbole zastępcze prawidłowymi wartościami. 
    ...
    > [!IMPORTANT]
    > Zastąp symbole zastępcze prawidłowymi wartościami.
    > - `<Event Hubs namespace connection string>` z parametrami połączenia do przestrzeni nazw Event Hubs. Aktualizacja 
    > - `<Event hub name>` Nazwa centrum zdarzeń w przestrzeni nazw. 
    > - `<Storage connection string>` z parametrami połączenia do konta usługi Azure Storage. 
    > - `<Storage container name>` nazwa kontenera w magazynie obiektów blob platformy Azure. 
    ...
    ```
1. Dodaj następującą `main` metodę do klasy. 
