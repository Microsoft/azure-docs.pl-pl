...
Wyeksportuj oryginalną aplikację LUIS, a następnie zaimportuj aplikację z powrotem do oddzielnych aplikacji. Każda aplikacja ma swój identyfikator aplikacji. Podczas publikowania, zamiast korzystać z tego samego klucza we wszystkich aplikacjach, należy utworzyć osobny klucz dla każdej aplikacji. Należy zrównoważyć obciążenie dla wszystkich aplikacji, aby nie było przeciążać żadnej pojedynczej aplikacji. Dodaj [Application Insights](luis-tutorial-bot-csharp-appinsights.md) , aby monitorować użycie. 
