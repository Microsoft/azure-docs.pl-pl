...
1. Utwórz funkcję o nazwie `Search()`, aby wysłać żądanie wyszukiwania. W tym celu wykonaj następujące czynności:

2. W ramach tej funkcji Utwórz obiekt JSON zawierający parametry żądania. Użyj `Get` metody i Dodaj informacje o hoście i ścieżce. Dodaj klucz subskrypcji do nagłówka `Ocp-Apim-Subscription-Key`. 

3. Użyj `https.request()` , aby wysłać żądanie z użyciem wcześniej utworzonej procedury obsługi odpowiedzi i parametrów wyszukiwania.
