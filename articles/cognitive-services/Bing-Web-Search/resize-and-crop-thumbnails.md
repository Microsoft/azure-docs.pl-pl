...
Aby zażądać innego rozmiaru obrazu miniatury, Usuń wszystkie parametry zapytania z adresu URL miniatury, z `id` wyjątkiem `pid` parametrów i. Następnie należy dodać parametr `&w` zapytania (Width) `&h` lub (Height) o żądanym rozmiarze obrazu w pikselach, ale nie oba. W usłudze Bing zostanie zachowany oryginalny współczynnik proporcji obrazu. 
