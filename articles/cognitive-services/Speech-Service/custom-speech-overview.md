...
Niestandardowa mowa używa zarówno **modeli podstawowych** , jak i **modeli niestandardowych**. Każdy język ma jeden lub więcej **modeli podstawowych**. Ogólnie rzecz biorąc, gdy nowy model mowy jest publikowany do zwykłej usługi mowy, jest również importowany do usługi Custom Speech jako nowy **model podstawowy**. Są one aktualizowane co 3-6 miesięcy, a starsze modele zazwyczaj stają się mniej użyteczne w miarę upływu czasu, ponieważ najnowszy model ma zwykle znacznie wyższą dokładność.

Z kolei **modele niestandardowe** są tworzone przez dostosowanie wybranego modelu podstawowego do określonego scenariusza klienta. Możesz nadal używać określonego modelu niestandardowego przez dłuższy czas, gdy zostanie on osiągnięty, który spełnia Twoje wymagania, lub w czasie z dodatkowymi danymi. 
...
Zmiany modeli bazowych w celu dodania nowych funkcji i zwiększenia wydajności mogą powodować problemy z poprzednią zgodnością dla starszych modeli, a także powodować zmiany dokładności zaobserwowane przy użyciu danego modelu w konkretnym zestawie danych testowych. Aby zarządzać nakładem na utrzymanie modeli i punktów końcowych, zobacz następujące wiersze czasu dla osi czasu wygaśnięcia dla modelu i punktu końcowego.
