...
Poniższy przykład pokazuje, jak utworzyć podpis głosowy przy [użyciu interfejsu API REST](https://aka.ms/cts/signaturegenservice) w języku C#. Zwróć uwagę na to, że musisz zastąpić prawdziwe informacje dla `subscriptionKey` , `region` i ścieżkę do pliku przykładowego `.wav` .
...
Uruchomienie funkcji `GetVoiceSignatureString()` zwraca ciąg podpisu głosowego w poprawnym formacie. Uruchom funkcję dwukrotnie, aby użyć dwóch ciągów do użycia jako dane wejściowe do zmiennych `voiceSignatureStringUser1` i `voiceSignatureStringUser2` poniżej.
...
* Tworzy `AudioStreamReader` z przykładowego `.wav` pliku do transkrypcja.
* Tworzy element `Conversation` using `CreateConversationAsync()` .
* Tworzy `ConversationTranscriber` przy użyciu konstruktora i subskrybuje niezbędne zdarzenia
* Dodaje uczestników do konwersacji. Ciągi `voiceSignatureStringUser1` i `voiceSignatureStringUser2` powinny być podane jako dane wyjściowe z powyższych kroków z funkcji `GetVoiceSignatureString()` .
* Sprzęga konwersację i rozpoczyna transkrypcję.

Wywołaj funkcję, `TranscribeConversationsAsync()` Aby rozpocząć transkrypcję konwersacji.
