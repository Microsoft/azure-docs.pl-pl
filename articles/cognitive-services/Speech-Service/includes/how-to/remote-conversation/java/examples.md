...
W pokazanym tutaj kodzie potrzebna jest **wersja 1.8.0**, obsługiwana tylko w przypadku języka Java (1.8.0 lub nowszego) w systemach Windows i Linux. 

### <a name="obtaining-the-async-conversation-client-sdk"></a>Uzyskiwanie zestawu SDK klienta konwersacji asynchronicznej

Możesz uzyskać dostęp do **konwersacji zdalnej** , edytując plik pom.xml w następujący sposób.

1. Na końcu pliku przed tagiem zamykającym `</project>` Utwórz `repositories` element z odwołaniem do repozytorium Maven dla zestawu Speech SDK:
...
2. Należy również dodać `dependencies` element z remoteconversation-Client-SDK 1.8.0 jako zależność:
