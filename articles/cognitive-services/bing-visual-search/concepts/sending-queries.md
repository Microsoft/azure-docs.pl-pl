...
- Użycie tokenu usługi Insights, który uzyskuje się z obrazu w poprzednim wywołaniu jednego z [interfejs API wyszukiwania obrazów Bing](https://docs.microsoft.com/rest/api/cognitiveservices/bing-images-api-v7-reference) punktów końcowych.
...
Poniżej przedstawiono parametry zapytania, które należy określić w żądaniu. Należy podać co najmniej parametr `mkt` zapytania:
...
Każde żądanie musi zawierać `Content-Type` nagłówek. Nagłówek musi być ustawiony na: `multipart/form-data; boundary=\<boundary string\>`, gdzie \<ciąg\> graniczny jest unikatowym, nieprzezroczystym ciągiem, który identyfikuje granicę danych formularza. Na przykład `boundary=boundary_1234-abcd`.

W przypadku wysłania wyszukiwanie wizualne tokenu lub adresu URL obrazu Poniższy fragment kodu przedstawia dane formularza, które należy uwzględnić w treści wpisu. Dane formularza muszą zawierać `Content-Disposition` nagłówek i należy ustawić jego `name` parametr na wartość "knowledgeRequest". Aby uzyskać szczegółowe informacje `imageInfo` na temat obiektu, zobacz żądanie.
...
Opcjonalnie można ustawić `enableEntityData` atrybut w nagłówku, aby `true` uzyskać szczegółowe informacje dotyczące głównej jednostki w przekazywanym obrazie, w tym linki do sieci Web i informacji o przypisywaniu. To pole jest `false` domyślnie.
...
Jeśli obraz zawiera rozpoznaną jednostkę, taką jak dobrze znana/popularna osoba, miejsce lub rzecz, jeden z tagów może zawierać szczegółowe informacje o jednostce. Pola `mainEntity` i `data` są dostępne tylko wtedy, `enableEntityData` gdy atrybut w `Content-Type` nagłówku jest ustawiony na `true`.
