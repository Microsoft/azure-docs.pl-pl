...
W `parameters` sekcji szablonu należy określić, które wartości można wprowadzać podczas wdrażania zasobów. W szablonie można umieścić maksymalnie 256 parametrów. Można zmniejszyć liczbę parametrów za pomocą obiektów, które zawierają wiele właściwości.
...
| typ |Tak |Typ wartości parametru. Dozwolone typy i wartości to **String**, **SecureString**, **int**, **bool**, **Object**, **secureobject** i **Array**. Zobacz [typy danych w szablonach ARM](data-types.md). |
...
W `variables` sekcji można skonstruować wartości, które mogą być używane w całym szablonie. Nie musisz definiować zmiennych, ale często upraszczamy szablon przez zredukowanie złożonych wyrażeń. Format każdej zmiennej odpowiada jednemu z [typów danych](data-types.md).
...
W `resources` sekcji zdefiniujesz wdrożone lub zaktualizowane zasoby.
...
| scope | Nie | Właściwość Scope jest dostępna tylko dla [typów zasobów rozszerzenia](../management/extension-resource-types.md). Użyj go podczas określania zakresu, który jest inny niż zakres wdrożenia. Zobacz [ustawienie zakresu dla zasobów rozszerzenia w szablonach ARM](scope-extension-resources.md). |
...
W `outputs` sekcji należy określić wartości, które są zwracane z wdrożenia. Zazwyczaj zwracane są wartości z zasobów, które zostały wdrożone.
...
W Visual Studio Code [rozszerzenia narzędzi Azure Resource Manager](quickstart-create-templates-use-visual-studio-code.md) mogą automatycznie wykrywać szablon ARM i zmieniać tryb języka. Jeśli widzisz **szablon Azure Resource Manager** w prawym dolnym rogu Visual Studio Code, możesz użyć komentarzy wbudowanych. Komentarze w tekście nie są już oznaczone jako nieprawidłowe.
...
Dla `parameters` , Dodaj `metadata` obiekt z `description` właściwością.
...
Dla `resources` , Dodaj `comments` element lub `metadata` obiekt. Poniższy przykład pokazuje zarówno `comments` element, jak i `metadata` obiekt.
...
Dla programu `outputs` Dodaj `metadata` obiekt do wartości wyjściowej.
...
Możesz przerwać ciąg w wielu wierszach. Na przykład zapoznaj się z `location` właściwością i jednym z komentarzy w poniższym przykładzie JSON.
