...
* [Serwer aplikacji SAP](https://wiki.scn.sap.com/wiki/display/ABAP/ABAP+Application+Server) lub [serwer komunikatów SAP](https://help.sap.com/saphelp_nw70/helpdata/en/40/c235c15ab7468bb31599cc759179ef/frameset.htm) , do którego chcesz uzyskać dostęp z Logic Apps. Aby uzyskać informacje o serwerach SAP i akcjach SAP, których można używać z łącznikiem, zobacz [zgodność SAP](#sap-compatibility).
    ...
    * Serwer SAP należy skonfigurować tak, aby zezwalał na korzystanie z specyfikacji RFC. Aby uzyskać więcej informacji, zobacz następujące tematy SAP Uwaga: [460089-minimalne profile autoryzacji dla zewnętrznych programów RFC](https://launchpad.support.sap.com/#/notes/460089). 
...
1. Sformatuj komunikat wejściowy za pomocą koperty XML. Aby zapoznać się z przykładem, zobacz następujący przykładowy ładunek XML:
...
##### <a name="the-requestcontext-on-the-ireplychannel-was-closed-without-a-reply-being-sent"></a>RequestContext w IReplyChannel zostało zamknięte bez wysłania odpowiedzi

Ten komunikat o błędzie oznacza nieoczekiwane błędy, gdy procedura obsługi catch-all dla kanału kończy pracę kanału z powodu błędu i ponownie kompiluje kanał w celu przetworzenia innych komunikatów.

Aby potwierdzić, że aplikacja logiki otrzymała IDoc, [Dodaj akcję odpowiedzi](../connectors/connectors-native-reqres.md#add-a-response-action) , która zwraca `200 OK` kod stanu. Pozostaw zawartość pustą i nie zmieniaj ani nie dodawaj do nagłówków. IDoc jest transportowana przez tRFC, która nie zezwala na ładunek odpowiedzi.

Aby zamiast tego odrzucić IDoc, Odpowiedz przy użyciu dowolnego kodu stanu HTTP innego niż `200 OK` . Adapter SAP zwraca wyjątek z powrotem do SAP w Twoim imieniu. Należy tylko odrzucić IDoc, aby sygnalizować błędy transportu z powrotem do SAP, takie jak błędne trasy IDoc, których aplikacja nie może przetworzyć. Nie należy odrzucać IDoc błędów na poziomie aplikacji, takich jak problemy z danymi zawartymi w IDoc. W przypadku opóźnienia akceptacji transportu na poziomie aplikacji może wystąpić negatywna wydajność z powodu zablokowania połączenia z transportem innych IDocs.

Jeśli otrzymujesz ten komunikat o błędzie i wystąpią błędy systemowe wywołujące Logic Apps, sprawdź, czy ustawienia sieciowe usługi lokalnej bramy danych zostały skonfigurowane dla danego środowiska. Na przykład jeśli środowisko sieciowe wymaga użycia serwera proxy do wywoływania punktów końcowych platformy Azure, musisz skonfigurować lokalną usługę bramy danych do korzystania z serwera proxy. Aby uzyskać więcej informacji, zobacz [Konfiguracja serwera proxy](/dotnet/framework/network-programming/proxy-configuration).

Jeśli otrzymujesz ten komunikat o błędzie i wystąpią sporadyczne błędy wywołujące Logic Apps, może być konieczne zwiększenie liczby ponownych prób i/lub interwału ponawiania. 

1. Sprawdź ustawienia SAP w pliku konfiguracji usługi lokalnej bramy danych `Microsoft.PowerBI.EnterpriseGateway.exe.config` . Wartość ustawienia liczba ponownych prób jest następująca `WebhookRetryMaximumCount="2"` . Ustawienie interwału ponawiania będzie wyglądać tak `WebhookRetryDefaultDelay="00:00:00.10"` , jak w przypadku formatu TimeSpan `HH:mm:ss.ff` . 
    
1. Zapisz zmiany i ponownie uruchom lokalną bramę danych.

##### <a name="the-segment-or-group-definition-e2edk36001-was-not-found-in-the-idoc-meta"></a>Nie znaleziono E2EDK36001 definicji segmentu lub grupy w metadanych IDoc

Ten komunikat o błędzie oznacza, że oczekiwane błędy są spowodowane innymi błędami. Na przykład niepowodzenie generowania ładunku XML IDoc, ponieważ jego segmenty nie są uwalniane przez system SAP. W związku z tym brak metadanych typu segmentu wymaganego do konwersji.
