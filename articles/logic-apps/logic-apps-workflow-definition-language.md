...
| <*Typ parametru*> | Tak | int, float, String, bool, Array, Object, SecureString, secureobject <p><p>**Uwaga**: w przypadku wszystkich haseł, kluczy i wpisów tajnych Użyj `securestring` typów `secureobject` lub, ponieważ `GET` operacja nie zwraca tych typów. Aby uzyskać więcej informacji na temat zabezpieczania parametrów, zapoznaj się z [zaleceniami dotyczącymi zabezpieczeń i parametrami wejściowymi](../logic-apps/logic-apps-securing-a-logic-app.md#secure-action-parameters). | Typ parametru |
...
| . | Aby odwołać się do właściwości w obiekcie, użyj operatora kropki. Na przykład, aby uzyskać `name` właściwość dla obiektu `customer` json: <p>`"@parameters('customer').name"` |
