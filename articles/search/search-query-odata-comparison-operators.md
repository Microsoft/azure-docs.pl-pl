...
W przypadku porównań, które nie są dozwolone, takich jak porównywanie `Edm.Int64` pola `NaN`typu z, interfejs API REST platformy Azure wyszukiwanie POZNAWCZE zwróci błąd "http 400: złe żądanie".
...
W przypadku korzystania z operatorów porównania należy pamiętać, że wszystkie pola niebędące kolekcjami w usłudze Azure Wyszukiwanie poznawcze mogą `null`być potencjalnie. W poniższej tabeli przedstawiono wszystkie możliwe wyniki dla wyrażenia porównania, które mogą być `null`:
