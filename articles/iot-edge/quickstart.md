...
* Maszyna wirtualna z systemem Windows, która będzie działać jako urządzenie IoT Edge. Tę maszynę wirtualną można utworzyć przy użyciu następującego polecenia, zastępując `{password}` je bezpiecznym hasłem:
...
Poniższy kod tworzy bezpłatny koncentrator **F1** w grupie `IoTEdgeResources`zasobów. Zamień `{hub_name}` na unikatową nazwę Centrum IoT.
