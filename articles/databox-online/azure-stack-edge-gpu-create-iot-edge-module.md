---
title: Moduł IoT Edge C# dla Azure Stack EDGE Pro z procesorem GPU | Microsoft Docs
description: Dowiedz się, jak opracowywać moduł IoT Edge C#, który można wdrożyć na urządzeniu z systemem Azure Stack Edge.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
![Jak działa moduł Azure IoT Edge na Azure Stack EDGE Pro](./media/azure-stack-edge-gpu-create-iot-edge-module/how-module-works-1.png)
...
- Urządzenie, na którym działa Azure Stack Edge.
    ...
    Aby uzyskać więcej informacji, przejdź do pozycji [Konfigurowanie obliczeń](azure-stack-edge-j-series-deploy-configure-compute.md#configure-compute) dla Azure Stack Edge.<!--Update link?-->
...
5. Po utworzeniu rejestru kontenerów przejdź do niego i wybierz pozycję **Klucze dostępu**.
    ...
    ![Pobierz klucze dostępu](./media/azure-stack-edge-gpu-create-iot-edge-module/get-access-keys-1.png)
...
3. W palecie poleceń wprowadź i uruchom polecenie **Azure IoT Edge: nowe rozwiązanie usługi IoT Edge**. W palecie poleceń podaj następujące informacje, aby utworzyć rozwiązanie:
    ...
    2. Podaj nazwę rozwiązania lub zaakceptuj nazwę domyślną **EdgeSolution**.
        ...
        ![Utwórz nowe rozwiązanie 1](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-1.png)
    ...
    4. Zastąp domyślną nazwę modułu nazwą, którą chcesz przypisać, w tym przypadku jest to **FileCopyModule**.
        ...
        ![Utwórz nowe rozwiązanie 2](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-2.png)
    ...
    5. Określ rejestr kontenerów utworzony w poprzedniej sekcji jako repozytorium obrazów dla pierwszego modułu. Zastąp ciąg **localhost:5000** skopiowaną wartością serwera logowania.
        ...
        ![Utwórz nowe rozwiązanie 3](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-3.png)

4. Przejdź do **pliku > Otwórz folder**.

    ![Utwórz nowe rozwiązanie 4](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-4.png)

5. Przeglądaj i wskaż utworzony wcześniej folder **EdgeSolution** . Okno VS Code ładuje obszar roboczy rozwiązania IoT Edge z pięcioma składnikami najwyższego poziomu. Nie będziesz edytować `.vscode` folderu, pliku **GITIGNORE** , pliku **ENV** ani deployment.template.jsna * * w tym artykule.
    ...
    ![Utwórz nowe rozwiązanie 5](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-5.png)
...
2. Zaloguj się do platformy Docker, wprowadzając następujące polecenie w zintegrowanym terminalu.
    ...
    ![Rozwiązanie do kompilowania i wypychania IoT Edge](./media/azure-stack-edge-gpu-create-iot-edge-module/build-iot-edge-solution-1.png)
...
3. Po dostarczeniu poświadczeń można wypchnąć obraz modułu do usługi Azure Container Registry. W Eksploratorze VS Code kliknij prawym przyciskiem myszy **module.jsna** pliku i wybierz polecenie **Kompiluj i wypchnij IoT Edge rozwiązanie**.
    ...
    ![Kompiluj i wypchnij IoT Edge rozwiązanie 2](./media/azure-stack-edge-gpu-create-iot-edge-module/build-iot-edge-solution-2.png)
    ...
    ![Wybierz platformę](./media/azure-stack-edge-gpu-create-iot-edge-module/select-platform.png)
...
Aby wdrożyć i uruchomić ten moduł w Azure Stack EDGE Pro, zobacz kroki opisane w temacie [Dodawanie modułu](azure-stack-edge-j-series-deploy-configure-compute.md#add-a-module).<!--Update link?-->
