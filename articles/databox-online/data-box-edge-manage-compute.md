---
title: Azure Data Box Edge zarządzanie obliczeniami | Microsoft Docs
description: Opisuje sposób zarządzania ustawieniami obliczeń krawędzi, takimi jak wyzwalacz, moduły, wyświetlanie konfiguracji obliczeniowej, Usuwanie konfiguracji za pomocą Azure Portal w Azure Data Box Edge.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
---
# <a name="manage-compute-on-your-azure-data-box-edge"></a>Zarządzanie obliczeniami na Azure Data Box Edge

W tym artykule opisano sposób zarządzania obliczeniami na Azure Data Box Edge. Można zarządzać obliczeniami za pośrednictwem Azure Portal lub za pośrednictwem lokalnego interfejsu użytkownika sieci Web. Użyj Azure Portal do zarządzania modułami, wyzwalaczami i konfiguracją obliczeniową oraz lokalnym interfejsem użytkownika sieci Web do zarządzania ustawieniami obliczeniowymi.
...
Zdarzenia są takie same, jak w środowisku chmury lub na urządzeniu, na którym warto podjąć odpowiednie działania. Na przykład po utworzeniu pliku w udziale jest to zdarzenie. Wyzwalacze zgłaszają zdarzenia. W przypadku Data Box Edge wyzwalacze mogą być w odpowiedzi na zdarzenia plików lub harmonogram.
...
1. W Azure Portal przejdź do zasobu Data Box Edge, a następnie przejdź do pozycji **Oblicz compute > Trigger**. Wybierz pozycję **+ Dodaj wyzwalacz** na pasku poleceń.

    ![Wybierz pozycję Dodaj wyzwalacz](media/data-box-edge-manage-compute/add-trigger-1.png)
...
4. Wybierz pozycję **Dodaj** , aby utworzyć wyzwalacz. Powiadomienie pokazuje, że trwa tworzenie wyzwalacza. Po utworzeniu wyzwalacza zostanie on zaktualizowany w celu odzwierciedlenia nowego wyzwalacza.
    ...
    ![Lista zaktualizowanych wyzwalaczy](media/data-box-edge-manage-compute/add-trigger-2.png)
...
2. Kliknij prawym przyciskiem myszy, a następnie wybierz pozycję **Usuń**.
    ...
    ![Wybierz pozycję Usuń](media/data-box-edge-manage-compute/add-trigger-1.png)
...
Użyj Azure Portal, aby wyświetlić konfigurację obliczeń, usunąć istniejącą konfigurację obliczeniową lub odświeżyć konfigurację obliczeniową w celu zsynchronizowania kluczy dostępu dla urządzenia IoT i urządzenia IoT Edge Data Box Edge.
...
1. W Azure Portal przejdź do zasobu Data Box Edge, a następnie przejdź do pozycji **Oblicz compute > modules**. Wybierz pozycję **Wyświetl obliczenia** na pasku poleceń.

    ![Wybierz pozycję Wyświetl obliczenia](media/data-box-edge-manage-compute/view-compute-1.png)
...
1. W Azure Portal przejdź do zasobu Data Box Edge, a następnie przejdź do pozycji **Oblicz compute > Rozpocznij**. Na pasku poleceń wybierz pozycję **Usuń obliczenia** .
...
Podczas konfigurowania obliczeń na Data Box Edge zostanie utworzone urządzenie IoT i urządzenie IoT Edge. Do tych urządzeń są automatycznie przypisywane klucze dostępu symetrycznego. Ze względów bezpieczeństwa te klucze są regularnie obracane za pośrednictwem usługi IoT Hub.
...
Jeśli urządzenie IoT i IoT Edge klucze zostały obrócone, należy odświeżyć konfigurację na Data Box Edge, aby uzyskać najnowsze klucze dostępu. Synchronizacja pomaga urządzeniu pobrać najnowsze klucze urządzenia IoT i urządzenia IoT Edge. Data Box Edge używa tylko podstawowych kluczy dostępu.

Aby zsynchronizować klucze dostępu dla urządzenia, wykonaj następujące czynności w Azure Portal.

1. W Azure Portal przejdź do zasobu Data Box Edge, a następnie przejdź do pozycji **Oblicz compute > Rozpocznij**. Wybierz pozycję **Odśwież konfigurację** na pasku poleceń.
...
2. Wybierz opcję **tak** po wyświetleniu monitu o potwierdzenie.
     ...
     ![Wybierz opcję tak po wyświetleniu monitu](media/data-box-edge-manage-compute/refresh-configuration-2.png)

3. Po zakończeniu synchronizacji zamknij okno dialogowe.

## <a name="next-steps"></a>Następne kroki

- Dowiedz się, jak [zarządzać siecią obliczeniową programu Edge za pośrednictwem Azure Portal](data-box-edge-extend-compute-access-modules.md).
