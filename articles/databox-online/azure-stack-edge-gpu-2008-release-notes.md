...
|**trzynast.**|Kubernetes |Instalacje powiązań oparte na plikach nie są obsługiwane w przypadku Azure IoT Edge w usłudze Kubernetes na Azure Stack urządzeniu brzegowym.|IoT Edge używa warstwy translacji do translacji `ContainerCreate` opcji na konstrukcje Kubernetes. Tworzenie `Binds` map do `hostpath` katalogów lub tworzenie i w ten sposób Instalowanie powiązań opartych na plikach nie może być powiązane ze ścieżkami w kontenerach IoT Edge.|
