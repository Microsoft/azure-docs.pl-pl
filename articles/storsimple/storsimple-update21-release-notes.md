...
| 2 |Lokalnie przypięte woluminy |W rzadkich przypadkach system mógłby ulec awarii podczas tworzenia woluminu przypiętego lokalnie. Ta usterka została rozwiązana w tej wersji. |Yes |Nie |
| 3 |Warstw |Wystąpiły Sporadyczne awarie, gdy metadane dla urządzeń w chmurze StorSimple (8010 i 8020) są warstwowe w chmurze. Ten problem został rozwiązany w tej wersji. |Nie |Yes |
| 4 |Tworzenie migawki |Wystąpiły problemy związane z tworzeniem migawek przyrostowych w scenariuszach z dużymi woluminami i minimalnym niezmianą danych. Te problemy zostały rozwiązane w tej wersji. |Yes |Yes |
| 5 |Uwierzytelnianie OpenStack |W przypadku korzystania z OpenStack jako dostawcy usług w chmurze, użytkownik będzie uruchamiał się w nieczęstej usterki związanej z uwierzytelnianiem, gdzie Analizator JSON spowodował awarię. Ta usterka została rozwiązana w tej wersji. |Yes |Nie |
| 6 |Kopia po stronie hosta |We wcześniejszych wersjach oprogramowania podczas kopiowania danych z jednego woluminu na inny występuje nierzadko występujący błąd związany z ODX chronometrażu. Spowoduje to przejście do trybu failover kontrolera, a system może być w stanie przejść do trybu odzyskiwania. Ta usterka została rozwiązana w tej wersji. |Yes |Nie |
| 7 |Instrumentacja zarządzania Windows (WMI) |W poprzednich wersjach oprogramowania Wystąpił błąd serwera proxy sieci Web z wyjątkiem wyjątku "\<managementexception> błąd ładowania dostawcy". Ta usterka została przypisana do przecieku pamięci usługi WMI i została teraz naprawiona. |Yes |Nie |
| 8 |Aktualizowanie |W niektórych rzadkich wystąpieniach w poprzednich wersjach oprogramowania użytkownik otrzymał komunikat "CisPowershellHcsscripterror" podczas próby skanowania lub instalowania aktualizacji. Ten problem został rozwiązany w tej wersji. |Yes |Yes |
| 9 |Pakiet pomocy technicznej |W tej wersji wprowadzono ulepszenia sposobu, w jaki pakiet pomocy technicznej jest zbierany i przekazywany. |Yes |Yes |
...
| 15 |Azure PowerShell polecenia cmdlet i woluminy przypięte lokalnie |Nie można utworzyć woluminu przypiętego lokalnie za pomocą poleceń cmdlet Azure PowerShell. (Każdy wolumin tworzony za pośrednictwem Azure PowerShell będzie warstwowy). |Należy zawsze używać usługi StorSimple Manager, aby konfigurować woluminy przypięte lokalnie. |Yes |Nie |
...
Aby uzyskać więcej informacji na temat instalowania aktualizacji sterowników, sterownika Storport, Spaceport i oprogramowania układowego dysku, zobacz [Instalowanie aktualizacji 2,2](storsimple-install-update-21.md) na urządzeniu StorSimple.
