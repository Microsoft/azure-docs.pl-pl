...
- Można wybrać maksymalnie 10 maszyn wirtualnych do replikacji za pośrednictwem Azure Portal. Jeśli chcesz migrować więcej maszyn, wykonaj replikację w grupach o wartości 10. Nie ma żadnego limitu liczby maszyn wirtualnych, które mogą być replikowane za pośrednictwem poleceń cmdlet programu PowerShell. Nasze zalecenie polega na replikacji nie więcej niż 500 maszyn wirtualnych jednocześnie z jednego programu vCenter za pośrednictwem programu PowerShell w celu zapewnienia optymalnej wydajności.
- W przypadku migracji programu VMware bez agenta można uruchamiać maksymalnie 500 replikacje jednocześnie na podstawie poszczególnych vCenter Server.
...
**uprawnienia vCenter Server** | Migracja bez agentów używa [urządzenia migracji](migrate-appliance.md). Urządzenie musi mieć następujące uprawnienia w vCenter Server:<br/><br/> - **Datastore. Browse** (magazyn danych — > przeglądać magazyn danych): umożliwia przeglądanie plików dzienników maszyn wirtualnych w celu rozwiązywania problemów z tworzeniem i usuwaniem migawek.<br/><br/> - **Magazyn danych. usługi filemanagement** (operacje na plikach z niskim poziomem > magazynu danych): Zezwalaj na operacje odczytu/zapisu/usuwania/zmiany nazwy w przeglądarce magazynu danych, aby rozwiązywać problemy z tworzeniem i usuwaniem migawki.<br/><br/> - **VirtualMachine.Config. Śledzenia zmian** (> śledzenia zmian dysku maszyny wirtualnej): Zezwalaj na włączanie lub wyłączanie śledzenia zmian dysków maszyn wirtualnych w celu ściągania zmienionych bloków danych między migawkami.<br/><br/> - **VirtualMachine.Config. DiskLease** (> dzierżawy dysku): Zezwalaj na operacje dzierżawy dysku dla maszyny wirtualnej w celu odczytania dysku przy użyciu zestawu VMware vSphere Virtual Disk Development Kit (VDDK).<br/><br/> - **VirtualMachine. Provisioning. DiskRandomRead** (maszyna wirtualna-> aprowizacji — > zezwolić na dostęp tylko do odczytu): Zezwól na otwieranie dysku na maszynie wirtualnej w celu odczytania dysku przy użyciu VDDK.<br/><br/> - **VirtualMachine. Provisioning. DiskRandomAccess** (maszyna wirtualna > aprowizacji — > zezwolić na dostęp do dysku): Zezwól na otwieranie dysku na maszynie wirtualnej w celu odczytania dysku przy użyciu VDDK.<br/><br/> - **VirtualMachine. Provisioning. GetVmFiles** (maszyna wirtualna-> aprowizacji — > zezwolić na pobieranie maszyny wirtualnej): zezwala na operacje odczytu plików SKOJARZONYCH z maszyną wirtualną, pobieranie dzienników i rozwiązywanie problemów w przypadku wystąpienia błędu.<br/><br/> - **VirtualMachine. State. \* *_ (Maszyna wirtualna > zarządzanie migawkami): umożliwia tworzenie migawek maszyn wirtualnych i zarządzanie nimi na potrzeby replikacji. <br/> <br/> -_* VirtualMachine. Interaction. wyłączenie** (interakcja z maszyną wirtualną > — > wyłączyć): umożliwia wyłączenie maszyny wirtualnej podczas migracji na platformę Azure.
