...
## <a name="retrieve-the-status-of-a-job"></a>Pobieranie stanu zadania

Stan zadania można monitorować przy użyciu `Get-AzMigrateJob` polecenia cmdlet. 
...
[Azure Migrate: Migracja serwera](migrate-services-overview.md#azure-migrate-server-migration-tool) umożliwia zmianę właściwości docelowych, takich jak nazwa, rozmiar, Grupa zasobów, konfiguracja karty sieciowej i tak dalej, w przypadku replikacji maszyny wirtualnej. Polecenie cmdlet zwraca zadanie, które może być śledzone w celu monitorowania stanu operacji. 
...
Po zakończeniu testowania Oczyść test migracji przy użyciu `Start-AzMigrateTestMigrationCleanup` polecenia cmdlet. Polecenie cmdlet zwraca zadanie, które może być śledzone w celu monitorowania stanu operacji. 
...
2. Zainstaluj agenta systemu [Linux](../virtual-machines/extensions/agent-linux.md) na zmigrowanych maszynach, jeśli komputer ma system operacyjny Linux. Podczas migracji automatycznie instalujemy agenta maszyny wirtualnej dla maszyn wirtualnych z systemem Windows.
