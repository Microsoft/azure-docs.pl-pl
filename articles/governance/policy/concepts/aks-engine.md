...
1. Tworzenie przypisania roli dla jednostki usługi klastra
   ...
   - Przypisz przypisanie roli modułu zapisywania danych usługi Policy Insights (wersja zapoznawcza) do identyfikatora aplikacji głównej (wartość _aadClientID_ z poprzedniego kroku) przy użyciu interfejsu wiersza polecenia platformy Azure. Zastąp `<subscriptionId>` wartość identyfikatorem subskrypcji `<aks engine cluster resource group>` i grupą zasobów, w której znajduje się klaster Kubernetes aparatu AKS.
...
- Instalowanie w istniejącym klastrze z wykresami Helm
  ...
  1. Zainstaluj dodatek przy użyciu wykresu Helm. Zastąp `<subscriptionId>` wartość identyfikatorem subskrypcji `<aks engine cluster resource group>` i grupą zasobów, w której znajduje się klaster Kubernetes aparatu AKS.
...
Strażnika pod, _strażnik-kontroler-Manager-0_, zazwyczaj znajduje się w `gatekeeper-system` przestrzeni `kube-system` nazw lub, ale może znajdować się w innej przestrzeni nazw w zależności od sposobu jego wdrożenia.
