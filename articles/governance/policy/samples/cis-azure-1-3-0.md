...
W poniższym artykule szczegółowo opisano sposób mapowania wbudowanej definicji  inicjatywy  Azure Policy Regulatory Compliance na domeny zgodności i mechanizmy kontroli w temencie CIS Microsoft Azure Foundations Benchmark 1.3.0.
...
Następnie znajdź i wybierz wbudowaną definicję inicjatywy **CIS Microsoft Azure Foundations Benchmark v1.3.0** Regulatory Compliance.
...
**IDENTYFIKATOR:** CIS Azure 1.1 **Ownership**: Customer
...
**IDENTYFIKATOR:** CIS Azure 1.2 **Ownership**: Customer
...
**Identyfikator:** CIS Azure 2.2 **Ownership**: Customer (Własność cis Azure 2.2: klient)
...
**Identyfikator:** CIS Azure 2.7 **Ownership**: Customer
...
**Identyfikator:** CIS Azure 2.8 **Ownership**: Customer (Własność platformy Azure CIS 2.8): Klient
...
**Identyfikator:** CIS Azure 2.11 **Własność:** Klient
...
### <a name="ensure-additional-email-addresses-is-configured-with-a-security-contact-email"></a>Upewnij się, że skonfigurowano "Dodatkowe adresy e-mail" przy użyciu kontaktowej wiadomości e-mail dotyczącej zabezpieczeń

**Identyfikator:** CIS Azure 2.13 **Ownership**: Customer
...
### <a name="ensure-that-notify-about-alerts-with-the-following-severity-is-set-to-high"></a>Upewnij się, że ustawienie "Powiadamiaj o alertach o następującej ważności" ma wartość "Wysoka"

**Identyfikator:** CIS Azure 2.14 **Ownership**: Customer
...
### <a name="ensure-that-secure-transfer-required-is-set-to-enabled"></a>Upewnij się, że opcja "Wymagany bezpieczny transfer" ma wartość "Włączone"
...
### <a name="ensure-that-public-access-level-is-set-to-private-for-blob-containers"></a>Upewnij się, że dla kontenerów obiektów blob dla ustawienia "Poziom dostępu publicznego" ustawiono wartość Prywatny

**Identyfikator:** CIS Azure 3.5 **Ownership**: Customer (Własność platformy Azure CIS 3.5): Klient
...
**Identyfikator:** CIS Azure 3.6 **Ownership**: Customer (Własność platformy Azure CIS 3.6): Klient
...
**Identyfikator:** CIS Azure 3.9 **Własność:** Klient
...
**Identyfikator:** CIS Azure 4.1.1 **Własność:** Klient
...
**Identyfikator:** CIS Azure 4.1.2 **Ownership**: Customer (Własność cis Azure 4.1.2): Klient
...
**Identyfikator:** CIS Azure 4.1.3 Ownership ( Własność **platformy** Azure 4.1.3): Klient
...
### <a name="ensure-that-advanced-threat-protection-atp-on-a-sql-server-is-set-to-enabled"></a>Upewnij się, że opcja Advanced Threat Protection (ATP) na serwerze SQL jest ustawiona na wartość "Włączono"
...
### <a name="ensure-that-va-setting-send-scan-reports-to-is-configured-for-a-sql-server"></a>Upewnij się, że dla serwera SQL skonfigurowano ustawienie Va VaS Send scan reports to (Wyślij raporty skanowania do)
...
**Identyfikator:** CIS Azure 4.3.1 Ownership ( Własność **platformy** Azure 4.3.1): Klient
...
**Identyfikator:** CIS Azure 4.3.5 **Ownership**: Customer (Własność platformy Azure CIS 4.3.5): Klient
...
**Identyfikator:** CIS Azure 4.3.6 **Ownership**: Customer (Własność platformy Azure CIS 4.3.6): Klient
...
### <a name="ensure-sql-servers-tde-protector-is-encrypted-with-customer-managed-key"></a>Upewnij się, że funkcji ochrony TDE programu SQL Server jest szyfrowana przy użyciu klucza zarządzanego przez klienta

**Identyfikator:** CIS Azure 4.5 **Ownership**: Customer (Własność platformy Azure CIS 4.5): Klient
...
**Identyfikator:** CIS Azure 5.1.4 **Ownership**( Własność platformy Azure 5.1.4): Klient
...
**Identyfikator:** CIS Azure 5.2.2 Ownership ( Własność **platformy** Azure 5.2.2): Klient
...
### <a name="ensure-that-activity-log-alert-exists-for-create-or-update-network-security-group-rule"></a>Upewnij się, że istnieje alert dziennika aktywności dla tworzenia lub aktualizowania reguły sieciowej grupy zabezpieczeń
...
### <a name="ensure-that-activity-log-alert-exists-for-the-delete-network-security-group-rule"></a>Upewnij się, że istnieje alert dziennika aktywności dla reguły Usuń sieciową grupę zabezpieczeń

**Identyfikator:** CIS Azure 5.2.6 **Ownership**: Customer (Własność platformy Azure CIS 5.2.6): Klient
...
**Identyfikator:** CIS Azure 5.2.7 **Ownership**( Własność platformy Azure 5.2.7): Klient
...
**IDENTYFIKATOR:** CIS Azure 6.1 Ownership : Customer (Własność platformy Azure 6.1 **CIS:** klient)
...
### <a name="ensure-that-unattached-disks-are-encrypted-with-cmk"></a>Upewnij się, że "Niedołączene dyski" są szyfrowane za pomocą szyfrowania cmk

**Identyfikator:** CIS Azure 7.3 **Ownership**: Customer
...
### <a name="ensure-that-only-approved-extensions-are-installed"></a>Upewnij się, że zainstalowane są tylko zatwierdzone rozszerzenia
...
**Identyfikator:** CIS Azure 7.6 **Ownership**: Customer (Własność platformy Azure CIS 7.6): Klient
...
|Nazwa<br /><sub>(Azure Portal)</sub> |Opis |Efekty |Wersja<br /><sub>(GitHub)</sub> |
|---|---|---|---|
|[Key Vault tajne powinny mieć datę wygaśnięcia](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F98728c90-32c7-4049-8429-847dc0f4fe37) |Wpisy tajne powinny mieć zdefiniowaną datę wygaśnięcia i nie powinny być trwałe. Wpisy tajne, które są prawidłowe, na zawsze zapewniają potencjalnej osobie atakującej więcej czasu na jego złamanie. Ustawianie dat wygaśnięcia wpisów tajnych jest zalecanym rozwiązaniem w zakresie zabezpieczeń. |Inspekcja, Odmowa, Wyłączone |[1.0.1-preview](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Key%20Vault/Secrets_ExpirationSet.json) |
...
### <a name="ensure-app-service-authentication-is-set-on-azure-app-service"></a>Upewnij się App Service że uwierzytelnianie jest ustawione na Azure App Service

**Identyfikator:** CIS Azure 9.1 **Ownership**: Customer

|Nazwa<br /><sub>(Azure Portal)</sub> |Opis |Efekty |Wersja<br /><sub>(GitHub)</sub> |
|---|---|---|---|
|[Uwierzytelnianie powinno być włączone w aplikacji interfejsu API](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2Fc4ebc54a-46e1-481a-bee2-d4411e95d828) |Azure App Service Authentication to funkcja, która może uniemożliwić anonimowym żądaniom HTTP dotarcie do aplikacji interfejsu API lub uwierzytelnienie tych, które mają tokeny, zanim dotrą do aplikacji interfejsu API |AuditIfNotExists, Disabled |[1.0.0](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/App%20Service/AppService_Authentication_ApiApp_Audit.json) |
|[Uwierzytelnianie powinno być włączone w aplikacji funkcji](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2Fc75248c1-ea1d-4a9c-8fc9-29a6aabd5da8) |Azure App Service Authentication to funkcja, która może uniemożliwić anonimowym żądaniom HTTP dotarcie do aplikacji funkcji lub uwierzytelnienie tych, które mają tokeny, zanim dotrą do aplikacji funkcji |AuditIfNotExists, Disabled |[1.0.0](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/App%20Service/AppService_Authentication_functionapp_Audit.json) |
...
### <a name="ensure-that-http-version-is-the-latest-if-used-to-run-the-web-app"></a>Upewnij się, że "Wersja HTTP" jest najnowsza, jeśli jest używana do uruchamiania aplikacji internetowej
