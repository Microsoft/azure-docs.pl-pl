...
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Lista regionów, w których należy włączyć Network Watcher|Rozdzielana średnikami lista regionów. Aby wyświetlić pełną listę regionów, Użyj Get-AzLocation. Np.: Wschodnie stany USA; Wschód stany USA 2|
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Log Analytics identyfikator obszaru roboczego, dla którego należy skonfigurować maszyny wirtualne|To jest identyfikator (GUID) obszaru roboczego Log Analytics, dla którego należy skonfigurować maszyny wirtualne.|
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Opcjonalnie: lista obrazów maszyn wirtualnych z systemem Windows, które obsługują agenta Log Analytics do dodania do zakresu inspekcji|Rozdzielana średnikami lista obrazów|
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Opcjonalne: lista obrazów maszyn wirtualnych z systemem Linux, które obsługują agenta Log Analytics do dodania do zakresu inspekcji|Rozdzielana średnikami lista obrazów|
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Najnowsza wersja języka PHP|Najnowsza obsługiwana wersja języka PHP dla App Services|
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Najnowsza wersja języka Java|Najnowsza obsługiwana wersja języka Java dla App Services|
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Najnowsza wersja języka Python systemu Windows|Najnowsza obsługiwana wersja języka Python dla App Services|
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Najnowsza wersja języka Python dla systemu Linux|Najnowsza obsługiwana wersja języka Python dla App Services|
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Lista typów zasobów, dla których włączono obsługę dzienników diagnostycznych|Lista typów zasobów do inspekcji, jeśli nie włączono ustawienia dziennika diagnostycznego. Akceptowalne wartości można znaleźć Azure Monitor w obszarze [schematy dzienników diagnostycznych](../../../azure-monitor/platform/resource-logs-schema.md).|
|\[Wersja zapoznawcza \] : NIST SP 800-171 R2|Przypisywanie zasad|Minimalna wersja protokołu TLS dla serwerów sieci Web systemu Windows|Minimalna wersja protokołu TLS, która powinna być włączona na serwerach sieci Web systemu Windows.|
