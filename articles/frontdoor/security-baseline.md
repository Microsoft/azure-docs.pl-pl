...
**Wskazówki** : nie dotyczy; Drzwi frontonu platformy Azure nie są przeznaczone do wdrażania w sieci prywatnej ani do ich zabezpieczania, ponieważ ta kontrolka jest przeznaczona do opisywania łączności sieciowej i nie ma zastosowania.

**Monitorowanie Azure Security Center** : nie dotyczy

**Odpowiedzialność** : nie ustawiono. Podaj wartość w elemencie roboczym.
...
**Wskazówki** : nie dotyczy — drzwi frontonu platformy Azure nie są przeznaczone do wdrożenia lub zabezpieczonej sieci wirtualnej w celu uzyskania dostępu do sieci prywatnej.
...
**Wskazówki** : Użyj szyfrowania, aby chronić ruch w sieciach zewnętrznych i publicznych, ponieważ ma kluczowe znaczenie dla ochrony danych. Dodatkowo:

- Użyj kontroli dostępu,

- Ochrona danych przed atakami typu "poza pasmem" (takich jak przechwytywanie ruchu) przy użyciu szyfrowania w celu zapewnienia, że osoby atakujące nie mogą łatwo odczytać ani zmodyfikować danych.
- Upewnij się, że wszyscy klienci łączący się z zasobami platformy Azure dla ruchu HTTP mogą negocjować protokół TLS w wersji 1.2 lub nowszej.
- Użyj protokołu SSH (dla systemu Linux) lub RDP/TLS (w systemie Windows) do zdalnego zarządzania zamiast nieszyfrowanych protokołów

- Wyłącz przestarzałe wersje protokołu SSL/TLS/SSH, protokoły i słabe szyfry

Domyślnie platforma Azure udostępnia dane do szyfrowania podczas przesyłania danych między centrami danych platformy Azure. 
...
**Wskazówki** : nie dotyczy; Nie można używać drzwi frontonu platformy Azure do zapewnienia bezpieczeństwa zasobów w procesie zarządzania cyklem życia. Klient jest odpowiedzialny za obsługę atrybutów i konfiguracji sieci zasobów, które są uważane za duże znaczenie. 
