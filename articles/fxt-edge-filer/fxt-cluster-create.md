...
Po nawiązaniu połączenia Zaloguj się przy użyciu nazwy `root` użytkownika i hasła ustawionego podczas uruchamiania węzła po raz pierwszy.  
...
* **Hasło administratora** — Ustaw hasło dla domyślnego użytkownika administracyjnego `admin`.
  ...
  Należy skonfigurować indywidualne konta użytkowników dla każdej osoby, która administruje klastrem, ale nie można usunąć użytkownika `admin`. Zaloguj się tak `admin` , jakby trzeba utworzyć dodatkowych użytkowników.
 
  Hasło `admin` można zmienić na stronie ustawienia**użytkowników** **administracyjnych** > w panelu sterowania klastra. Aby uzyskać szczegółowe informacje, zapoznaj się z dokumentacją **użytkowników** w [podręczniku konfiguracji klastra](https://azure.github.io/Avere/legacy/ops_guide/4_7/html/gui_users.html).
...
Zaloguj się do interfejsu sieci Web przy użyciu nazwy `admin` użytkownika i hasła ustawionego podczas tworzenia klastra.
