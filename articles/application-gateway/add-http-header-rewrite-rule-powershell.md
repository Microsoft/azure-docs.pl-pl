...
W tym przykładzie zestaw do ponownego zapisu zostanie natychmiast skojarzony z podstawową regułą routingu. W przypadku reguły routingu opartej na ścieżce skojarzenie nie będzie domyślnie włączone. Zestaw do ponownego zapisywania można włączyć za pomocą sprawdzania ścieżek, na których należy zastosować za pośrednictwem portalu, lub podając ścieżkę URL mapowania ścieżki określając RewriteRuleSet dla każdej opcji ścieżki.  
