...
Ta linia bazowa zabezpieczeń stosuje wskazówki dotyczące [usługi Azure Security test w wersji 1.0](../security/benchmarks/overview-v1.md) do Virtual Machine Scale Sets. Test porównawczy zabezpieczeń platformy Azure zawiera zalecenia dotyczące sposobu zabezpieczania rozwiązań w chmurze na platformie Azure. zawartość jest pogrupowana według **kontroli zabezpieczeń** zdefiniowanych przez usługę Azure Security test i powiązane ze sobą wskazówki dotyczące Virtual Machine Scale Sets.

> [!NOTE]
> Nie zostały wykluczone żadne **kontrolki** , które mają zastosowanie do Virtual Machine Scale Sets lub dla których odpowiedzialnością jest firma Microsoft. Aby dowiedzieć się, jak Virtual Machine Scale Sets całkowicie mapować do testu porównawczego zabezpieczeń platformy Azure, zobacz **[pełny Virtual Machine Scale Sets pliku mapowania linii bazowej zabezpieczeń](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/virtual-machine-scale-sets-security-baseline-v1.1.xlsx)**.
...
**Wskazówki**: Jeśli korzystasz z zestawu skalowania maszyn wirtualnych do hostowania aplikacji sieci Web, użyj sieciowej grupy zabezpieczeń (sieciowej grupy zabezpieczeń) w podsieci zestawu skalowania maszyn wirtualnych, aby ograniczyć ruch sieciowy, porty i protokoły, które mogą się komunikować. Postępuj zgodnie z najniższym podejściem do sieci podczas konfigurowania sieciowych grup zabezpieczeń, aby zezwalać na wymagany ruch do aplikacji.
...
**Wskazówki**: w przypadku używania zestawu skalowania maszyn wirtualnych do hostowania aplikacji sieci Web można wdrożyć usługę Azure Application Gateway dla aplikacji sieci Web z WŁĄCZONYm protokołem HTTPS/SSL dla zaufanych certyfikatów. Dzięki usłudze Azure Application Gateway kierowanie ruchu w sieci Web do określonych zasobów przez przypisanie odbiorników do portów, tworzenie reguł i Dodawanie zasobów do puli zaplecza, takiej jak zestaw skalowania maszyn wirtualnych itd.
...
- [Informacje o obszarze roboczym Log Analytics](/azure/azure-monitor/logs/get-started-portal)

- [Jak wykonywać niestandardowe zapytania w Azure Monitor](../azure-monitor/logs/log-analytics-tutorial.md)
...
**Wskazówki**: zestaw skalowania maszyn wirtualnych platformy azure i Azure Active Directory (Azure AD) nie mają koncepcji domyślnych haseł. Klient odpowiedzialny za aplikacje innych firm i usługi w portalu Marketplace, które mogą korzystać z domyślnych haseł.
...
- [Jak monitorować maszyny wirtualne na platformie Azure](/azure/azure-monitor/vm/monitor-vm-azur)
...
**Wskazówki**: usługa Azure Virtual Machine Scale Sets może korzystać z automatycznych uaktualnień obrazu systemu operacyjnego. W przypadku bazowych maszyn wirtualnych w zestawie skalowania maszyn wirtualnych można użyć rozszerzenia konfiguracji żądanego stanu (DSC) platformy Azure. Konfiguracja DSC służy do konfigurowania maszyn wirtualnych, gdy są one w trybie online, tak aby działały żądane oprogramowanie.
...
**Wskazówki**: w zależności od typu skryptów, można użyć konfiguracji specyficznych dla systemu operacyjnego lub zasobów innych firm, aby ograniczyć możliwość wykonywania skryptów w ramach zasobów obliczeniowych platformy Azure.
