...
Alternatywnie możesz użyć [Azure Cloud Shell](https://docs.microsoft.com/azure/cloud-shell/overview), który ma `az` interfejs wiersza polecenia platformy Azure i `kubectl` Narzędzia już zainstalowane.
...
Aby Agent Log Analytics mógł komunikować się z usługą, należy ją skonfigurować przy użyciu identyfikatora obszaru roboczego i klucza obszaru roboczego. Aby uzyskać identyfikator i klucz obszaru roboczego, należy utworzyć konto w usłudze <https://mms.microsoft.com> .
...
Po dodaniu identyfikatora i klucza obszaru roboczego do konfiguracji elementu daemonset można zainstalować agenta Log Analytics w klastrze za pomocą `kubectl` narzędzia wiersza polecenia:
