...
3. W tym miejscu zostanie wyświetlony monit o wprowadzenie `wsid` identyfikatora obszaru roboczego log Analytics i `wskey` (klucz podstawowy dla identyfikatora obszaru roboczego). Aby uzyskać oba `wsid` te `wskey` elementy i, musisz utworzyć konto w <https://mms.microsoft.com>usłudze.
   Postępuj zgodnie z instrukcjami, aby utworzyć konto. Po zakończeniu tworzenia konta musisz uzyskać `wsid` swoją rolę i `wskey` kliknąć pozycje **Ustawienia**, a następnie **połączone źródła**, a następnie serwery z **systemem Linux**, jak pokazano poniżej.
