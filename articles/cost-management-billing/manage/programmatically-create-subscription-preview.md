...
Uruchom następujące polecenie [az account create](/cli/azure/ext/subscription/account#-ext-subscription-az-account-create), zastępując wartość `<enrollmentAccountObjectId>` wartością `name` skopiowaną w pierwszym kroku (```747ddfe5-xxxx-xxxx-xxxx-xxxxxxxxxxxx```). Aby określić właścicieli, zobacz [jak uzyskać identyfikatory obiektów użytkowników](grant-access-to-create-subscription.md#userObjectId).
