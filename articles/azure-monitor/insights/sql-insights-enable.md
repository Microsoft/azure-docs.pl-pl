...
description: Włączanie usługi SQL Insights w u Azure Monitor
ms.topic: conceptual
author: bwren
ms.author: bwren
ms.date: 03/15/2021
...
ms.locfileid: "107478339"
---
# <a name="enable-sql-insights-preview"></a>Włączanie usługi SQL Insights (wersja zapoznawcza)
W tym artykule opisano sposób włączania [usługi SQL Insights](sql-insights-overview.md) w celu monitorowania wdrożeń SQL. Monitorowanie jest wykonywane z maszyny wirtualnej platformy Azure, która tworzy połączenie z wdrożeniami SQL i używa dynamicznych widoków zarządzania (DMV) do zbierania danych monitorowania. Przy użyciu profilu monitorowania można kontrolować, które zestawy danych są zbierane, oraz częstotliwość zbierania.

## <a name="create-log-analytics-workspace"></a>Tworzenie obszaru roboczego usługi Log Analytics
Usługa SQL Insights przechowuje swoje dane w co najmniej jednym obszarze roboczym [usługi Log Analytics.](../logs/data-platform-logs.md#log-analytics-workspaces)  Przed włączeniem usługi SQL Insights należy utworzyć [obszar](../logs/quick-create-workspace.md) roboczy lub wybrać istniejący. Pojedynczy obszar roboczy może być używany z wieloma profilami monitorowania, ale obszar roboczy i profile muszą znajdować się w tym samym regionie świadczenia usługi Azure. Aby włączyć funkcje usługi SQL Insights i uzyskać do nich dostęp, musisz mieć rolę współautora [usługi Log Analytics](../logs/manage-access.md) w obszarze roboczym. 

## <a name="create-monitoring-user"></a>Tworzenie użytkownika monitorowania 
Potrzebujesz użytkownika we wdrożeniach SQL, które chcesz monitorować. Wykonaj poniższe procedury dla różnych typów wdrożeń SQL.
...
W zależności od ustawień sieciowych zasobów SQL może być konieczne, aby maszyny wirtualne były umieszczone w tej samej sieci wirtualnej co zasoby SQL, aby można było na nich utworzyć połączenia sieciowe w celu zbierania danych monitorowania.  

## <a name="configure-network-settings"></a>Konfigurowanie ustawień sieciowych
Każdy typ języka SQL oferuje metody bezpiecznego uzyskiwania dostępu do bazy danych SQL przez monitoracyjną maszynę wirtualną.  W poniższych sekcjach o przedstawiono opcje oparte na typie języka SQL.
...
Podczas konfiguracji profilu na potrzeby monitorowania bazy danych SQL musisz mieć jedno z następujących uprawnień Key Vault zasobu, który ma być potrzebny:
...
Nowe zasady dostępu zostaną automatycznie utworzone w ramach tworzenia profilu SQL Monitoring, który używa Key Vault danych. Użyj *ustawienia Zezwalaj na dostęp z Key Vault* sieci.
...
Można na przykład utworzyć jeden profil o nazwie *SQL Production* i inny o nazwie *SQL Staging* z różnymi ustawieniami częstotliwości zbierania danych, danymi do zebrania i obszarem roboczym, do którego mają zostać wysłanie danych. 
...
Po **wprowadzeniu** szczegółów profilu monitorowania kliknij pozycję Utwórz profil monitorowania. Wdrożenie profilu może potrwać do minuty.  Jeśli nowy profil nie jest wyświetlany  w polu kombi Profil monitorowania, kliknij przycisk odświeżania. Powinien on zostać wyświetlony po zakończeniu wdrażania.  Po wybraniu nowego profilu wybierz kartę Zarządzanie profilem, aby dodać maszynę monitorowania, która zostanie skojarzona z profilem. 
...
Wybierz subskrypcję i nazwę maszyny wirtualnej monitorowania. Jeśli używasz usługi Key Vault do przechowywania hasła użytkownika monitorującego, wybierz zasoby usługi Key Vault z tymi wpisami tajnymi i wprowadź adres URL oraz nazwę wpisów tajnych, które mają być używane w parametry połączenia. Zobacz następną sekcję, aby uzyskać szczegółowe informacje na temat identyfikowania parametrów połączenia dla różnych wdrożeń SQL.
...
Aby monitorować pomocniczy element czytelny, dołącz wartość klucz-wartość `ApplicationIntent=ReadOnly` do parametrów połączenia. Usługa SQL Insights obsługuje monitorowanie pojedynczego pomocniczego serwera. Zebrane dane zostaną oznaczone w celu odzwierciedlenia podstawowego lub pomocniczego. 
...
Aby monitorować pomocniczy element czytelny, dołącz wartość klucz-wartość `ApplicationIntent=ReadOnly` do parametrów połączenia. Usługa SQL Insights obsługuje monitorowanie jednego pomocniczego serwera, a zebrane dane zostaną oznaczone w celu odzwierciedlenia danych podstawowych lub pomocniczych. 
...
Jeśli nie widzisz danych, zobacz [Troubleshooting SQL insights (Rozwiązywanie](sql-insights-troubleshoot.md) problemów ze szczegółowych informacji SQL), aby zidentyfikować problem. 
