...
| Klucz | Typ danych | Wartość | Opis |
|--|--|--|--|
| `[metric_collection_settings.collect_kube_system_pv_metrics] enabled =` | Boolean | true lub false | To ustawienie umożliwia zbieranie metryk użycia trwałych woluminów (PV) w przestrzeni nazw polecenia-system. Domyślnie metryki użycia dla woluminów trwałych z trwałymi oświadczeniami woluminów w przestrzeni nazw polecenia-system nie są zbierane. Gdy to ustawienie jest ustawione na wartość `true` , zbierane są metryki użycia PV dla wszystkich przestrzeni nazw. Domyślnie jest to ustawienie `false` . |
