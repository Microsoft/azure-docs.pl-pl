---
title: Przykłady szablonów Menedżer zasobów dla ustawień diagnostycznych
description: Przykładowe szablony Azure Resource Manager do zastosowania Azure Monitor ustawień diagnostycznych do zasobu platformy Azure.
ms.topic: sample
author: bwren
ms.author: bwren
ms.date: 09/11/2020
...
## <a name="diagnostic-setting-for-log-analytics-workspace"></a>Ustawienie diagnostyczne dla Log Analytics obszaru roboczego
Poniższy przykład tworzy ustawienie diagnostyczne dla Log Analytics magazynu obszarów roboczych przez dodanie zasobu typu `Microsoft.OperationalInsights/workspaces/providers/diagnosticSettings` do szablonu. Ten przykład wysyła dane inspekcji dotyczące zapytań wykonywanych w obszarze roboczym do tego samego obszaru roboczego.
...
## <a name="diagnostic-setting-for-azure-storage"></a>Ustawienie diagnostyczne usługi Azure Storage
Poniższy przykład tworzy ustawienie diagnostyczne dla każdego punktu końcowego usługi magazynu, który jest dostępny na koncie magazynu. To ustawienie jest stosowane do poszczególnych usług magazynu, które są dostępne na koncie. Dostępne usługi magazynu zależą od typu konta magazynu. Ten szablon służy do tworzenia ustawień diagnostycznych dla usługi magazynu na koncie tylko wtedy, gdy istnieje dla tego konta. Dla każdej dostępnej usługi ustawienie diagnostyczne włącza metryki transakcji oraz kolekcję dzienników zasobów dla operacji odczytu, zapisu i usuwania.
