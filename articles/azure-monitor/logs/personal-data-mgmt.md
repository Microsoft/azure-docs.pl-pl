...
> [!NOTE]
> Na potrzeby tego artykułu _dane dziennika_ odnoszą się do danych wysyłanych do obszaru roboczego log Analytics, podczas gdy _dane aplikacji_ odwołują się do danych zbieranych przez Application Insights. Jeśli używasz zasobu Application Insights opartego na obszarze roboczym, informacje o danych dziennika będą stosowane, ale jeśli korzystasz z klasycznego zasobu Application Insights, zostaną zastosowane dane aplikacji.
...
* *Dane niestandardowe*: log Analytics zezwala na zbieranie w różnych metodach: dzienniki niestandardowe i pola niestandardowe, [interfejs API modułu zbierającego dane http](../logs/data-collector-api.md) oraz dane niestandardowe zbierane w ramach dzienników zdarzeń systemu. Wszystkie z nich są podatne na dane prywatne i powinny być zbadane w celu sprawdzenia, czy istnieją takie dane.
...
- Aby dowiedzieć się więcej o tym, jak Log Analytics dane są zbierane, przetwarzane i zabezpieczone, zobacz [log Analytics zabezpieczenia danych](../logs/data-security.md).
