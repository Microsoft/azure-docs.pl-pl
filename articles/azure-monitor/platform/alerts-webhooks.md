...
|:--- |:--- |:--- |:--- |
| status |T |Aktywowane, rozwiązane |Stan alertu na podstawie ustawionych warunków. |
| kontekst |T | |Kontekst alertu. |
...
| subscriptionId |T | |Identyfikator subskrypcji platformy Azure. |
| resourceGroupName |T | |Nazwa grupy zasobów dla zasobu, którego to dotyczy. |
| resourceName |T | |Nazwa zasobu, którego dotyczy ten zasób. |
| resourceType |T | |Typ zasobu, którego dotyczy ten zasób. |
| resourceId |T | |Identyfikator zasobu, którego dotyczy ten zasób. |
| resourceRegion |T | |Region lub lokalizacja zasobu, którego to dotyczy. |
| portalLink |T | |Bezpośredni link do strony podsumowania zasobów portalu. |
