...
`Get-AzLog`obsługuje wiele innych parametrów. Więcej informacji `Get-AzLog` można znaleźć w dokumentacji.
...
Możesz tworzyć właściwości poczty e-mail i elementu webhook `New-AzAlertRuleEmail` odpowiednio `New-AzAlertRuleWebhook`przy użyciu i. W oknie polecenia cmdlet reguły alertu Przypisz te właściwości jako akcje do właściwości **Actions** reguły alertu.
...
| Wartość metryki alertu, który został utworzony |\PhysicalDisk (_Total) \Bajty zapisy/s. Aby uzyskać `Get-MetricDefinitions` dokładne nazwy metryk, zobacz polecenie cmdlet |
...
Pełna lista dostępnych opcji dla programu `Get-AzMetricDefinition` jest dostępna w witrynie [Get-MetricDefinitions](https://msdn.microsoft.com/library/mt282458.aspx).
...
Aby usunąć ustawienie automatycznego `Remove-Autoscalesetting` skalowania, można użyć polecenia cmdlet.
...
Możesz utworzyć *profil dziennika* i wyeksportować dane z dziennika aktywności do konta magazynu, aby można było skonfigurować dla niego przechowywanie danych. Opcjonalnie można również przesłać strumieniowo dane do centrum zdarzeń. Ta funkcja jest obecnie dostępna w wersji zapoznawczej i można utworzyć tylko jeden profil dziennika na subskrypcję. Do tworzenia profilów dzienników i zarządzania nimi można użyć następujących poleceń cmdlet z bieżącą subskrypcją. Możesz również wybrać określoną subskrypcję. Mimo że program PowerShell domyślnie jest bieżącą subskrypcją, można zawsze zmienić ten `Set-AzContext`program za pomocą polecenia. Dziennik aktywności można skonfigurować w taki sposób, aby dane były kierowane do dowolnego konta magazynu lub centrum zdarzeń w ramach danej subskrypcji. Dane są zapisywane jako pliki obiektów BLOB w formacie JSON.
