...
1. Wybierz przedział czasu, do którego chcesz ocenić określony warunek, przy użyciu opcji [**okres**](../platform/alerts-unified-log.md#query-time-range) .

1. Alerty dzienników mogą opierać się na dwóch typach [**miar**](../platform/alerts-unified-log.md#measure):
...
1. W przypadku logiki alertów pomiarów metryk można opcjonalnie określić sposób [dzielenia alertów według wymiarów](../platform/alerts-unified-log.md#split-by-alert-dimensions) przy użyciu opcji **Agreguj dla** . Wyrażenie grupowania wierszy musi być unikatowe i posortowane.
...
1. Następnie na podstawie danych w wersji zapoznawczej Ustaw [ **operatora**, **wartość progową**](../platform/alerts-unified-log.md#threshold-and-operator)i [**częstotliwość**](../platform/alerts-unified-log.md#frequency).

1. Opcjonalnie można również ustawić [liczbę naruszeń wyzwalających alert](../platform/alerts-unified-log.md#number-of-violations-to-trigger-alert) przy użyciu **łącznych lub kolejnych naruszeń**.
...
1. Wybierz, jeśli chcesz pominąć akcje reguły przez czas po uruchomieniu alertu, użyj opcji [**pomijania alertów**](../platform/alerts-unified-log.md#state-and-resolving-alerts) . Zasada będzie nadal działać i tworzyć alerty, ale akcje nie zostaną wyzwolone w celu uniemożliwienia szumu. Wartość wyciszania akcji musi być większa niż częstotliwość obowiązywania alertu.

    ![Pomijaj alerty dla alertów dziennika](media/alerts-log/AlertsPreviewSuppress.png)

1. Określ, czy reguła alertu powinna wyzwalać co najmniej jedną [**grupę akcji**](../platform/action-groups.md#webhook) po spełnieniu warunku alertu.
...
1. Rozpocznij od karty **warunek** :
    ...
    1. Sprawdź, czy wartość [**miary**](../platform/alerts-unified-log.md#measure), [**typ agregacji**](../platform/alerts-unified-log.md#aggregation-type)i [**stopień szczegółowości agregacji**](../platform/alerts-unified-log.md#aggregation-granularity) są poprawne. 
    ...
    1. W razie konieczności wybierz pozycję [podział alertów według wymiarów](../platform/alerts-unified-log.md#split-by-alert-dimensions): 
    ...
    1. Następnie na podstawie danych w wersji zapoznawczej Ustaw **logikę alertów**. [ **Operator**, **wartość progowa**](../platform/alerts-unified-log.md#threshold-and-operator)i [**częstotliwość**](../platform/alerts-unified-log.md#frequency).

        ![Podgląd wykresu z logiką progu i alertu](media/alerts-log/chart-and-alert-logic.png)

    1. Opcjonalnie możesz ustawić [**liczbę naruszeń, aby wyzwolić alert**](../platform/alerts-unified-log.md#number-of-violations-to-trigger-alert) w sekcji **Opcje zaawansowane** .
    
        ![Opcje zaawansowane](media/alerts-log/advanced-options.png)

1. Na karcie **Akcje** wybierz lub Utwórz wymagane [grupy akcji](../platform/action-groups.md).

    ![Karta akcje](media/alerts-log/actions-tab.png)

1. Na karcie **szczegóły** Zdefiniuj **szczegóły reguły alertu** i **szczegóły projektu**. Opcjonalnie można określić, czy nie **uruchamiać teraz** ani [**wyciszać akcji**](../platform/alerts-unified-log.md#state-and-resolving-alerts) przez okres po uruchomieniu reguły alertu.
...
1. W obszarze Zarządzanie alertami są wyświetlane wszystkie alerty, które zostały wyzwolone. [Dowiedz się więcej o zarządzaniu alertami](alerts-managing-alert-instances.md).
    ...
    > [!NOTE]
    > Reguły alertów dziennika są obecnie [bezstanowe i nie są rozwiązywane](../platform/alerts-unified-log.md#state-and-resolving-alerts).
...
* Dowiedz się więcej o [alertach dzienników](../platform/alerts-unified-log.md).
