...
|  :------| :-------------|  
|  AcquireDiskLeaseFailed  |  Nie można uzyskać dzierżawy podczas tworzenia dysku{0}"" przy użyciu obiektu {1}BLOB z identyfikatorem URI. Obiekt BLOB jest już używany.  |  
...
|  ArtifactNotFound  |  Nie można odnaleźć rozszerzenia maszyny wirtualnej{0}z wydawcą{1}"" i typem "" w lokalizacji{2}"".  |
|  ArtifactNotFound  |  Nie można odnaleźć rozszerzenia{0}z wydawcy ""{1}, typu "" i wersji programu{2}obsługi typu "" w repozytorium rozszerzeń.  |
|  ArtifactVersionNotFound  |  W repozytorium artefaktów nie znaleziono wersji spełniającej żądaną wersję "{0}".  |
|  ArtifactVersionNotFound  |  W repozytorium artefaktów nie znaleziono wersji spełniającej żądaną wersję "{0}" dla rozszerzenia maszyny wirtualnej z wydawcą "{1}"{2}i typem "".  |
|  AttachDiskWhileBeingDetached  |  Nie można dołączyć dysku danych{0}"" do maszyny{1}wirtualnej "", ponieważ dysk jest obecnie odłączony. Zaczekaj na całkowite odłączenie dysku, a następnie spróbuj ponownie.  |
...
|  BadRequest  |  Wiele VMExtensions na procedurę obsługi nie jest obsługiwana dla typu{0}systemu operacyjnego "". VMExtension "{1}" z obsługą{2}"" została już dodana lub określona w danych wejściowych.  |
|  BadRequest  |  Operacja "{0}" nie jest obsługiwana dla zasobu "{1}" z dyskami zarządzanymi.  |
|  CertificateImproperlyFormatted  |  Reprezentacja JSON klucza tajnego uzyskana z {0} zawiera pole danych, które nie jest poprawnie sformatowanym plikiem pfx lub podane hasło nie dekoduje poprawnie pliku PFX.  |
...
|  ConflictingUserInput  |  Nie można{0}dołączyć dysku "", ponieważ dysk jest już własnością maszyny wirtualnej "{1}".  |
|  ConflictingUserInput  |  Źródłowe i docelowe grupy zasobów są takie same.  |
|  ConflictingUserInput  |  Źródłowe i docelowe konta magazynu dla {0} dysku są różne.  |
|  ContainerAlreadyOnLease  |  Istnieje już dzierżawa w kontenerze magazynu przechowującym obiekt BLOB z identyfikatorem URI {0}.  |
|  CrossSubscriptionMoveWithKeyVaultResources  |  Żądanie Move Resources zawiera zasoby magazynu kluczy, do których odwołuje się {0}jeden lub więcej s w żądaniu. Nie jest to obecnie obsługiwane w przypadku przenoszenia między subskrypcjami. Sprawdź szczegóły błędu dotyczące identyfikatorów zasobów magazynu kluczy.  |
...
|  DiskBlobAlreadyInUseByAnotherDisk  |  Obiekt {0} BLOB jest już używany przez inny dysk należący do maszyny wirtualnej{1}"". Można przeanalizować metadane obiektu BLOB dla informacji referencyjnych dysku.  |
|  DiskBlobNotFound  |  Nie można znaleźć obiektu BLOB dysku VHD {0} z identyfikatorem{1}URI dla dysku "".  |
|  DiskBlobNotFound  |  Nie można odnaleźć obiektu BLOB VHD z {0}identyfikatorem URI.  |
|  DiskEncryptionKeySecretMissingTags  |  {0}wpis tajny nie {1} ma tagów. Zaktualizuj wersję wpisu tajnego, Dodaj wymagane znaczniki i ponów próbę.  |
|  DiskEncryptionKeySecretUnwrapFailed  |  Odwinięcie wartości klucza {0} tajnego przy {1} użyciu klucza nie powiodło się.  |
|  DiskImageNotReady  |  Obraz {0} dysku jest w {1} stanie. Spróbuj ponownie, gdy obraz będzie gotowy.  |
...
|  ImageBlobNotFound  |  Nie można znaleźć obiektu BLOB dysku VHD {0} z identyfikatorem{1}URI dla dysku "".  |
|  ImageBlobNotFound  |  Nie można odnaleźć obiektu BLOB VHD z {0}identyfikatorem URI.  |
|  IncorrectDiskBlobType  |  Obiekty blob na dyskach mogą być typu stronicowego obiektu BLOB. Obiekt {0} BLOB dla dysku{1}"" jest typu blokowego obiektu BLOB.  |
|  IncorrectDiskBlobType  |  Obiekty blob na dyskach mogą być typu stronicowego obiektu BLOB. Obiekt {0} BLOB jest typu "{1}".  |
|  IncorrectImageBlobType  |  Obiekty blob na dyskach mogą być typu stronicowego obiektu BLOB. Obiekt {0} BLOB dla dysku{1}"" jest typu blokowego obiektu BLOB.  |
|  IncorrectImageBlobType  |  Obiekty blob na dyskach mogą być typu stronicowego obiektu BLOB. Obiekt {0} BLOB jest typu "{1}".  |
|  InternalOperationError  |  Nie można rozpoznać konta {0}magazynu. Upewnij się, że został on utworzony za pomocą dostawcy zasobów magazynu w tej samej lokalizacji co zasób obliczeniowy.  |
...
|  Nieprawidłowy parametr  |  Nazwa {0} kontenera jest nieprawidłowa. Nazwy kontenerów muszą mieć długość 3-63 znaków i mogą zawierać tylko małe znaki alfanumeryczne i łącznik. Łącznik musi być poprzedzony znakiem alfanumerycznym.  |
|  Nieprawidłowy parametr  |  Nazwa {0} kontenera w adresie URL {1} jest nieprawidłowa. Nazwy kontenerów muszą mieć długość 3-63 znaków i mogą zawierać tylko małe znaki alfanumeryczne i łącznik. Łącznik musi być poprzedzony znakiem alfanumerycznym.  |
...
|  Nieprawidłowy parametr  |  Dysk o nazwie "{0}" już używa tego samego adresu URL {1}dysku VHD.  |
|  Nieprawidłowy parametr  |  Określona liczba {0} domen błędów musi należeć do zakresu {1} od do {2}.  |
|  Nieprawidłowy parametr  |  Typ {0} licencji jest nieprawidłowy. Prawidłowe typy licencji to: Windows_Client lub Windows_Server, z uwzględnieniem wielkości liter.  |
|  Nieprawidłowy parametr  |  Nazwa hosta systemu Linux nie {0} może być dłuższa niż znaków ani zawierać następujących {1}znaków:.  |
|  Nieprawidłowy parametr  |  Ścieżka docelowa kluczy publicznych SSH jest obecnie ograniczona do wartości {0} domyślnej ze względu na znany problem w agencie aprowizacji systemu Linux.  |
...
|  Nieprawidłowy parametr  |  Nazwa obiektu BLOB w {0} adresie URL musi{1}kończyć się rozszerzeniem "".  |
...
|  Nieprawidłowy parametr  |  Dysk w jednostce LUN {0} już istnieje.  |
|  Nieprawidłowy parametr  |  Nie można utworzyć maszyny wirtualnej, ponieważ żądany rozmiar {0} nie jest dostępny w klastrze, w którym zestaw dostępności jest aktualnie przydzielony. Dostępne są następujące rozmiary: {1}. Więcej informacji na temat strategii zmiany rozmiarów maszyny https://aka.ms/azure-resizevmwirtualnej można znaleźć pod adresem.  |
|  Nieprawidłowy parametr  |  Żądany rozmiar {0} maszyny wirtualnej nie jest dostępny w bieżącym regionie. Dostępne są rozmiary w bieżącym regionie: {1}. Dowiedz się więcej na temat dostępnych rozmiarów maszyn wirtualnych w każdym https://aka.ms/azure-regionsregionie.  |
|  Nieprawidłowy parametr  |  Żądany rozmiar {0} maszyny wirtualnej nie jest dostępny w bieżącym regionie. Dowiedz się więcej na temat dostępnych rozmiarów maszyn wirtualnych w każdym https://aka.ms/azure-regionsregionie.  |
|  Nieprawidłowy parametr  |  Nazwa użytkownika administratora systemu Windows nie może być {0} dłuższa niż znaków, kończyć się kropką (.) lub zawierać następujące znaki: {1}.  |
|  Nieprawidłowy parametr  |  Nazwa komputera z systemem Windows nie może {0} być dłuższa niż znaków, być całkowicie numeryczna ani zawierać następujących {1}znaków:.  |
...
|  NotFound  |  Nie można znaleźć {0} zestawu dostępności.  |
...
|  NotSupported  |  Typ licencji to {0}, ale obiekt BLOB {1} obrazu nie znajduje się w środowisku lokalnym.  |
|  OperationNotAllowed  |  Nie można {0} usunąć zestawu dostępności. Przed usunięciem zestawu dostępności upewnij się, że nie zawiera on żadnej maszyny wirtualnej.  |
...
|  OperationNotAllowed  |  Nie można {0} utworzyć zasobu z obrazu {1} do momentu pomyślnego utworzenia obrazu.  |
...
|  OperationNotAllowed  |  Maksymalna liczba dysków z danymi, które mogą być dołączone do maszyny wirtualnej o tym rozmiarze, {0}to.  |
...
|  OperationNotAllowed  |  Operacja "{0}" jest niedozwolona w rozszerzeniu maszyny{1}wirtualnej "", ponieważ jest oznaczona do usunięcia. Można ponowić próbę wykonania operacji usuwania (lub poczekać na jej zakończenie).  |
...
|  OperationNotAllowed  |  Operacja "{0}" jest niedozwolona na maszynie wirtualnej{1}"", ponieważ maszyna wirtualna{2}ma rozszerzenie "" w stanie niepowodzenia.  |
...
|  OperationNotAllowed  |  Rozszerzenia zestawu skalowania maszyn wirtualnych programu{0}obsługi "" można dodać tylko w momencie tworzenia zestawu skalowania maszyn wirtualnych.  |
|  OperationNotAllowed  |  Rozszerzenia zestawu skalowania maszyn wirtualnych programu{0}obsługi "" można usunąć tylko w czasie usuwania zestawu skalowania maszyn wirtualnych.  |
|  OperationNotAllowed  |  Maszyna wirtualna{0}"" korzysta już z dysków zarządzanych.  |
|  OperationNotAllowed  |  Maszyna wirtualna{0}"" należy do "klasycznego" zestawu{1}dostępności "". Zaktualizuj zestaw dostępności, aby używał "wyrównanej" jednostki SKU, a następnie ponów próbę konwersji.  |
...
|  OperationNotAllowed  |  Bieżąca operacja zmienia stan zużywanej maszyny {0} wirtualnej na. {1} Wykonaj operację {2} za jakiś czas.  |
|  OperationNotAllowed  |  Nie można dodać lub zaktualizować maszyny wirtualnej. Żądany rozmiar {0} maszyny wirtualnej może nie być dostępny w istniejącej jednostce alokacji. Więcej informacji na temat strategii zmiany rozmiarów maszyny https://aka.ms/azure-resizevmwirtualnej można znaleźć pod adresem.  |
|  OperationNotAllowed  |  Nie można zmienić rozmiaru maszyny wirtualnej, ponieważ żądany {0} rozmiar nie jest dostępny w klastrze, w którym zestaw dostępności jest aktualnie przydzielony. Dostępne są następujące rozmiary: {1}. Więcej informacji na temat strategii zmiany rozmiarów maszyny https://aka.ms/azure-resizevmwirtualnej można znaleźć pod adresem.  |
|  OperationNotAllowed  |  Nie można zmienić rozmiaru maszyny wirtualnej, ponieważ żądany {0} rozmiar nie jest dostępny w klastrze, w którym maszyna wirtualna jest aktualnie przypisana. Aby zmienić rozmiar maszyny wirtualnej {1} w celu cofnięcia alokacji (jest to operacja zatrzymania w Azure Portal), a następnie spróbuj ponownie wykonać operację zmiany rozmiaru. Więcej informacji na temat strategii zmiany rozmiarów maszyny https://aka.ms/azure-resizevmwirtualnej można znaleźć pod adresem.  |
|  OSProvisioningClientError  |  Inicjowanie obsługi systemu operacyjnego nie powiodło się{0}dla maszyny wirtualnej "", ponieważ obecnie jest inicjowana obsługa systemu operacyjnego gościa.  |
|  OSProvisioningClientError  |  Inicjowanie obsługi administracyjnej systemu operacyjnego dla maszyny{0}wirtualnej "" nie powiodło się. Szczegóły błędu: {1} upewnij się, że obraz został prawidłowo przygotowany (uogólniony). <ul><li>Instrukcje dla systemu Windows:https://azure.microsoft.com/documentation/articles/virtual-machines-windows-upload-image/  </li></ul> |
...
|  OSProvisioningInternalError  |  Inicjowanie obsługi systemu operacyjnego nie powiodło się{0}dla maszyny wirtualnej "" z powodu błędu wewnętrznego.  |
...
|  StorageAccountLocationMismatch  |  Nie można rozpoznać konta {0}magazynu. Upewnij się, że został on utworzony za pomocą dostawcy zasobów magazynu w tej samej lokalizacji co zasób obliczeniowy.  |
|  StorageAccountNotFound  |  Nie znaleziono {0} konta magazynu. Upewnij się, że konto magazynu nie zostało usunięte i należy do tej samej lokalizacji platformy Azure co maszyna wirtualna.  |
|  StorageAccountNotRecognized  |  Użyj konta magazynu zarządzanego przez dostawcę zasobów magazynu. Użycie nie {0} jest obsługiwane.  |
|  StorageAccountOperationInternalError  |  Wystąpił błąd wewnętrzny podczas uzyskiwania dostępu {0}do konta magazynu.  |
|  StorageAccountSubscriptionMismatch  |  Konto {0} magazynu nie należy do subskrypcji {1}.  |
|  StorageAccountTooBusy  |  Konto magazynu "{0}" jest obecnie zajęte. Rozważ użycie innego konta.  |
|  StorageAccountTypeNotSupported  |  Użycie {0} {1} dysku, które jest kontem magazynu obiektów BLOB. Spróbuj ponownie za pomocą konta magazynu ogólnego przeznaczenia.  |
|  StorageAccountTypeNotSupported  |  Konto {0} magazynu jest {1} typu. Diagnostyka rozruchu obsługuje {2} typy kont magazynu.  <ul><li>Ten błąd występuje, jeśli używasz konta usługi Premium Storage na potrzeby diagnostyki rozruchu. Aby uzyskać więcej informacji, zobacz [jak korzystać z diagnostyki rozruchu](boot-diagnostics.md). </li></ul> |
|  SubscriptionNotAuthorizedForImage  |  Subskrypcja nie jest autoryzowana.  |
|  TargetDiskBlobAlreadyExists  |  Obiekt {0} BLOB już istnieje. Podaj inny identyfikator URI obiektu BLOB, aby utworzyć nowy pusty dysk danych "{1}".  |
|  TargetDiskBlobAlreadyExists  |  Nie można kontynuować operacji przechwytywania, ponieważ obiekt {0} BLOB obrazu docelowego już istnieje i flaga zastąpienia obiektów BLOB wirtualnego dysku twardego nie została ustawiona. Usuń obiekt BLOB lub Ustaw flagę w celu zastąpienia obiektów BLOB wirtualnego dysku twardego, a następnie ponów próbę.  |
|  TargetDiskBlobAlreadyExists  |  Nie można kontynuować operacji przechwytywania, ponieważ docelowy {0} obiekt BLOB obrazu ma aktywną dzierżawę.   |
|  TargetDiskBlobAlreadyExists  |  Obiekt {0} BLOB już istnieje. Podaj inny identyfikator URI obiektu BLOB jako element docelowy dla dysku{1}"".  |
...
|  VHDSizeInvalid  |  Określona wartość rozmiaru dysku {0} dla dysku "{1}" z obiektem BLOB {2} jest nieprawidłowa. Rozmiar dysku musi mieścić się {3} w {4}przedziale od do.  |
...
|  VMExtensionHandlerNonTransientError  |  Procedura obsługi{0}"" zgłosiła niepowodzenie dla rozszerzenia{1}maszyny wirtualnej "" z kodem{2}błędu terminalu "" i komunikatem o błędzie: "{3}"  |
|  VMExtensionManagementInternalError  |  Wystąpił błąd wewnętrzny podczas przetwarzania rozszerzenia maszyny{0}wirtualnej "".  |
...
|  VMExtensionProvisioningTimeout  |  Przekroczono limit czasu aprowizacji rozszerzenia maszyny{0}wirtualnej "". Instalacja rozszerzenia może być zbyt długa lub nie można uzyskać stanu rozszerzenia.  |
|  VMMarketplaceInvalidInput  |  Tworzenie maszyny wirtualnej na podstawie obrazu spoza witryny Marketplace nie wymaga informacji o planie, Usuń informacje o planie w żądaniu. Nazwa dysku systemu operacyjnego {0}to.  |
|  VMMarketplaceInvalidInput  |  Informacje o zakupie nie są zgodne. Nie można wdrożyć z obrazu portalu Marketplace. Nazwa dysku systemu operacyjnego {0}to.  |
|  VMMarketplaceInvalidInput  |  Tworzenie maszyny wirtualnej na podstawie obrazu z portalu Marketplace wymaga informacji o planie w żądaniu. Nazwa dysku systemu operacyjnego {0}to.  |
|  VMNotFound  |  Nie można znaleźć{0}maszyny wirtualnej "".  |
|  VMRedeploymentFailed  |  Ponowne wdrażanie{0}maszyny wirtualnej "" nie powiodło się z powodu błędu wewnętrznego. Spróbuj ponownie później.  |
