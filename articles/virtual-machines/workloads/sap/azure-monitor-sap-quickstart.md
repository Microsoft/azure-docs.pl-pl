---
title: Wdrażanie Azure Monitor rozwiązań SAP przy użyciu Azure Portal
description: Wdrażanie Azure Monitor rozwiązań SAP przy użyciu Azure Portal
author: sameeksha91
ms.author: sakhare
ms.topic: how-to
...
Azure Monitor dla rozwiązań SAP zasoby można tworzyć za pomocą [Azure Portal](https://azure.microsoft.com/features/azure-portal). Ta metoda udostępnia oparty na przeglądarce interfejs użytkownika do wdrażania Azure Monitor dla rozwiązań SAP i konfigurowania dostawców.
...
1. Wybierz **Azure Monitor dla rozwiązań SAP** z **Azure Marketplace**.
   ...
   :::image type="content" source="./media/azure-monitor-sap/azure-monitor-quickstart-1.png" alt-text="Obraz przedstawia wybieranie oferty Azure Monitor rozwiązań SAP z Azure Marketplace." lightbox="./media/azure-monitor-sap/azure-monitor-quickstart-1.png":::
...
1. Wybierz **kartę Dostawca,** aby dodać dostawców, których chcesz skonfigurować. Możesz dodać wielu dostawców jeden po drugim lub dodać ich po wdrożeniu zasobu monitorowania. 
   ...
   :::image type="content" source="./media/azure-monitor-sap/azure-monitor-quickstart-3.png" alt-text="Pokazuje kartę dostawcy, aby dodać kolejnych dostawców do Azure Monitor dla rozwiązań SAP." lightbox="./media/azure-monitor-sap/azure-monitor-quickstart-3.png":::

2. Wybierz **pozycję Dodaj dostawcę** i **SAP HANA** z listy rozwijanej. 

   > [!IMPORTANT]
   > Upewnij się SAP HANA że dostawca usługi jest skonfigurowany dla SAP HANA "głównego".
...
1. Wybierz **pozycję Klaster wysokiej dostępności (Pacemaker)** z listy rozwijanej. 
   ...
   > [!IMPORTANT]
   > Aby skonfigurować dostawcę klastra wysokiej dostępności (Pacemaker), upewnij się, ha_cluster_provider jest zainstalowany w każdym węźle. Aby uzyskać więcej informacji, zobacz [Temat eksporterów klastrów ha](https://github.com/ClusterLabs/ha_cluster_exporter#installation)
...
>[!IMPORTANT]
> Aby skonfigurować dostawcę systemu operacyjnego (Linux), upewnij się, że na każdym hoście (BareMetal lub maszynie wirtualnej), który chcesz monitorować, jest zainstalowana najnowsza wersja programu Node_Exporter. Użyj tego [linku] ( https://prometheus.io/download/#node_exporter) , aby znaleźć najnowszą wersję. Aby uzyskać więcej informacji, zobacz [Node_Exporter](https://github.com/prometheus/node_exporter)
...
>[!Tip]
> Jeśli host systemu Linux jest maszyną wirtualną platformy Azure, upewnij się, że wszystkie odpowiednie sieciowe sieci zezwalają na ruch przychodzący na porcie 9100 z sieci wirtualnej "VirtualNetwork" jako źródło.
...
2. Wybierz **pozycję Dodaj dostawcę** i **Microsoft SQL Server** z listy rozwijanej. 

3. Wypełnij pola, korzystając z informacji skojarzonych z Microsoft SQL Server. 
...
#### <a name="pre-requisites-for-adding-netweaver-provider"></a>Wymagania wstępne dotyczące dodawania dostawcy NetWeaver
...
1. Otwieranie połączenia z graficznym interfejsem użytkownika oprogramowania SAP z serwerem SAP
2. Logowanie przy użyciu konta administracyjnego
3. Wykonywanie transakcji RZ10
4. Wybierz odpowiedni profil (DOMYŚLNY. PFL)
5. Wybierz pozycję "Rozszerzona konserwacja" i kliknij pozycję Zmień 
6. Zmodyfikuj wartość parametru, którego dotyczy problem "service/protectedwebmethods", na "SDEFAULT -GetQueueStatistic –ABAPGetWPTable –EnqGetStatistic –GetProcessList" na zalecane ustawienie, a następnie kliknij przycisk Kopiuj
7. Wstecz i wybierz pozycję Profil >Zapisz
...
2.  Na stronie Azure Portal w obszarze AMS wybierz pozycję Dodaj dostawcę i z listy rozwijanej wybierz pozycję SAP NetWeaver
