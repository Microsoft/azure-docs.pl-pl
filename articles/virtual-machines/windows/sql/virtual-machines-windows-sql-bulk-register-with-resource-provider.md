...
W tym artykule opisano, jak przeprowadzić zbiorczą rejestrację SQL Server maszyny wirtualnej na platformie Azure za pomocą dostawcy zasobów maszyny wirtualnej SQL `Register-SqlVMs` przy użyciu polecenia cmdlet programu PowerShell.
...
Otwórz Terminal programu PowerShell w środowisku administracyjnym i przejdź do lokalizacji `RegisterSqlVMs.psm1` , w której zapisano plik. Następnie uruchom następujące polecenie cmdlet programu PowerShell w celu zaimportowania skryptu jako modułu: 
...
Raport i plik dziennika są generowane za każdym razem, `Register-SqlVMs` gdy polecenie cmdlet jest używane. 
