...
Użyj następującego polecenia, aby utworzyć grupę `<resourceGroupName>` zasobów w regionie. Zamień `<resourceGroupName>` na wybraną nazwę. Korzystamy z `East US 2` tego samouczka. Aby uzyskać więcej informacji, zobacz poniższy [Przewodnik Szybki Start](../quick-create-cli.md).
...
Następnym krokiem jest utworzenie zestawu dostępności. Uruchom następujące polecenie w Azure Cloud Shell i Zastąp `<resourceGroupName>` wartość nazwą grupy zasobów. Wybierz nazwę dla `<availabilitySetName>`.
...
> [!IMPORTANT]
> Domyślny obraz tworzony za pomocą powyższego polecenia domyślnie tworzy dysk o 32 GB systemu operacyjnego. W przypadku tej instalacji domyślnej można było za mało miejsca. Do powyższego `az vm create` polecenia można użyć następującego parametru, aby utworzyć dysk systemu operacyjnego z 128 GB na przykład: `--os-disk-size-gb 128`.
>
...
1. Użyj poniższego polecenia, aby otworzyć plik Hosts i skonfigurować rozpoznawanie nazw hostów. Aby uzyskać więcej informacji, zobacz [Konfigurowanie usługi AG](/sql/linux/sql-server-linux-availability-group-configure-ha#prerequisites) na konfigurowaniu pliku Hosts.
    ...
    W edytorze **VI** wpisz `i` polecenie, aby wstawić tekst, a w pustym wierszu Dodaj **prywatny adres IP** odpowiedniej maszyny wirtualnej. Następnie Dodaj nazwę maszyny wirtualnej po miejscu obok adresu IP. Każdy wiersz powinien mieć oddzielny wpis.
...
### <a name="register-a-new-application-in-azure-active-directory"></a>Zarejestruj nową aplikację w Azure Active Directory
 ...
 5. Wprowadź **nazwę** `<resourceGroupName>-app`, na przykład, wybierz **konta tylko w tym katalogu organizacji**
...
Przypisz rolę `Linux Fence Agent Role-<username>` niestandardową utworzoną w ostatnim kroku do nazwy głównej usługi. Nie używaj już roli właściciela!
...
- `<servicePrincipalPassword>` Zastąp wartość wartością z wpisu tajnego klienta.
- Zastąp `<resourceGroupName>` wartość with grupy zasobów z Twojej subskrypcji, która została użyta w tym samouczku.
- Zastąp `<tenantID>` wartość i `<subscriptionId>` z subskrypcji platformy Azure.
...
1. Połącz się ze **wszystkimi węzłami** przy użyciu narzędzia SQL Server Management Studio (SSMS) lub polecenia SQL. Uruchom następujące polecenia, aby włączyć sesję AlwaysOn_health i utworzyć klucz główny:
    ...
    - Zastąp `<Master_Key_Password>` wartość własnym hasłem.
...
1. Połącz się z repliką podstawową przy użyciu programu SSMS lub CMD. Poniższe polecenia spowodują utworzenie certyfikatu przy `/var/opt/mssql/data/dbm_certificate.cer` użyciu klucza `var/opt/mssql/data/dbm_certificate.pvk` prywatnego w replice SQL Server głównej:

    - Zastąp `<Private_Key_Password>` wartość własnym hasłem.
...
1. Skopiuj dwa pliki, które zostały utworzone w tej samej lokalizacji na wszystkich serwerach, które będą hostować repliki dostępności.
    ...
    - `<username>` Zastąp `<VM2>` wartości i nazwą użytkownika i docelową nazwą maszyny wirtualnej, której używasz.
    - Uruchom to polecenie dla wszystkich replik pomocniczych.

    > [!NOTE]
    > Nie trzeba uruchamiać `sudo -i`programu, który zapewnia środowisko główne. Po prostu można uruchomić `sudo` polecenie przed każdym poleceniem, tak jak wcześniej w tym samouczku.
...
- Zastąp `<VM1>`wartości `<VM2>`, i `<VM3>` wartościami nazw wystąpień SQL Server, które obsługują repliki.
...
Jeśli `synchronization_state_desc` lista jest zsynchronizowana `db1`z, oznacza to, że repliki są synchronizowane. Serwery pomocnicze są `db1` wyświetlane w replice podstawowej.
...
1. Użyj następującego polecenia, aby utworzyć zasób `ag_cluster` w grupie `ag1`dostępności.
...
1. Użyj dostępnego statycznego adresu IP z sieci, aby utworzyć zasób wirtualnego adresu IP. Można go znaleźć za pomocą narzędzia `nmap`poleceń.
...
1. Uruchom następujące polecenie, aby ręcznie przełączyć replikę podstawową `<VM2>`do trybu failover. Zamień `<VM2>` na wartość nazwy serwera.
...
Możesz testować STONITH, uruchamiając następujące polecenie. Spróbuj uruchomić poniższe polecenie w programie `<VM1>` dla `<VM3>`programu.
...
> [Samouczek: Konfigurowanie odbiornika grupy dostępności dla SQL Server na maszynach wirtualnych RHEL na platformie Azure](sql-server-linux-rhel-ha-listener-tutorial.md)
