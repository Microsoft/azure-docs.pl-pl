---
title: Uruchamianie, monitorowanie i anulowanie przebiegów trenowania w języku Python
...
1. Wybierz **pozycję Ustawienia diagnostyczne,** a następnie **wybierz pozycję + Dodaj ustawienie diagnostyczne.**
    ...
    ![Zrzut ekranu przedstawiający ustawienia diagnostyczne dla powiadomień e-mail](./media/how-to-manage-runs/diagnostic-setting.png)
...
1. W ustawieniu diagnostycznym 
    ...
    ![Gdzie zapisać powiadomienie e-mail](./media/how-to-manage-runs/log-location.png)

1. Na karcie **Dzienniki** dodaj nową **regułę alertu**. 

    ![Nowa reguła alertu](./media/how-to-manage-runs/new-alert-rule.png)

1. Zobacz, [jak tworzyć alerty dzienników i zarządzać](../azure-monitor/alerts/alerts-log.md)nimi przy użyciu Azure Monitor .
...
Przejdź do strony **Szczegóły uruchomienia** dla swojego uruchomienia i wybierz ikonę edycji lub ołówka, aby dodać, edytować lub usunąć opisy przebiegów. Aby utrwalić zmiany na liście uruchomień, zapisz zmiany w istniejącym widoku niestandardowym lub nowym widoku niestandardowym. Opisy przebiegów są obsługiwane w formacie Markdown, który umożliwia osadzane obrazy i łączenie głębokie, jak pokazano poniżej.

:::image type="content" source="media/how-to-manage-runs/run-description.gif" alt-text="Zrzut ekranu: tworzenie opisu uruchomienia"::: 
...
Utwórz podrzędne przebiegi, aby zgrupować powiązane przebiegi, na przykład dla różnych iteracji dostrajania hiperparametrów.

> [!NOTE]
> Przebiegi podrzędne można tworzyć tylko przy użyciu zestawu SDK.

W tym przykładzie kodu użyto skryptu , aby utworzyć partię pięciu przebiegów podrzędnych z przesłanego uruchomienia `hello_with_children.py` przy użyciu [`child_run()`](/python/api/azureml-core/azureml.core.run%28class%29#child-run-name-none--run-id-none--outputs-none-) metody :
...
> [!NOTE]
> Gdy przejdą poza zakres, przebiegi podrzędne są automatycznie oznaczane jako ukończone.
...
Możesz użyć pola `Run.parent` , aby uzyskać dostęp do uruchomienia, w przypadku których uruchomiono bieżący przebieg podrzędny. Powszechnym zastosowaniem użycia jest `Run.parent` łączenie wyników dziennika w jednym miejscu. Należy pamiętać, że przebiegi podrzędne są wykonywane asynchronicznie i nie ma gwarancji, że kolejność lub synchronizacja nie będzie przekraczać możliwości oczekiwania przez element nadrzędny na ukończenie jego przebiegów.
...
* Aby dowiedzieć się, jak rejestrować metryki dla eksperymentów, zobacz [Rejestrowanie metryk podczas przebiegów trenowania.](how-to-track-experiments.md)
