...
Po wyzwoleniu zdarzenia usługa Event Grid wysyła dane dotyczące tego zdarzenia w celu subskrybowania punktu końcowego. Ta sekcja zawiera przykład sposobu, w jaki będą wyglądały dane dla każdego zdarzenia.
...
| `ServiceComputeType` | ciąg | Typ obliczeń (na przykład ACI, AKS) wdrożonej usługi. |
  | `ModelIds` | ciąg | Rozdzielana przecinkami lista identyfikatorów modeli. Identyfikatory modeli wdrożonych w usłudze. |
| `ServiceTags` | object | Tagi wdrożonej usługi. |
| `ServiceProperties` | object | Właściwości wdrożonej usługi. |
...
| `experimentName` | ciąg | Nazwa eksperymentu, do którego należy uruchomienie. |
...
| `DataDriftName` | ciąg | Nazwa monitora dryfowania danych, który wyzwolił zdarzenie. |
