...
## <a name="the-defender-for-iot-platform"></a>Usługa Defender for IoT
...
:::image type="content" source="media/integration-fortinet/firewall.png" alt-text="Widok okna zapory FortiGate.":::
...
1. Zapisz klucz interfejsu API.

    :::image type="content" source="media/integration-fortinet/api-key.png" alt-text="Opis telefonu komórkowego automatycznie generuje nowy klucz interfejsu API":::

## <a name="block-suspected-traffic-using-the-fortigate-firewall"></a>Blokuj podejrzany ruch przy użyciu zapory FortiGate

1. W lewym okienku wybierz pozycję **przekazywanie dalej**.

    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="Opcja okna przekazywania w czujniku.":::](media/integration-fortinet/forwarding-view.png#lightbox)

1. Wybierz pozycję **Utwórz reguły przekazywania** i zdefiniuj parametry reguł.

    :::image type="content" source="media/integration-fortinet/new-rule.png" alt-text="Zrzut ekranu przedstawiający okno reguła tworzenia przekazujące":::
    ...
    | **Protokoły** | Domyślnie są wybierane wszystkie protokoły.<br><br>Aby wybrać określony protokół, wybierz opcję **określony** i wybierz protokół, dla którego ma zostać zastosowana ta reguła. |
    | **Silnika** | Domyślnie są wykorzystywane wszystkie silniki zabezpieczeń.<br><br>Aby wybrać konkretny aparat zabezpieczeń, dla którego zostanie zastosowana ta reguła, wybierz pozycję **określone** i wybierz aparat. |
    | **Powiadomienia systemowe** | Stan *online czujnika i tryb offline* . Ta opcja jest dostępna tylko wtedy, gdy zarejestrowano się w lokalnej konsoli zarządzania. |

1. Aby poinstruować usługę Defender for IoT o wysłanie reguł zapory do FortiGate, wybierz opcję **Akcja** , a następnie wybierz pozycję **Wyślij do Fortigate**.

    :::image type="content" source="media/integration-fortinet/fortigate.png" alt-text="Utwórz okno reguły przekazywania i wybierz pozycję Wyślij do FortiGate":::

1. Aby skonfigurować regułę przekazywania FortiGate:

    :::image type="content" source="media/integration-fortinet/configure.png" alt-text="Skonfiguruj okno reguła tworzenia przekazujące":::

1. W okienku **Akcje** ustaw następujące parametry:

    | Parametr | Opis |
    |--|--|
    | Host | Typ adresu IP serwera FortiGate. |
    | Port | Typ portu serwera FortiGate. |
    | Nazwa użytkownika | Typ nazwy użytkownika serwera FortiGate. |
    | Klucz interfejsu API | Wprowadź klucz interfejsu API utworzony w FortiGate. |
    | Interfejs przychodzący| Zdefiniuj sposób blokowania:<br /><br />**Według adresów IP**: zawsze tworzy zasady blokowania na panoramze na podstawie adresu IP.<br /><br />**Według nazwy FQDN lub adresu IP**: tworzy zasady blokowania na panoramze na podstawie nazwy FQDN, jeśli istnieje, w przeciwnym razie adres IP.| 
    | Interfejs wychodzący |Ustaw adres e-mail dla wiadomości e-mail z powiadomieniem o zasadach. <br /><br /> **Uwaga**: Upewnij się, że skonfigurowano serwer poczty w programie XSense. Jeśli adres e-mail nie zostanie wprowadzony, XSense nie wyśle wiadomości e-mail z powiadomieniem.| 
    |Konfigurowanie| Skonfiguruj następujące opcje, aby umożliwić blokowanie podejrzanych źródeł przez zaporę FortiGate: <br /><br />**Blokuj niedozwolone kody funkcji**: naruszenia protokołu — niedozwolona wartość pola narusza specyfikację protokołu ICS (potencjalną lukę w zabezpieczeniach)<br /><br />**Blokuj nieautoryzowane aktualizacje programów PLC/oprogramowania układowego**: nieautoryzowane zmiany PLC<br /><br />**Blokuj nieautoryzowane zatrzymywanie PLC**: PLC Stop (przestój)<br /><br />**Zablokuj alerty dotyczące złośliwego oprogramowania**: blokowanie prób związanych ze złośliwym oprogramowaniem (Triton, NotPetya itp.). Można wybrać opcję **automatycznego blokowania**. W takim przypadku blokowanie jest wykonywane automatycznie i od razu.<br /><br />*Blokuj nieautoryzowane skanowanie*: nieautoryzowane skanowanie (potencjalna Rekonesans)
